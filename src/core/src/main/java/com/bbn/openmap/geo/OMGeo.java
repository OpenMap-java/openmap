...
public abstract class OMGeo extends OMGraphicAdapter implements GeoExtent {
	...
	public static class Pt extends OMGeo implements GeoPoint {
		...
		public boolean generate(Projection proj) {
			...
			if (isOval) {
				setShape(new GeneralPath(new Ellipse2D.Float((float) Math.min(x2, x1), (float) Math.min(y2, y1),
						(float) Math.abs(x2 - x1), (float) Math.abs(y2 - y1))));
			} else {
		...
		public boolean generate(Projection proj) {
			...
			if (proj instanceof GeoProj) {
				lines = ((GeoProj) proj).forwardLine(new LatLonPoint.Double(latlons[0], latlons[1]),
						new LatLonPoint.Double(latlons[2], latlons[3]), lineType, -1);
			} else {
	...
	public static class Polyline extends OMGeo implements GeoPath {
		...
		protected boolean isGeometryClosed(float[] rawllpts) {
			...
			if (rawllpts != null) {
				...
				if (l > 4) {
					geometryClosed = (MoreMath.approximately_equal(rawllpts[0], rawllpts[l - 2])
							&& MoreMath.approximately_equal(rawllpts[1], rawllpts[l - 1]));
				}
			}
	...
	public static OMGeo create(GeoExtent extent) {
		if (extent instanceof GeoPoint) {
			return new OMGeo.Pt((GeoPoint) extent);
		} else if (extent instanceof GeoRegion) {
			return new OMGeo.Polygon((GeoRegion) extent);
		} else if (extent instanceof GeoSegment) {
			return new OMGeo.Line((GeoSegment) extent);
