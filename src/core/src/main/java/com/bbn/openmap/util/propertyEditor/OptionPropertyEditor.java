...
/**
 * The base class for property editors that support a list of options being
 * presented in the GUI. When defining the properties that the property editor
 * will use for a PropertyConsumer in the getPropertyInfo method, just use the
 * name of the property that the options are being used for:
 * 
 * <pre>
 * 
 * 
 ...
 * </pre>
 * 
 * Don't use the property prefix for the PropertyConsumer being defined, only
 * the property.
 */
public abstract class OptionPropertyEditor extends PropertyConsumerPropertyEditor {
	...
	public void setProperties(String prefix, Properties props) {
		...
		if (optionListProperty != null) {
			List<String> optionVector = PropUtils.parseSpacedMarkers(optionListProperty);
			options = new String[optionVector.size()];
			for (int i = 0; i < options.length; i++) {
				options[i] = props.getProperty(prefix + optionVector.get(i));
			}
