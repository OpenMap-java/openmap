...
public class RoadFinder implements RoadServices, ProjectionListener, RoadLayer {
	...
	protected RoadList roads = new RoadList();

	protected List<Road> removedRoads = new ArrayList<>();
	...
	protected QuadTree<Intersection> interQuadTree;
	...
	protected void getRoads() throws Exception {
		roadsMade = 0;
		OMGraphicList rectangle = layer.getOMGraphicsFromSource();
		...
		synchronized (rectangle) {
			...
			int num = 0;
			int made = 0;
			for (OMGraphic graphic : rectangle) {
				double lastXOff = 0;
				double lastYOff = 0;
					...
					case PathIterator.SEG_LINETO:
						if (offScreen) {
							if (segment > 0) {
								...
								if (doInterp) {
									Point interpPt = interp(xPoints[segment - 1], yPoints[segment - 1], points[0],
	...
	protected Point interp(double x1, double y1, double x2, double y2, int width, int height) {
		...
		if (logger.isLoggable(Level.INFO)) {
			logger.info("from " + x1 + "," + y1 + " to " + x2 + "," + y2 + "w " + width + " h " + height + " interp "
					+ intX + "," + intY);
		}
	...
	/**
	 * Makes a road object given the points on the shape that are within the visible
	 * box
	 * 
	...
	protected RoadObject createRoadFromPoints(int id, int[] xpoints, int[] ypoints, int nPoints) {
			...
			if (drawIntersections) {
				if (showLines) {
					...
					toDraw.add(new OMText((xpoints[i - 1] - xpoints[i]) / 2 + xpoints[i - 1],
							(ypoints[i - 1] - ypoints[i]) / 2 + ypoints[i - 1] - 5, "" + roadsMade, 0));
				} else {
		...
		if (drawIntersections) {
			if (showLines) {
				OMLine line = new YellowLine(xpoints[nPoints - 2], ypoints[nPoints - 2], xpoints[nPoints - 1],
						ypoints[nPoints - 1], width);
				toDraw.add(line);
				toDraw.add(new OMText((xpoints[nPoints - 2] - xpoints[nPoints - 1]) / 2 + xpoints[nPoints - 2],
						(ypoints[nPoints - 2] - ypoints[nPoints - 1]) / 2 + ypoints[nPoints - 2] - 5, "" + roadsMade,
						0));
	...
	protected Intersection findIntersection(LatLonPoint loc, String name) {
		...
		if (intersection != null) {
			...
			if (distance * Intersection.GRID > 0.1f) {
				...
				// too far away.
				System.out
						.println("Using " + intersection.getName() + " instead of " + name + " distance = " + distance);
				return intersection;
			}
		} else {
	...
	/**
	 * Split a road into two roads at one of its corners. An intersection is created
	 * where the corner was and the segments before the corner become the segments
	 * of the original road. The segments after the corner become the segments of a
	 ...
	 * then we quietly delete the road and remove the intersection. The roads might
	 * not be distinct if they form an isolated loop (such as a racetrack). Thus
	 * situation is particularly problematic if the road has no inner points. The
	 * RoadPoints of both roads are concatenated with a new RoadPoint where the
	 * intersection was between them. This code is a little complicated because the
	 * RoadPoints must be assembled in a valid order. The order used is to start
	 * from the other intersection of the first road to the given intersection and
	 * from the given intersection of the second road to the other intersection of
	 * the second road.
	 */
	 ...
	 * @param segments as side effect, populated with PathSegments between returned
	 *                 WayPoints
	...
	/**
	 * Finds closest intersection to start and end find path from start intersection
	 * to end intersection
	 * <p>
	 * 
	...
	public Route getPathOnRoad(LatLonPoint start, LatLonPoint end) {
		...
		if (startTemp != null && endTemp != null) {
			...
			bestRoute = Route.getBestRoute(startTemp, endTemp, roadClasses.getBestConvoySpeed(),
	...
	/**
	 * Look in intersection Quad Tree for closest intersection to point at specified
	 ...
	 * start of the next. This is not guaranteed by the route, so we have to check.
	 * 
	 ...
	 */
	protected void populatePointsAndSegments(Route bestRoute, List<Point> newPoints, List<Segment> segments) {
		...
		Set<Intersection> loopSet = new HashSet<>();
		if (doLoopCheck) {
			loopSet.add(origin);
		}
	 ...
	 */
	protected Segment createSegment(List<Point> newPoints) {
	 ...
	 * roads of the intersection: Each road has two intersections. If the road has
	 * already been encountered, then we recorded its "other" intersection and that
	 * must match this intersection. If it doesn't match, record an error. If it
	 * does match reset its recorded other intersection to be a special marker
	 * indicating that both ends of the road have been accounted for. If the road
	 * has not already been encountered, then record its "other" intersection. Scan
	 * the known roads. Every road should accounted for in the "other" intersection
	 * table and should be marked as having both intersections accounted for. Note
	 * the roads which were not found in the first scan and the roads which were
	 * found, but for which both intersections were not found. Remark every road.
	 * Finally scan the other intersection table for entries which were not marked
	...
	protected void checkIntegrity() {
		...
		Map<Road, Intersection> otherIntersections = new HashMap<>();

		for (Intersection intersection : intersections.getIntersections()) {
			...
			for (Road road : intersection.getRoads()) {
				...
				if (other == null) {
					...
					otherIntersections.put(road, Intersection.INTEGRITY_CHECK_BOTH_INTERSECTIONS);
				} else {
		...
		for (Road road : roads.getRoads()) {
			...
			Intersection other = otherIntersections.get(road);
			if (other == null) {
				errors.println("Road not found in intersections");
				errors.println("          Road = " + road);
			} else if (other != Intersection.INTEGRITY_CHECK_BOTH_INTERSECTIONS) {
				errors.println("Road incompletely connected");
				errors.println("          Road = " + road);
				errors.println("    Road.Other = " + other);
			} else if (other == Intersection.INTEGRITY_CHECK_INROADS) {
				errors.println("Road doubly listed");
				errors.println("          Road = " + road);
			}
			
			otherIntersections.put(road, Intersection.INTEGRITY_CHECK_INROADS);
		}

		for (Road road : otherIntersections.keySet()) {
			Object other = otherIntersections.get(road);
			if (other != Intersection.INTEGRITY_CHECK_INROADS) {
	...
	public static class RoadClasses extends HashMap<String, RoadClass> {
		private static final long serialVersionUID = 1L;
