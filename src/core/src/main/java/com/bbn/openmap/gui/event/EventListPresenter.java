...
public class EventListPresenter extends AbstractEventPresenter implements EventPresenter, ListSelectionListener,
	...
	protected LinkedList<FilterPresenter> macroFilters;
	protected EventPanel parentPanel;
	protected JList<OMEvent> displayList;
	protected Clock clock = null;
	protected MapBean map = null;
	protected List<String> activeFilters;
	protected Map<String, Boolean> filters = new Hashtable<String, Boolean>();
	...
	public EventListPresenter() {
		...
		macroFilters = new LinkedList<FilterPresenter>();
	...
	public void setProperties(String prefix, Properties props) {
		...
		if (crc != null) {
			setEventCellRenderer((EventListCellRenderer) ComponentFactory.create(crc, prefix, props));
		}

		displayTimeWindow = PropUtils.longFromProperties(props, prefix + DisplayIntervalProperty, displayTimeWindow);

		prefWidth = PropUtils.intFromProperties(props, prefix + PreferredWidthProperty, prefWidth);
		prefHeight = PropUtils.intFromProperties(props, prefix + PreferredHeightProperty, prefHeight);
	...
	public synchronized Iterator<OMEvent> getActiveEvents() {
		if (activeEvents == null) {
			...
			for (OMEventHandler aeh : eventHandlers) {
	 ...
	 *            the side effect of resetting the clock. You want this to be
	 *            false when event handlers are being added.
	 */
	protected synchronized void initInterface(Iterator<OMEvent> it, boolean setSelected) {
		...
		if (displayList == null) {
			...
			displayList = new JList<OMEvent>(listModel);
	...
	public EventListCellRenderer getEventCellRenderer() {
	...
	protected void rebuildFilterInterface() {
		...
		try {
			for (String title : filters.keySet()) {
	...
	public List<String> getActiveFilters() {
		if (activeFilters == null) {
			activeFilters = new LinkedList<String>();
	...
	public void retrieveFiltersFromEventHandlers() {
		...
		for (Iterator<OMEventHandler> it = eventHandlers.iterator(); it.hasNext();) {
			...
			// filter interface.
			List<String> filters = meh.getFilters();

			if (filters != null) {
				for (String filter : filters) {
					Boolean val = meh.getFilterState(filter);
					addFilter(filter, val != null ? val : Boolean.FALSE);
				}
			}
		}
		rebuildFilterInterface();
	...
	protected synchronized void setListModel(ListModel<OMEvent> lm) {
		displayList.setModel(lm);
	}

	protected synchronized ListModel<OMEvent> getListModel() {
	...
	public void valueChanged(ListSelectionEvent e) {
		if (e.getValueIsAdjusting() == false && displayList != null) {
			...
			if (indicies.length > 0) {
				ListModel<OMEvent> listModel = getListModel();
				OMEvent selectedEvent = listModel.getElementAt(indicies[0]);
				...
				for (Iterator<OMEvent> it = getAllEvents(); it.hasNext();) {
					((OMEvent) it.next()).putAttribute(OMEvent.ATT_KEY_SELECTED, null);
				}
				...
				for (int i = 0; i < indicies.length; i++) {
					...
					selectedEvent.putAttribute(OMEvent.ATT_KEY_SELECTED, OMEvent.ATT_VAL_SELECTED);
					...
					if (curIndex == lastIndex + 1) {
						...
						lastSelectedEvent.putAttribute(OMEvent.ATT_KEY_SELECTED, OMEvent.ATT_VAL_SELECTED_END_RANGE);
						inRange = false;
					}
	...
	public void findAndInit(Object someObj) {
		...
		if (someObj instanceof FilterPresenter && someObj != this) {
			FilterPresenter filterPresenter = (FilterPresenter) someObj;
			macroFilters.add(filterPresenter);
			filterPresenter.addPropertyChangeListener(this);
		}
		...
		if (someObj instanceof TimePanel) {
			((TimePanel) someObj).setPlayFilterVisible(getEventCellRenderer().getIconPackage().isShowPlayFilter());
		}
	}
	...
	public void findAndUndo(Object someObj) {
		...
		if (someObj instanceof FilterPresenter && someObj != this) {
			FilterPresenter filterPresenter = (FilterPresenter) someObj;
			macroFilters.remove(filterPresenter);
			filterPresenter.removePropertyChangeListener(this);
		}
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		if (eventPropertyName == OMEventSelectionCoordinator.EventsSelectedProperty) {
			Object someObj = evt.getNewValue();
			if (someObj instanceof List) {
	...
	protected void scrollToSelected(List<OMEvent> selectedEvents) {
		if (selectedEvents != null && !selectedEvents.isEmpty()) {
			...
			ListModel<OMEvent> dlm = getListModel();
	...
	protected synchronized void highlightCurrentEvent(long newCurrentTime) {
		...
		ListModel<OMEvent> dlm = displayList.getModel();
		boolean timeMarked = false;
		OMEvent lastClosest = null;
		int size = dlm.getSize();
		for (int i = 0; i < size; i++) {
			OMEvent event = dlm.getElementAt(i);
			long timeStamp = event.getTimeStamp();
	...
	public void showDetails(String contents) {
		...
		if (contents != null) {
			detailSpace.setText("<html><body bgcolor=\"#ffffcc\"> <font style=\"plain\">" + contents + "</html>");
		}
	...
	protected int getDisplayListIndex(MouseEvent e) {
		...
		if (displayList != null) {
			...
			if (logger.isLoggable(Level.FINE)) {
				logger.fine("For " + e.getY() + ", the first vis: " + displayList.getFirstVisibleIndex()
						+ ", heights are " + height + ", so index of click is " + index);
			}
		}
	...
	protected JComponent getEventControlPanel() {
		if (eventControlPanel == null) {
			eventControlPanel = cellRenderer.getIconPackage().createEventControlPanel(this);
		}
