...
public class VPFAutoFeatureGraphicWarehouse implements VPFFeatureWarehouse, PropertyConsumer {
	...
	/**
	 * If set, the warehouse will limit visibility to specified facc and print out
	 * decision making process.
	 */
	protected String debugFacc = null;

	/**
	 * Set which library to use. If null, all applicable libraries in database will
	 * be searched.
	 */
	 ...
	 * water areas (shallow depth zones) to be one of two symbology scenarios. The
	 * attribute values are 1 and 0, which toggle the shallow display mode to be on
	 * or off respectively. When ISDM is set on (1), the display of all depth zones
	 * shallower than the defined values of the Ships Safety Depth Contour (SSDC)
	 * are overprinted with a lattice pattern. This mode can be initiated in the
	 * four- or two-depth zone display modes (not including the drying line),
	 * defined by the Interactive Display Selection Mode, (IDSM). The shallow
	 * display mode is made available due to viewing limitations of the shallow
	 * depth zones in night displays.
	 */
	protected double isdm = 0;
	/**
	 * Interactive Display Selection Mode:ECDIS defines the display of depth zones
	 * to be divided into two or four depth areas. This variable allows for the
	 * mariner to interactively set either display mode. The two-zone mode uses only
	 * the ships safety depth contour (SSDC) as a zone separator, whereas the
	 * four-zone mode further divides zones based on the mariner selected deep and
	 * shallow contours (MSDC, MSSC). Attribute values are 0 and 1 meaning four- and
	 * two-zone modes respectively.
	 */
	protected double idsm = 0;
	/**
	 * Ship's Safety Depth Contour: The ships safety depth contour represents a safe
	 * contour based on the draft of the ship. This value must be entered by the
	 * mariner using an application interface. This interface must ensure that if a
	 * contour value does not exist within the data, that a next deeper value is
	 * specified as the SSDC. This checking must be dynamic as one traverses tile
	 * boundaries within the data..
	 */
	protected double ssdc = 5;
	/**
	 * Mariner Specified Deep Contour: The four-zone display mode requires the
	 * establishment of a deep contour that must be specified by the mariner through
	 * application inquiry. A default value may be implemented at 30m according to
	 * the ISO Color and Symbol Specification directives.
	 */
	protected double msdc = 30;
	/**
	 * Mariner Specified Shallow Contour - The four-zone display mode requires the
	 * establishment of a shallow contour that must be specified by the mariner
	 * through application inquiry. A default value may be implemented at 2m
	...
	protected void init() {
		...
		try {
			...
			for (List<Object> row : priorityFile) {
				...
				try {
					...
					logger.warning(
							"Bad entry in priority file: " + lineCheck + "," + type + "," + facc + "," + conditions);
					continue;
				}
			...
			int foundRecords = 0;
			for (List<Object> row : symbolLookupFile) {
				...
				if (faccList != null) {
					...
					if (!found) {
						...
						if (logger.isLoggable(Level.FINE)) {
							logger.fine("didn't find matching PriorityHolder for " + facc + "|" + type + "|"
									+ symbolCode + "|" + conditions);
						}
					}
					...
					if (debugFacc == null) {
						if (logger.isLoggable(Level.FINE)) {
							logger.fine("can't find faccLookup for " + facc + " for" + type + "|" + symbolCode + "|"
									+ conditions);
						}
					}
				}
			...
			if (logger.isLoggable(Level.FINE)) {
				logger.fine("matched up " + foundRecords + " of " + numSymbols + " symbols, " + numPriorities
						+ " priority entries");
			}
	...
	public OMGraphic createArea(CoverageTable covtable, AreaTable areatable, List<Object> facevec, LatLonPoint ll1,
		...
		OMPoly py = LayerGraphicWarehouseSupport.createAreaOMPoly(ipts, totalSize, ll1, ll2, dpplat, dpplon,
	 ...
	 *
	 */
	public OMGraphic createEdge(CoverageTable c, EdgeTable edgetable, List<Object> edgevec, LatLonPoint ll1,
			LatLonPoint ll2, double dpplat, double dpplon, CoordFloatString coords, String featureType, int primID) {
	 ...
	 *
	 */
	public OMGraphic createText(CoverageTable c, TextTable texttable, List<Object> textvec, double latitude,
			double longitude, String text, String featureType, int primID) {
	...
	public OMGraphicList getFeatures(LibrarySelectionTable lst, LatLonPoint ll1, LatLonPoint ll2, Projection proj,
			OMGraphicList omgList) throws FormatException {
		...
		/*
		 * BoundingCircle screenBounds = new GeoSegment.Impl(new Geo[] { new
		 * Geo(ll1.getLatitude(), ll1.getLongitude()), new Geo(ll2.getLatitude(),
		 * ll2.getLongitude()) }).getBoundingCircle();
		 */
		...
		for (String libraryName : lst.getLibraryNames()) {
			...
			if (bounds != null) {
				/*
				 * Point2D min = bounds.getMin(); Point2D max = bounds.getMax(); BoundingCircle
				 * catCircle = new GeoSegment.Impl(new Geo[] { new Geo(min.getY(), min.getX()),
				 * new Geo(max.getY(), max.getX()) }).getBoundingCircle();
				 */
			...
			for (String covname : cat.getCoverageNames()) {
				if (logger.isLoggable(Level.FINER)) {
					logger.finer("for coverage: " + covname + ", coverage topology level: "
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.bbn.openmap.PropertyConsumer#getPropertyInfo(java.util.Properties)
	 */
	public Properties getPropertyInfo(Properties list) {
		...
		PropUtils.setI18NPropertyInfo(i18n, list, VPFAutoFeatureGraphicWarehouse.class, PRIORITY_FILE_PROPERTY,
				"Priority File", "The path to the file containing feature type and order to use for display",
				"com.bbn.openmap.util.propertyEditor.FilePropertyEditor");
		PropUtils.setI18NPropertyInfo(i18n, list, VPFAutoFeatureGraphicWarehouse.class, CGM_DIR_PROPERTY,
				"CGM Directory Path", "The path to the directory containing GeoSym CGM files",
				"com.bbn.openmap.util.propertyEditor.DirectoryPropertyEditor");
		PropUtils.setI18NPropertyInfo(i18n, list, VPFAutoFeatureGraphicWarehouse.class, FACC_DEBUG_PROPERTY,
				"FACC Debug", "A FACC code to use to debug problems with data set", null);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.bbn.openmap.layer.vpf.VPFWarehouse#getGUI(com.bbn.openmap.layer.vpf
	...
	/**
	 * Set the object used to manage attribute formatting and display for features
	 * on the map.
	 * 
	 ...
	 * feature types to be rendered. It is responsible for evaluating attributes of
	 * a VPF feature and determining if a particular feature matches what this
	...
	protected static abstract class FeaturePriorityHolder {
		...
		/**
		 * A handle to any debug FACC code listed by the warehouse, so that a specific
		 * type of feature can be singled out for debugging.
		 */
		protected String debugFacc = null;
		 ...
		 */
		public abstract boolean matches(String facc, char type, String conditions, String symbolFileName, String size,
				String xoff, String yoff);

		protected abstract void add(OMGraphic omg);

		protected static class Basic extends FeaturePriorityHolder {
					...
					try {
						...
						if (cgmTitle == null) {
							...
							for (int i = 0; i < cgmTitle.length; i++) {
								...
								icon = cgmDisplay[i].getBufferedImage((int) (dim * sizePercent),
										(int) (dim * sizePercent));
							}
						}
			 ...
			 */
			public boolean matches(String facc, char type, String conditions, String symbolFileName, String size,
					String xoff, String yoff) {
				boolean basicMatch = this.facc.equals(facc) && type == this.type;

				boolean conditionMatch = ((this.conditions == null || this.conditions.trim().length() == 0)
				...
				if (ret) {
					List<String> names = PropUtils.parseSpacedMarkers(symbolFileName);
			 ...
			 * (non-Javadoc)
			 * 
			 * @see com.bbn.openmap.layer.vpf.VPFAutoFeatureGraphicWarehouse. PriorityHolder
			 * #getConditions()
			 */
		...
		/**
		 * A Compound FeaturePriorityHolder is used for buoys and other features that
		 * have parts added to their representation based on their feature attributes.
		 * It contains a list of Basic FeaturePriorityHolders, and each one adds its
		 * touch to the resulting OMGraphic as needed.
		 * 
		 * @author dietrick
		 */
		protected static class Compound extends FeaturePriorityHolder implements ImageObserver {
			...
			/**
			 * Used to match features with PriorityHolder. We need to do a little more work
			 * here, to build up an image that matches the all of the attributes set on this
			 * feature. So if the feature matches at the first level, walk through the parts
			...
			public boolean matches(String facc, FeatureClassInfo fci, List<Object> row) {
				...
				/**
				 * Building up the current image based on the attributes. Give each part a
				 * chance to evaluate whether it should be rendered into the image or not.
				 */
				if (facc.equals(this.facc) && this.type == type) {
			...
			public boolean matches(String facc, char type, String conditions, String symbolFileName, String size,
				...
				/**
				 * We need to step through each part so that each part can find it's symbol.
				 */
				for (FeaturePriorityHolder.Basic part : parts) {
