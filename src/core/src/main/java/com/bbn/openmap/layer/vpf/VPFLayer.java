...
public class VPFLayer extends OMGraphicHandlerLayer implements ProjectionListener, ActionListener, Serializable {
	...
	/**
	 * Property that lets you search for graphics via feature type. Dangerously slow
	 * for features that have many graphics spread out over several tiles. Set to
	 * true to search by feature, false (default) to get the tiles first, and then
	 * look for graphics.
	 */
	...
	public void setProperties(String prefix, Properties props) {
		...
		searchByFeatures = PropUtils.booleanFromProperties(props, realPrefix + searchByFeatureProperty,
				searchByFeatures);
			...
			if (libraryBeanName != null) {
				...
				if (dataPaths == null) {
					...
					try {
						...
						StringBuilder buf = new StringBuilder("VPFLayer|").append(getName()).append(": problem with path: ");
						if (dataPaths != null) {
							for (String dPath : dataPaths) {
								buf.append("\n\t").append(dPath);
							}
						} else {
							buf.append(" NO DATA PATHS SPECIFIED");
						}
	...
	/**
	 * Use doPrepare() method instead. This was the old method call to do the same
	 * thing doPrepare is now doing, from the OMGraphicHandler superclass.
	...
	public synchronized OMGraphicList prepare() {
		...
		while (t.hasMoreTokens()) {
			...
			if (searchByFeatures) {
				lst.drawFeatures((int) p.getScale(), p.getWidth(), p.getHeight(), currentCoverage,
						(VPFFeatureWarehouse) warehouse, upperleft, lowerright);
			} else {
				lst.drawTile((int) p.getScale(), p.getWidth(), p.getHeight(), currentCoverage, warehouse, upperleft,
						lowerright);
			}
		}
	...
	public Component getGUI() {
		...
		if (box == null) {
			...
			pal = PaletteHelper.createCheckbox("Show:",
					new String[] { VPFUtil.Edges, VPFUtil.Text, VPFUtil.Area, VPFUtil.EPoint, VPFUtil.CPoint },
					new boolean[] { warehouse.drawEdgeFeatures(), warehouse.drawTextFeatures(),
							warehouse.drawAreaFeatures(), warehouse.drawEPointFeatures(),
