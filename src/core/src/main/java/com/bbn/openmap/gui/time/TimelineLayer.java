...
public class TimelineLayer
	...
	/**
	 * This property is used to signify whether any OMEvents have been designated as play filterable, so GUI controls
	 * for the play filter can be enabled/disabled.
	 */
	...
	/**
	 * This property is used to send event details that can be displayed when the mouse is over an event in the
	 * timeline.
	 */
	public final static String EventDetailsProperty = "eventDetails";
	/**
	 * This property is used to notify listeners that the time projection parameters have changed, and they need to
	 * contact this object to figure out how to display those changes.
	 */
	...
	public TimelineLayer() {
		...
		// received.
//		setProjectionChangePolicy(new NullProjectionChangePolicy(this));
	...
	public SelectionArea getSelectionRectangle() {
		return selectionRect;
	}

	public SelectionArea getSelectionRectangle(Projection proj) {
		SelectionArea selectRect = getSelectionRectangle();

		if (selectRect == null) {
			selectionRect = new SelectionArea();
			selectRect = selectionRect;
			if (eventPresenter != null) {
				selectRect.setFillPaint(eventPresenter.getSelectionDrawingAttributes().getSelectPaint());
			}
		}
		...
		if (selectRect.isVisible()) {
			...
			selectionRectToRender.setLocation(selectRect.getWestLon(), selectRect.getEastLon());
	...
	protected OMGraphicList getEventList(Projection projection) {
		...
		if (eventPresenter != null) {
			...
			if (eventPresenter instanceof AbstractEventPresenter) {
				...
				eventGraphicList = getEventList(((AbstractEventPresenter) eventPresenter).getActiveEvents(start, end,
					step),
					projection);
			} else {
	...
	public void updateTimeBounds(TimeBoundsEvent tbe) {
		...
		if (tb != null) {
			...
			if (realTimeMode && selectRect != null && selectRect.isVisible()) {
				...
				double newWest = selectRect.getWestLon() - selectionDelta;
				double newEast = selectRect.getEastLon() - selectionDelta;
				selectRect.setLocation(newWest, newEast);
	...
	public boolean mousePressed(MouseEvent e) {
		...
		if (selectRect != null) {
			...
			selectRect.setLocation(up,
				(float) lon,
				down,
				(float) lon,
				OMGraphic.LINETYPE_STRAIGHT);
			selectRect.generate(projection);
		}
		timeSliderLayer.setSelectionValid(false);
	...
	public boolean mouseClicked(MouseEvent e) {
		...
		SelectionArea selectionRect = getSelectionRectangle();
	...
	private void adjustSelection(MouseEvent e) {
		...
		if (selectRect != null) {
			...
			selectRect.setLocation(west, east);
			selectRect.generate(proj);
		}
	...
	protected List<OMEvent> handleEventSelection() {
		...
		if (aesc != null && selectRect != null) {
			...
			boolean goodDrag = selectRect.isVisible();
			double lowerTime = selectRect.getWestLon();
			double upperTime = selectRect.getEastLon();
			// Convert to millis
	...
	protected void setSelectionRectangleToEvents() {
		if (aesc != null) {
			SelectionArea selectRect = getSelectionRectangle(getProjection());
			...
			if (selectRect != null) {
				if (upperTime != Double.NEGATIVE_INFINITY
					...
					selectRect.setLocation((float) lowerTime, (float) upperTime);
					selectRect.setVisible(true);
					selectRect.generate(getProjection());
				} else {
					selectRect.setVisible(false);
				}
			}
	...
	public static class SelectionArea
		extends com.bbn.openmap.omGraphics.OMRect {
		...
		public static class PlayFilterSection
			 ...
			 *
			 * @param timel time in unprojected milliseconds, offset from game start time.
			 * @return 0 if time is within bounds, -1 if time is before bounds, 1 if time is after bounds.
			 */
	...
	public abstract static class TimeHashMarks
		extends OMGraphicList {
			...
			if (realTimeMode) {
				...
				for (double i = stepStart; i < stepEnd; i += stepSize, count++) {
					...
					if (doLabel) {
						...
						OMText label = new OMText((float) anchory, (float) anchorx, 2, -5, labelString,
							OMText.JUSTIFY_LEFT);
	...
	protected void setMapBeanMaxScale(boolean setScaleToMax) {
		...
		if (proj != null) {
			float scale = (float) (TimeSliderLayer.magicScaleFactor
				* (double) forwardProjectMillis(gameEndTime - gameStartTime) / proj.getWidth());
