...
/**
 * An OpenMap Layer that displays outlines of a shapefile,
 * extended or shrunk by some amount ["ribbon"]. You can tell it which specific
 * shape you want in a shapefile with dbfcolumnchoice and defaultdbfcolumnvalue
 * or includeAllShapes to avoid filtering. The gui for the layer is populated
 * with values from the requested DBF column.
 * 
 * <p>
 * <code><pre>
...
public class ShapeRibbonLayer extends OMGraphicHandlerLayer {
    ...
    public void setProperties(String prefix, Properties props) {
        ...
        String lengthUnitProp = String.valueOf(PropUtils.objectFromProperties(props, realPrefix + rangeUnitsProperty));
        ...
        double originalRangeValue = PropUtils.doubleFromProperties(props, realPrefix + rangePropertyProperty, units.fromRadians(rangeRadians));
        rangeRadians = units.toRadians(originalRangeValue);
        
        removeShapeFromFill = PropUtils.booleanFromProperties(props, realPrefix + removeShapeFromFillProperty, removeShapeFromFill);
        includeAllShapes = PropUtils.booleanFromProperties(props, realPrefix + includeAllShapesProperty, includeAllShapes);
    ...
    /**
     * Create the esriLayer object. Assume that .shp is accompanied by .dbf and .shx files
     */
    ...
    protected static class OMProjectedShape extends OMShape {
        OMProjectedShape(Shape s) {
            super(s);
    ...
    public Component getGUI() {
        ...
        final JSpinner rangeSpinner = new JSpinner(new SpinnerNumberModel(units.fromRadians(rangeRadians), -1000.0, 1000.0, 100.0));
    ...
    public Area createAreaFromEsri(OMGraphic g, double distRadians) {
        ...
        if(g instanceof EsriGraphicList) {
            ...
            if(Math.abs(distRadians) <= epsilon) {
                ...
                // Wind the array backwards for negative values. This costs no extra memory
                if(distRadians < 0) {
        ...
        if(null != ribbonArea) {
            if(removeShapeFromFill || rangeRadians < 0) {
                ...
                if(rangeRadians > 0) {
                    ...
                    // If the ribbon is inside, always subtract the ribbon from the shape
