...
public abstract class Azimuth extends GeoProj {
	...
	protected void computeParameters() {
		...
		if (Debug.debugging("proj")) {
			Debug.output("Azimuth.computeParameters(): " + "world.x = " + world.x + " XSCALE_THRESHOLD = "
					+ XSCALE_THRESHOLD);
		}
	}
	 ...
	 * @return List float[] of x[], y[], x[], y[], ... projected poly
	 */
	protected ArrayList<float[]> _forwardPoly(float[] rawllpts, int ltype, int nsegs, boolean isFilled) {
		...
		if (prev_invalid) {
			...
			azVar.index = 0;
			azVar.current_azimuth = GreatCircle.sphericalAzimuth((float) centerY, (float) centerX, rawllpts[0],
					rawllpts[1]);
		...
		for (i = 1, j = 2; i < len; i++, j += 2) {
			...
			if (!curr_invalid && prev_invalid) {
				...
				azVar.current_azimuth = GreatCircle.sphericalAzimuth((float) centerY, (float) centerX, rawllpts[j - 2],
						rawllpts[j - 1]);
	...
	protected ArrayList<float[]> _forwardPoly(double[] rawllpts, int ltype, int nsegs, boolean isFilled) {
		...
		for (i = 1, j = 2; i < len; i++, j += 2) {
			...
			if (!curr_invalid && prev_invalid) {
				// record transition (outside -> inside)
				azVar.index = i - 1;// include outside point
				azVar.current_azimuth = (float) GreatCircle.sphericalAzimuth(centerY, centerX, rawllpts[j - 2],
						rawllpts[j - 1]);
				// Debug.output("marker oi="+azVar.index+
				// " az="+ProjMath.radToDeg(azVar.current_azimuth));
				sections.add(azVar);
	...
	private double[] getHemisphereEdge(double oiAz, double ioAz) {
		...
		if (npts == 0)
			++npts;
		double[] radpts = GreatCircle.greatCircle(ll1.getRadLat(), ll1.getRadLon(), ll2.getRadLat(), ll2.getRadLon(),
				npts, true);
	 ...
	 *         visible.
	 */
	public boolean forwardRaw(float[] rawllpts, int rawoff, float[] xcoords, float[] ycoords, boolean[] visible,
	 ...
	 *         visible.
	 */
	public boolean forwardRaw(double[] rawllpts, int rawoff, float[] xcoords, float[] ycoords, boolean[] visible,
			int copyoff, int copylen) {
		Point temp = new Point();
		AzimuthVar azVar = new AzimuthVar();
		boolean ok = true;
