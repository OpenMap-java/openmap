...
public abstract class EsriGraphicList extends OMGraphicList implements ShapeConstants, EsriGraphic {
	 ...
	 */
	public static EsriGraphicList getEsriGraphicList(URL shp, URL shx, DrawingAttributes drawingAttributes,
			DbfTableModel dbf) {
	 ...
	 *            edge, clear fill) will be used if this is null.
	 * @param coordTranslator
	 *            used to transform coordinates to lat/lon decimal degrees for
	 *            OpenMap.
	 * @return A new EsriGraphicList, null if there is a problem.
	 */
	 ...
	 * @return new List<EsriGraphic> of shapes with value in column
	 * @throws FormatException
	 *             thrown if columnName isn't found.
	 */
			...
			if (shape.endsWith(".shp")) {
				...
				try {
					...
					if (contentLength != indexedContentLength) {
						System.out.println(
								shape + " content length - shp: " + contentLength + ", shx: " + indexedContentLength);
						raf.seek(24);
