...
public class CSVEventImporter
	...
	/**
	 * TimeFormat default is similar to IETF standard date syntax: "Sat, 12 Aug 1995 13:30:00 GMT" represented by (EEE,
	 * d MMM yyyy HH:mm:ss z), except for the local timezone.
	 */
	...
	/**
	 * Read the data files and construct the TemporalOMGraphics. You also need to create TimeBounds, keep track of the
	 * time stamps from the data source, and set the new TimeBounds on the EventLayer before returning from this method.
	 * If you want to set the DataBounds on the layer, in order for the view menu to have a selection for the area of
	 * interest, fetch the DataBounds object and set it accordingly while you are in this method.
	 * <p>
	 ...
	 * that would be helpful (range, default value, etc.). This method takes care of the basic LocationHandler
	 * parameters, so any LocationHandlers that extend the AbstractLocationHandler should call this method, too, before
