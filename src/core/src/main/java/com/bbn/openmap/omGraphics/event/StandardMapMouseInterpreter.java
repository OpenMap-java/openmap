...
/**
 * The StandardMapMouseInterpreter is a basic implementation of the MapMouseInterpreter, working with an
 * OMGraphicHandlerLayer to handle MouseEvents on it. This class allows the OMGraphicHandlerLayer, which implements the
 * GestureResponsePolicy, to not have to deal with MouseEvents and the OMGraphicList, but to just react to the meanings
 * of the user's gestures.
 * <p>
 *
 * The StandardMapMouseInterpreter uses highlighting to indicate that mouse movement is occurring over an OMGraphic, and
 * gives the layer three ways to react to that movement. After finding out if the OMGraphic is highlightable, the SMMI
 * will tell the layer to highlight the OMGraphic (which usually means to call select() on it), provide a tool tip
 * string for the OMGraphic, and provide a string to use on the InformationDelegator info line. The layer can reply or
 * ignore any and all of these notifications, depending on how it's supposed to act.
 * <p>
 *
 * For left mouse clicks, the SMMI uses selection as a notification that the user is choosing an OMGraphic, and that the
 * OMGraphic should be prepared to be moved, modified or deleted. For a single OMGraphic, this is usually handled by
 * handing the OMGraphic off to the OMDrawingTool. However the GestureResponsPolicy handles the situation where the
 * selection is of multiple OMGraphics, and the layer should prepare to handle those situations as movement or deletion
 * notifications. This usually means to change the OMGraphic's display to indicate that the OMGraphics have been
 * selected. Selection notifications can come in series, and the GestureResponsePolicy is expected to keep track of
 * which OMGraphics it has been told are selected. Deselection notifications may come as well, or other action
 * notifications such as cut or copy may arrive. For cut and copy notifications, the OMGraphics should be removed from
 * any selection list. For pastings, the OMGraphics should be added to the selection list.
 * <p>
 *
 * For right mouse clicks, the layer will be provided with a JPopupMenu to use to populate with options for actions over
 * a OMGraphic or over the map.
 * <p>
 *
 * The StandardMapMouseInterpreter uses a timer to pace how mouse movement actions are responded to. Highlight reactions
 * only occur after the mouse has paused over the map for the timer interval, so the application doesn't try to respond
 * to constantly changing mouse locations. You can disable this delay by setting the timer interval to zero.
 */
public class StandardMapMouseInterpreter
	 ...
	 *
	 * @param l OMGraphicHandlerLayer this interpreter is serving.
	 */
	...
	/**
	 * Helper class used to keep track of OMGraphics of interest. Interest means that a MouseEvent that occurred over an
	 * OMGraphic that combined with another MouseEvent, may be interpreted as a significant event.
	 */
	public class GeometryOfInterest {
		 ...
		 *
		 * @param geom geometry of interest.
		 * @param me the mouse event making the geom interesting.
		 */
		 ...
		 *
		 * @param geom the OMGraphic to check if it applies against this GOI.
		 * @return true if geom isn't null and equals this GOI
		 */
		...
		/**
		 * A check to see if a mouse event that is occurring over an OMGraphic is in fact occurring over the one of
		 * interest, and with the same mouse button.
		 *
		 * @param geom the OMGraphic to check if it applies against this GOI.
		 * @param me MouseEvent for more info.
		 * @return true if geom isn't null and equals this GOI, matching mouse buttons
		 */
		 ...
		 *
		 * @param me the MouseEvent to check button on
		 * @return true if buttons match the original settings.
		 */
		 ...
		 *
		 * @param me get the button selection for this MouseEvent
		 * @return SWING int for button.
		 */
	...
	/**
	 * A flag to tell the interpreter to be selfish about consuming MouseEvents it receives. If set to true, it will
	 * consume events so that other MapMouseListeners will not receive the events. If false, lower layers will also
	 * receive events, which will let them react too. Intended to let other layers provide information about what the
	 * mouse is over when editing is occurring.
	 *
	 * @param consume true to consume events
	 */
	...
	/**
	 * Set the ID's of the mouse modes that this interpreter should be listening to. If set to null, this SMMI won't
	 * receive MouseEvents.
	 *
	 * @param list String array of MouseMode ID that this interpreter should receive events from.
	 */
	 ...
	 *
	 * @param me checking event for button press
	...
	/**
	 * Return a list of the modes that are interesting to the MapMouseListener. You MUST override this with the modes
	 * you're interested in, or set the mouse mode service list, or you won't receive mouse events.
	 */
	...
	/**
	 * Set the GeometryOfInterest for something that the mouse is over. Prevents excessive modifications of the GUI if
	...
	/**
	 * Get the GeometryOfInterest for something that the mouse is over. Prevents excessive modifications of the GUI if
	 * this remains constant.
	 */
	 ...
	 *
	 * @param e MouseEvent
	 * @return false if nothing was pressed over, or the consumeEvents setting if something was.
	 */
	...
	/**
	 * Tests the MouseEvent to see if it's a popup trigger, and calls rightClick appropriately if there is an OMGraphic
	 * involved.
	 *
	 * @param e MouseEvent
	...
	/**
	 * Invoked when the mouse has been clicked. Notifies the left click methods for the applicable OMGraphic or the map.
	 * Right click methods are handled when the testForAndHandlePopupTrigger method is called in mousePressed and
	 * mouseReleased.
	 *
	 * @param e MouseEvent
	 ...
	 *
	 * @param e MouseEvent
	 * @return the result from mouseMoved (also called from this method) combined with the consumeEvents setting.
	 */
	 ...
	 *
	 * @param e MouseEvent
	 * @return the result of updateMouseMoved() if the timer isn't being used, or false.
	 */
	...
	/**
	 * Set whether to ignore the timer when movement is occurring over an OMGraphic. Sometimes unhighlight can be
	 * inappropriately delayed when timer is enabled.
	 */
	...
	/**
	 * Set the time interval that the mouse timer waits before calling upateMouseMoved. A negative number or zero will
	 * disable the timer.
	 */
	...
	/**
	 * The real mouseMoved call, called when mouseMoved is called and, if there is a mouse timer interval set, that
	 * interval time has passed.
	 *
	 * @param e mouse event (actually MapMouseEvent) being evaluated.
	 * @return the consumeEvents setting of the mouse event concerns an OMGraphic, false if it didn't.
	 */
	...
	/**
	 * Notification that the user clicked on something else other than the provided OMGraphic that was previously
	 * left-clicked on. Calls deselect(omg).
	 *
	 * @return false
	 */
