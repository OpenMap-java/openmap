...
/**
 * A convenience class that keeps track of a relationship between real-time
 * changes and scenario-time changes.
 */
public class TimerRateHolder implements PropertyConsumer, Serializable {
	
	private static final long serialVersionUID = 1L;
	/**
	 * The string used for the default pace baseline ("00:00:00"). Needs to match
	 * the pace format.
	 */
	...
	/**
	 * Property used to describe a list of properties for TimerRateHolders
	 * (timerRates)
	 */
	public final static String TimerRatesProperty = "timerRates";

	/**
	 * PaceFormatProperty represents the property used for describing the pace
	 * format (paceFormat).
	 */
	public final static String PaceFormatProperty = "paceFormat";
	/**
	 * PaceBaselineProperty represents the property used for baselining the pace
	 * value (paceBaseline).
	 */
	public final static String PaceBaselineProperty = "paceBaseline";
	/**
	 * Clock that describes how often the pace value is applied to the time value
	 * (clockIntervalMillis).
	 */
	...
	/**
	 * The pace is the amount of time that should pass, in simulation time, when the
	 * timer gets updated. If the clock is updating every second, and you have the
	 * pace set for 1 hour, then the situation controlled by the application's clock
	 * will be updated 1 hour every second.
	 */
	protected long pace;
	/**
	 * The format used in the properties to note the pace. Kept in order to write
	 * out the format if that's needed.
	 */
	...
	/**
	 * The prefix used to set the properties, saved in case the properties are
	 * retrieved.
	 */
	...
	/**
	 * Create a TimerRateHolder with a date format, and a baseline time. The default
	 * baseline time is "00:00:00", so if you need to change that, use this
	 * constructor. The pace for this TimerRateHolder should be a relative amount of
	 * time, and that relativity, taking into account the locale offset to GMT, is
	 * given by the baseline time. The baseline time should match the format given.
	 */
	...
	public String toString() {
		return "TimePanel.TimerRateHolder [" + label + ", clock:" + clockInterval + ", pace:" + pace + "] (" + valid
				+ ")";
	}
	...
	public void setProperties(String prefix, Properties props) {
		...
		try {
			...
			clockInterval = PropUtils.longFromProperties(props, prefix + ClockIntervalProperty, -1L);
			String paceString = props.getProperty(prefix + PaceProperty);
			pace = paceFormat.parse(paceString).getTime() - paceFormat.parse(paceZero).getTime();
			valid = true;
	...
	public static List<TimerRateHolder> getTimerRateHolders(String prefix, Properties properties) {
		...
		if (timerRatesString != null) {
			...
			for (String ratePrefix : PropUtils.parseSpacedMarkers(timerRatesString)) {
