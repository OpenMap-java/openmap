...
public class EventLayer extends OMGraphicHandlerLayer
	...
	public synchronized OMGraphicList prepare() {
		...
		if (p != null) {
			...
			if (DEBUG) {
				logger.fine("EventLayer (" + getName() + ") snapshot at " + currentTime);
			}

			scenarioGraphics.generate(p, currentTime);

			if (DEBUG) {
				logger.fine("EventLayer (" + getName() + ") setting list of " + scenarioGraphics.size()
						+ " scenario graphics");
			}
		}
	...
	public void setProperties(String prefix, Properties properties) {
		...
		if (importerClass != null) {
			importer = (EventImporter) ComponentFactory.create(importerClass, prefix, properties);
		}
	}
	...
	public Properties getPropertyInfo(Properties list) {
		...
		PropUtils.setI18NPropertyInfo(i18n, list, getClass(), ImporterProperty, "Data Importer",
				"Class name of data importer", null);
	...
	/**
	 * @return the importer that creates temporal events from data.
	 */
	public EventImporter getImporter() {
		return importer;
	}

	/**
	 * The importer creates temporal events from data.
	 * 
	 * @param importer the importer to set
	 */
	public void setImporter(EventImporter importer) {
