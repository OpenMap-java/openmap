...
public class MapTileMaker extends ImageServer implements EmptyTileHandler {
	 ...
	 *            the array of layers.
	 * @param formatter
	 *            the ImageFormatter to use for the output image format.
	 */
	...
	public void setProperties(String prefix, Properties props) {
		...
		List<ZoomLevelMaker> zoomLevels = (List<ZoomLevelMaker>) PropUtils
				.objectsFromProperties(props, prefix + ZOOM_LEVELS_PROPERTY,
	...
	public Properties getPropertyInfo(Properties props) {
		...
		PropUtils.setI18NPropertyInfo(Environment.getI18n(), props,
				com.bbn.openmap.dataAccess.mapTile.MapTileMaker.class,
				ROOT_DIRECTORY_PROPERTY, "Tile Directory",
				"Root directory for holding tile files.",
	 ...
	 * 
	 * @param uvx
	 *            uv x pixel coordinate
	 * @param uvy
	 *            uv y pixel coordinate
	 * @param zoomInfo
	 *            zoom level for image tile
	 * @param proj
	 *            projection for tile
	 * @return byte[] for raw image bytes
	 */
	public byte[] makeTile(double uvx, double uvy, ZoomLevelMaker zoomInfo,
			Proj proj) {
		Point2D center = tileUVToLatLon(new Point2D.Double(uvx + .5, uvy + .5),
				zoomInfo.getZoomLevel());
	 ...
	 *            zoom level for tile
	 * @param layers
	 *            layers to include in image
	 * @param proj
	 *            projection for tile
	 * @param background
	 *            the paint to use for the background of the image.
	 * @return byte[] for raw image bytes
	 */
	public byte[] makeTile(double uvx, double uvy, int zoomLevel,
			List<Layer> layers, Proj proj, Paint background) {
		Point2D center = tileUVToLatLon(new Point2D.Double(uvx + .5, uvy + .5),
				zoomLevel);
	 ...
	 * @throws IOException
	 */
	public String makeTileFile(double uvx, double uvy, ZoomLevelMaker zoomInfo,
			Proj proj) throws IOException {

		byte[] imageBytes = zoomInfo.makeTile(uvx, uvy, this, proj);

		String filePath = zoomInfo.formatImageFilePath(getRootDir(), (int) uvx,
				(int) uvy);
	 ...
	 * 
	 * @param imagePath
	 *            the path of the missing tile that is going to be used as cache
	 *            lookup later.
	 * @param x
	 *            the uv x coordinate of the tile.
	 * @param y
	 *            the uv y coordinate of the tile.
	 * @param zoomLevel
	 *            the zoom level of the tile.
	 * @param mtcTransform
	 *            the transform that converts x,y coordinates to lat/lon and
	 *            describes the layout of the uv tile coordinates.
	 * @param proj
	 ...
	 */
	public BufferedImage getImageForEmptyTile(String imagePath, int x, int y,
			int zoomLevel, MapTileCoordinateTransform mtcTransform,
			Projection proj) {
		...
		if (layers == null) {
			if (logger.isLoggable(Level.FINE)) {
				logger.fine("There are no layers defined for zoom level"
						+ zoomLevel + " in MapTileMaker, can't create image");
			}
			return null;
		}

		LatLonPoint center = tileUVToLatLon(new Point2D.Double(x + .5, y + .5),
				zoomLevel);
		Mercator m = new Mercator(center,
				mtcTransform.getScaleForZoom(zoomLevel), TILE_SIZE, TILE_SIZE);

		BufferedImage bufferedImage = new BufferedImage(TILE_SIZE, TILE_SIZE,
	...
	public void makeTiles() {
		...
		Proj proj = new Mercator(new LatLonPoint.Double(), 10000,
				MapTileCoordinateTransform.TILE_SIZE,
				MapTileCoordinateTransform.TILE_SIZE);
		...
		for (ZoomLevelMaker zfi : zoomLevels) {
				...
				for (int xoff = 0; xoff < xofflimit; xoff++) {
					...
					for (int yoff = 0; yoff < yofflimit; yoff++) {
						...
						if (parentDirPath == null) {
							parentDirPath = zfi.formatParentDirectoryName(
									getRootDir(), x, y);
						...
						try {
							...
							logger.warning("Caught IOException writing " + x
									+ ", " + y + ", " + zfi);
						}
					}
			...
			if (range < zoomLevel) {
				...
				for (int rangeZoomLevel = zoomLevel - 1; rangeZoomLevel >= range; rangeZoomLevel--) {
					...
					rangeZFI.setScale(mtcTransform
							.getScaleForZoom(rangeZoomLevel));
					...
					for (Rectangle2D rawBounds : zfi.getBounds()) {
						Rectangle2D bounds = rangeZFI.getUVBounds(rawBounds,
								mtcTransform, rangeZoomLevel);
						...
						for (int xoff = 0; xoff < xofflimit; xoff++) {
							...
							for (int yoff = 0; yoff < yofflimit; yoff++) {
								...
								try {
									...
									byte[] imageBytes = makeTile(x, y,
											rangeZoomLevel, subLayers, proj,
											OMColor.clear);

									String filePath = rangeZFI
											.formatImageFilePath(getRootDir(),
													(int) x, (int) y);
									String outputFile = writeImageFile(
											imageBytes, filePath, true);

									if (logger.isLoggable(Level.INFO)) {
										logger.finer("wrote: " + outputFile);
									}
								} catch (IOException ioe) {
									logger.warning("Caught IOException writing "
											+ x + ", " + y + ", " + zfi);
								}
							}
	...
	public void createDefaultZoomLevels(int maxZoomLevel) {
		...
		for (int i = 0; i <= maxZoomLevel; i++) {
			ZoomLevelMaker zfi = new ZoomLevelMaker("ZoomLayerInfo " + i,
					"Tiles for zoom level " + i, i);
	...
	/**
	 * @param latlon
	 *            a Point2D whose x component is the longitude and y component
	 *            is the latitude
	 * @param zoom
	 ...
	 *            number of tiles (each 256x256) from the origin (where the
	 *            origin is 90lat,-180lon)
	 * @param zoom
	 *            Tile Map Service (TMS) style zoom level (0-19 usually)
	...
	public static void main(String[] args) {
		...
		if (arg != null) {
			...
			if (props == null) {
				...
				props.put("shape.shapeFile",
						"data/shape/world/cntry02/cntry02.shp");
				props.put("shape.fillColor", "FFBBBBBB");
				shapeLayer.setProperties("shape", props);

				tim = new MapTileMaker(new Layer[] { shapeLayer },
			...
			if (!configurationProps.isEmpty()) {
				...
				for (Object key : orderedProperties.keySet()) {
					String value = configurationProps.getProperty((String) key);
			...
			try {
				...
				logger.warning("caught IOException writing property file: "
						+ ioe.getMessage());
			}
