 ...
 * When the layer is re-projected, it figures out the brightest point on the
 * earth (closest to the sun), and creates an image that has daytime pixels
 * clear and the nighttime pixels shaded. There are a couple of options
 * available for the layer. The terminator can be faded from light to dark, and
 * the width of the fading can be adjusted. The color of the shading can be
 * changed. The shading can reflect the current time, or be set to display the
 * shading of a specified time. A time interval can be set to have the layer
 ...
 * </pre>
 * <p>
 * In addition, you can get this layer to work with the OpenMap viewer by
 * editing your openmap.properties file:
 * 
 * <pre>
 * 
 *  # layers
...
public class DayNightLayer extends OMGraphicHandlerLayer implements ProjectionListener, ActionListener {
	/**
	 * Default value of fade to the terminator line, set to .10f. This means
	 * that the last 10% of the horizon will be faded out.
	 */
	...
	/**
	 * Percentage of the distance from the horizon to the brightest point to
	 * start fading to darkness. Expected to be between 0.0 and 0.5.
	 */
	...
	public void setProperties(String prefix, java.util.Properties properties) {
		...
		daytimeColor = PropUtils.parseColorFromProperties(properties, prefix + DaytimeColorProperty, daytimeColor);
		nighttimeColor = PropUtils.parseColorFromProperties(properties, prefix + NighttimeColorProperty,
				nighttimeColor);

		doPolyTerminator = PropUtils.booleanFromProperties(properties, prefix + DoPolyTerminatorProperty,
				doPolyTerminator);
	...
	public Properties getPropertyInfo(Properties props) {
		...
		interString = i18n.get(DayNightLayer.class, OverlayTimeProperty, I18n.TOOLTIP,
		...
		interString = i18n.get(DayNightLayer.class, CurrentTimeProperty, I18n.TOOLTIP,
				"If true, the layer will set the darkness according to the current time.");
		...
		interString = i18n.get(DayNightLayer.class, UpdateIntervalProperty, I18n.TOOLTIP,
				"Update interval to automatically update the layer, in milli-seconds.");
		...
		interString = i18n.get(DayNightLayer.class, TermFadeProperty, I18n.TOOLTIP,
		...
		interString = i18n.get(DayNightLayer.class, DaytimeColorProperty, I18n.TOOLTIP,
		...
		interString = i18n.get(DayNightLayer.class, NighttimeColorProperty, I18n.TOOLTIP,
		...
		interString = i18n.get(DayNightLayer.class, DoPolyTerminatorProperty, I18n.TOOLTIP,
		...
		interString = i18n.get(DayNightLayer.class, TerminatorVertsProperty, I18n.TOOLTIP,
		...
		props.put(initPropertiesProperty,
				CurrentTimeProperty + " " + OverlayTimeProperty + " " + UpdateIntervalProperty + " "
						+ NighttimeColorProperty + " " + DoPolyTerminatorProperty + " " + TerminatorVertsProperty + " "
						+ DaytimeColorProperty + " " + TermFadeProperty + " " + RemovableProperty + " "
	...
	/**
	 * Create the OMGraphic that acts as an overlay showing the day/night
	 * terminator. The brightest spot on the earth is calculated, and then each
	 * pixel is inverse projected to find out its coordinates. Then the great
	 * circle distance is calculated. The terminator is assumed to be the great
	 * circle where all the points are PI/2 away from the bright point. If the
	 * termFade variable is set, then the difference in color over the
	 * terminator is feathered, on equal amount of the terminator.
	 * 
	 * @param projection the projection of the screen,
	 * @return OMGraphic containing image to use for the layer. The image has
	 *         been projected.
	 */
	protected OMGraphic createImage(Projection projection) {
		...
		if (Debug.debugging("daynight")) {
			Debug.output("DayNightLayer: Calculating sun position at time " + Long.toString(overlayTime));
		}
		...
		if (doPolyTerminator) {
			...
			OMCircle circle = new OMCircle((float) darkPoint.getY(), (float) darkPoint.getX(),
		...
		Debug.message("daynight", getName() + "|createImage: Center of bright spot lat= " + brightPoint.getLatitude()
	...
	/**
	 * Returns whether the layer will set the overlayTime to the time the image
	 * is created.
	 */
	...
	/**
	 * Set whether the layer should set the overlayTime to the time the image is
	 * created. If the time is being set to reflect a time other than the
	 * current time, this needs to be set to false. It actually is, if you
	 * manually set the overlay time.
	 */
