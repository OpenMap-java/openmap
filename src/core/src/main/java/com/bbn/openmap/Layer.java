 ...
 */
public abstract class Layer extends JComponent implements ProjectionListener, ProjectionPainter, BeanContextChild,
		BeanContextMembershipListener, PropertyConsumer, ActionListener {
	...
	/**
	 * Tooltip to use by components listing layer in GUI components, like
	 * LayersMenu and LayersPanel.
	 */
	public final static String ToolTipProperty = OMGraphicConstants.TOOLTIP.toLowerCase();
	...
	/**
	 * Map containing attribute information for the layer. For serialized
	...
	public final void addMouseListener(MouseListener l) {
		...
		if (java.beans.Beans.isDesignTime() || pkg.equals(SWING_PACKAGE) || pkg.startsWith(SWING_PACKAGE)
			...
			throw new IllegalArgumentException("This operation is disallowed because the package \""
					+ getPackage(l.getClass()) + "\" is not in the swing package (\"" + SWING_PACKAGE + "\").");
		}
	}
	...
	protected void setLayerProperties(String prefix, Properties props) {
		...
		setAddToBeanContext(
				PropUtils.booleanFromProperties(props, realPrefix + AddToBeanContextProperty, addToBeanContext));

		setAddAsBackground(
				PropUtils.booleanFromProperties(props, realPrefix + AddAsBackgroundProperty, addAsBackground));
		...
		if (transClassName != null) {
			try {
				coordTransform = (GeoCoordTransformation) ComponentFactory.create(transClassName,
						realPrefix + TransformProperty, props);
			} catch (ClassCastException cce) {

			}
		}

		String tooltipString = props.getProperty(realPrefix + ToolTipProperty);
		if (tooltipString != null && !tooltipString.trim().isEmpty()) {
			putAttribute(ToolTipProperty, tooltipString);
		}
	...
	public Properties getProperties(Properties props) {
		...
		PropUtils.putIfNotDefault(props, prefix + PrettyNameProperty, getName());
		PropUtils.putIfNotDefault(props, prefix + AutoPaletteProperty, new Boolean(autoPalette).toString(), Boolean.FALSE);
		PropUtils.putIfNotDefault(props, prefix + AddAsBackgroundProperty, new Boolean(addAsBackground).toString(), Boolean.FALSE);
		PropUtils.putIfNotDefault(props, prefix + RemovableProperty, new Boolean(removable).toString(), Boolean.TRUE);
		PropUtils.putIfNotDefault(props, prefix + AddToBeanContextProperty, new Boolean(addToBeanContext).toString(), Boolean.TRUE);
		PropUtils.putIfNotDefault(props, prefix + ToolTipProperty, getAttribute(ToolTipProperty));
		PropUtils.putIfNotDefault(props, prefix + MinScaleProperty, Float.toString(getMinScale()), Float.MIN_VALUE);
		PropUtils.putIfNotDefault(props, prefix + MaxScaleProperty, Float.toString(getMaxScale()), Float.MAX_VALUE);
		PropUtils.putIfNotDefault(props, prefix + DataPathPrefixProperty, getAttribute(DataPathPrefixProperty));
	...
	public void repaint(long tm, int x, int y, int width, int height) {
		...
		if (p instanceof MapBean) {
			...
			if (Debug.debugging("basic")) {
				Debug.output(getName() + "|Layer: repaint(tm=" + tm + ", x=" + x + ", y=" + y + ", width=" + width
						+ ", height=" + height + ")");
			}
				...
				if (frame == null) {
					...
					if (mapBean == null) {
						...
						try {
							...
							while (parent.getParent() != null && !(parent instanceof java.awt.Window)) {
