...
public class ScaleFilterLayer extends Layer
	...
	/**
	 * Property for setting aggressive layer cleanup, when layer isn't being
	 * displayed on map.
	 */
	public final static transient String AGGRESSIVE_LAYER_CLEAUP_PROPERTY = "cleanupLayers";
	/**
	 * The layers.
	 */
	...
	/**
	 * Flag to notify layers when they aren't active, so they can dump resources
	 * when they aren't on the map. Set to false to improve reaction times when
	 * zooming, since any layer would be initialized once. Setting this to true
	 * will force layers to reinitialize when they are put back on the map.
	 * 
	 * This will only affect layers that override removed(Container) properly.
	 */
	public boolean aggressiveLayerCleanup = false;
	...
	public synchronized void renderDataForProjection(Projection proj, java.awt.Graphics g) {
		// This is a special case for a layer - It doesn't get told when
		// sub-layers are done working.
		// This may be the only time it finds out they are done (if part of SFL
		// in background).
	...
	protected Layer configureAppropriateLayer(float scale) {
		...
		if (changed) {
			...
			remove(currentLayer);
			// Allow current layer to clean up
			if (aggressiveLayerCleanup) {
				currentLayer.removed(this);
			}
	...
	public void removed(Container cont) {
		getAppropriateLayer().removed(this);
	}
	...
	/**
	 * @return the aggressiveLayerCleanup
	 */
	public boolean isAggressiveLayerCleanup() {
		return aggressiveLayerCleanup;
	}

	/**
	 * @param aggressiveLayerCleanup the aggressiveLayerCleanup to set
	 */
	public void setAggressiveLayerCleanup(boolean aggressiveLayerCleanup) {
		this.aggressiveLayerCleanup = aggressiveLayerCleanup;
	}
