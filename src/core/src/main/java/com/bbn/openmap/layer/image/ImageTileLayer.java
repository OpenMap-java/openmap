...
/**
 * The ImageTileLayer is a layer that manages geo-referenced images over a map.
...
public class ImageTileLayer extends OMGraphicHandlerLayer {
	...
	*/
	...
	protected List<ImageReaderLoader> imageReaderLoaders;
	...
	public void setProperties(String prefix, Properties props) {
		...
		imageCache.resetCache(
				PropUtils.intFromProperties(props, prefix + ImageCacheSizeProperty, imageCache.getCacheSize()));

		imageCache.setCutoffScaleRatio(PropUtils.floatFromProperties(props, prefix + ImageCutoffRatioProperty,
		...
		if (imageReaderLoaderString != null) {
			...
			List<String> idls = PropUtils.parseSpacedMarkers(imageReaderLoaderString);
	 ...
	 * imageReaderLoader Vector with specific ImageReaderLoaders. By default, loads
	 * GeoTIFFImageReader.Loader.
	 */
	...
	public Properties getPropertyInfo(Properties props) {
		...
		PropUtils.setI18NPropertyInfo(i18n, props, ImageTileLayer.class, ImageFilePathProperty, "Images",
				"A list of images or directories to display (separated by ;).",
				"com.bbn.openmap.util.propertyEditor.MultiDirFilePropertyEditor");

		PropUtils.setI18NPropertyInfo(i18n, props, ImageTileLayer.class, ImageCacheSizeProperty, "Cache Size",
				"Number of images to keep in cache.", null);

		PropUtils.setI18NPropertyInfo(i18n, props, ImageTileLayer.class, ImageCutoffRatioProperty, "Cutoff Scale",
				"Projection scale where larger values won't cause images to be loaded and displayed.", null);

		String dummyMarker = PropUtils.getDummyMarkerForPropertyInfo(getPropertyPrefix(), null);

		PropUtils.setI18NPropertyInfo(i18n, props, ImageTileLayer.class, dummyMarker, "Highlight Settings",
	 ...
	 * doPrepare() is called. If the internal OMGraphicList is null the image file
	 * paths will be used to read image files.
	 */
	 ...
	 * 
	 * @return OMGraphicList retrieved from getList(), or a new list of that list is
	 *         null.
	 */
	 ...
	 * place the image. If filePath is a directory, this method is called for each
	 * file contained within. ImageTile objects are created from the image files.
	 * 
	 * @param filePath
	...
	protected JList<ImageTile> resultsList;
	protected DefaultListModel<ImageTile> listModel;
	...
	/**
	 * A modified LayerControlButtonPanel that is used to control which image files
	 * are available and their display order relative to each other on the image
	 * stack.
	 * 
	 * @author dietrick
	 */
	class ImageControlButtonPanel extends LayerControlButtonPanel {
		...
		public ImageControlButtonPanel() {
			...
			top.setToolTipText(
					i18n.get(ImageTileLayer.class, "moveImageToTop", I18n.TOOLTIP, "Move selected image(s) to top"));
			...
			bottom.setToolTipText(i18n.get(ImageTileLayer.class, "moveImageToBottom", I18n.TOOLTIP,
			...
			if (selectedTiles != null && selectedTiles.length > 0) {
				...
				if (rec != null) {
					...
					if (logger.isLoggable(Level.FINE)) {
						logger.fine("Images cover " + anchor1 + " to " + anchor2 + ", scale adjusted to " + scale);
					}
	...
	/**
	 * Note the provided tiles as being highlighted. Selection, in this case, means
	 * the OMGraphic selection.
	 * 
	 * @param selectedTiles2
	 */
	...
	/**
	 * Note the provided tiles as being highlighted or not. Selection, in this case,
	...
	/**
	 * Take the drawing attributes held by the layer and push the settings on all of
	 * the ImageTiles.
	 */
	...
	protected void removeImages(ImageTile[] selectedTiles2) {
		...
		if (selectedTiles != null && selectedTiles.length > 0) {
			String confirmStringMulti = i18n.get(ImageTileLayer.class, "removeConfirmMultiple",
					"Are you sure you want to remove these images from the layer?");
			String confirmStringSolo = i18n.get(ImageTileLayer.class, "removeConfirmSolo",
			...
			int answer = JOptionPane.showConfirmDialog(this,
					(selectedTiles.length == 1 ? confirmStringSolo : confirmStringMulti), confirmTitleString,
			...
			if (answer == JOptionPane.YES_OPTION) {
				...
				if (list != null) {
					for (int i = 0; i < selectedTiles.length; i++) {
						...
						((DefaultListModel<ImageTile>) getListModel()).removeElement(selectedTile);
					}
					if (resultsList != null) {
						resultsList.repaint();
					}
	 ...
	 * ImageReaderLoaders are consulted to only allow files that can be handled to
	 * be selectable.
	 */
	...
	/**
	 * A special Thread subclass to handle image loading, so it's not managed by the
	 * AWT thread.
	 * 
	 * @author dietrick
	 */
	...
	/**
	 * Set the GUI button state to be enabled or not based on something on the list
	 * being selected.
	 * 
	 ...
	 */
	protected synchronized ListModel<ImageTile> getListModel() {
		if (listModel == null) {
			listModel = new DefaultListModel<ImageTile>();
		}
	...
	protected void addImageTileToList(ImageTile tile) {
		((DefaultListModel<ImageTile>) getListModel()).addElement(tile);
	}
	...
	protected void clearImageTileList() {
		((DefaultListModel<ImageTile>) getListModel()).clear();
	}
	...
	protected boolean removeImageTileFromList(ImageTile tile) {
		return ((DefaultListModel<ImageTile>) getListModel()).removeElement(tile);
	}
	...
	protected void rebuildListModel() {
		DefaultListModel<ImageTile> dlm = (DefaultListModel<ImageTile>) getListModel();
	 ...
	 * If all of the tiles are invisible, the GUI button will allow them to be made
	 * visible. If any of them are visible, all of them can be made invisible before
	 * the button will change them to the visible.
	 * 
	 */
		...
		public void valueChanged(ListSelectionEvent e) {
			if (e.getValueIsAdjusting() == false && resultsList != null) {
				...
				if (indicies.length > 0) {
					ListModel<ImageTile> listModel = getListModel();
	 ...
	 */
	public static class ImageListCellRenderer extends JPanel implements ListCellRenderer<ImageTile> {
		...
		public Component getListCellRendererComponent(JList<? extends ImageTile> list, ImageTile value, int index,
	...
	class ImageLoaderFileFilter extends FileFilter {
		...
		List<ImageReaderLoader> imageReaderLoaders;

		public ImageLoaderFileFilter(List<ImageReaderLoader> imgDcdrLdrs) {
		...
		public String getDescription() {
			String description = i18n.get(ImageTileLayer.class, "fileFilterDescription",
