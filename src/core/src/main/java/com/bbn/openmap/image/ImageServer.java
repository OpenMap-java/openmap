 ...
 */
public class ImageServer implements/* ImageReadyListener, ImageReceiver, */PropertyConsumer {
	 ...
	 * @return a byte[] representing the formatted image.
	 */
	public byte[] createImageFromLayers(Projection proj, int scaledWidth, int scaledHeight, List<Layer> layers,
			Paint background) {
	 ...
	 *            which layers are desired. Bit 1 of the mask refers to
	 *            layer[0], etc. A bit turned on means the layer will be
	 *            included.
	 * @return a byte[] representing the formatted image.
	 */
	 ...
	 *            formatters) to use in the properties file. If defined, then
	 *            this method will look for 'prefix.layers' and
	 ...
	 *            layer definitions. See the standard openmap.properties file
	 *            for more details on how to define layers and plugins.
	 * 
	 ...
	 *            should not have an appendix defined. This method will check
	 *            which formatter is being used, and will assign one based on
	 *            the image format (leave off the ., too).
	 * 
	...
	public static String createImageFile(String prefix, Properties props, Projection proj, String outputPath)
		...
		background = PropUtils.parseColorFromProperties(props, Environment.BackgroundColor, background);
		...
		if (proj == null) {
			...
			if (GeoProj.class.isAssignableFrom(projClass)) {
				center = new LatLonPoint.Float(PropUtils.floatFromProperties(props, Environment.Latitude, 0f),
						PropUtils.floatFromProperties(props, Environment.Longitude, 0f));
			} else {
				center = new Point2D.Float(PropUtils.floatFromProperties(props, Environment.Latitude, 0f),
						PropUtils.floatFromProperties(props, Environment.Longitude, 0f));
			}

			proj = projFactory.makeProjection(projClass, center,
