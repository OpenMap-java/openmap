...
public final class ProjMath {
	...
	public static float getScaleFromProjected(Point2D point1, Point2D point2, Projection projection) {
		 ...
		 */
		Point2D upperLeft = new Point2D.Double(Math.min(point1.getX(), point2.getX()),
				Math.min(point1.getY(), point2.getY()));
		Point2D lowerRight = new Point2D.Double(Math.max(point1.getX(), point2.getX()),
	 ...
	 * @param ll2 the lower right coordinates of the bounding box.
	 * @param point1 a java.awt.Point reflecting a pixel spot on the projection
	 *            that matches the ll1 coordinate in the new space, the upper
	 *            left corner of the area of interest. Where the ll1 is going to
	 *            go in the new projection.
	 * @param point2 a java.awt.Point reflecting a pixel spot on the projection
	 *            that matches the ll2 coordinate in the new space, usually the
	 *            lower right corner of the area of interest. Where the ll2 is
	 *            going to go in the new projection.
	...
	public static boolean isCrossingDateline(double leftLon, double rightLon, float projScale) {
		...
		return ((leftLon > rightLon)
				|| (MoreMath.approximately_equal(leftLon, rightLon, .001f) && projScale > 1000000f));
	}
	...
	public static boolean isVisible(Projection sourceMapProjection, Point2D llp) {
		...
		if (sourceMapProjection != null) {
			if (sourceMapProjection.isPlotable(llp)) {
				...
				double x = p.getX();
				double y = p.getY();
				if (x >= 0 && x <= sourceMapProjection.getWidth() && y >= 0 && y <= sourceMapProjection.getWidth()) {
					ret = true;
				}
			}
		}
