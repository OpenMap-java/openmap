...
public class ZoomLevelMakerFace extends OMComponentPanel {
	...
	protected static Logger logger = Logger
			.getLogger("com.bbn.openmap.dataAccess.mapTile.ZoomLevelMakerFace");
	...
	protected JList<BoundsObject> boundsList;
	...
	public ZoomLevelMakerFace(ZoomLevelMaker zfi, MapTileMakerComponent mtmc) {
		...
		JLabel intro = new JLabel(i18n.get(ZoomLevelMakerFace.class,
				"zoom_level", "Make tiles for zoom levels"));
		JLabel tileDimLabel = new JLabel(i18n.get(ZoomLevelMakerFace.class,
				"tile_dim", "Tile Dimensions"));
		JLabel baseScaleLabel = new JLabel(i18n.get(ZoomLevelMakerFace.class,
				"base_scale", "Base Scale for Zoom Level"));

		JSpinner zoomLevelChoice = new JSpinner(new SpinnerNumberModel(0, 0,
				20, 1.0)) {
		...
		String upperZoomLevelTT = i18n.get(ZoomLevelMakerFace.class,
				"upper_zoom_level", I18n.TOOLTIP, "Upper zoom level");
		String lowerZoomLevelTT = i18n.get(ZoomLevelMakerFace.class,
				"lower_zoom_level", I18n.TOOLTIP, "Lower zoom level");
		...
		addIntroEntry(introPanel, tileDimLabel, tileDimensions, 1, new Insets(
				0, 0, 5, 0));
		addIntroEntry(introPanel, baseScaleLabel, scaleButton, 2, new Insets(0,
				0, 5, 0));
		...
		String layers_for_title = i18n.get(ZoomLevelMakerFace.class,
				"layers_for_title", "Layers");
		outerLayerPanel.setBorder(BorderFactory
				.createTitledBorder(layers_for_title));
		JScrollPane jsp = new JScrollPane(layerPanel,
		...
		String boundaries_for_title = i18n.get(ZoomLevelMakerFace.class,
				"boundaries_for_title", "Boundaries");
		boundsPanel.setBorder(BorderFactory
				.createTitledBorder(boundaries_for_title));
		boundsObjectList = new ArrayList<BoundsObject>();
		boundsModel = new BoundsListModel();
		boundsList = new JList<BoundsObject>(boundsModel);
		...
		JScrollPane scrollableBoundsList = new JScrollPane(boundsList,
				JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
				JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		...
		String create_a_boundary_rectangle = i18n.get(ZoomLevelMakerFace.class,
				"create_a_boundary_rectangle", "Create a boundary rectangle");
		...
		String edit_a_selected_boundary_rectangle = i18n.get(
				ZoomLevelMakerFace.class, "edit_a_selected_boundary_rectangle",
				"Edit a selected boundary rectangle");
		...
		editBoundaryButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				if (boundsList.getSelectedIndex() != -1) {
					BoundsObject selected = (BoundsObject) boundsList
							.getSelectedValue();
		...
		String delete_a_selected_boundary_rectangle = i18n.get(
				ZoomLevelMakerFace.class,
				"delete_a_selected_boundary_rectangle",
				"Delete a selected boundary rectangle");
		deleteBoundaryButton
				.setToolTipText(delete_a_selected_boundary_rectangle);
			...
			public void actionPerformed(ActionEvent ae) {
				if (boundsList.getSelectedIndex() != -1) {
					...
					if (organizer.drawingTool.isActivated()) {
						organizer.drawingTool
								.deactivate(OMAction.DELETE_GRAPHIC_MASK);
					}

					((MapHandler) organizer.getBeanContext())
							.get(MapBean.class).repaint();
				}
			}
		});
	...
	protected void addIntroEntry(JPanel panel, JComponent left,
			JComponent right, int y, Insets i) {
		GridBagConstraints c = new GridBagConstraints();
	...
	public void updateZoomLevel(int zoomLevel) {
		...
		zfi.setScale(new OSMMapTileCoordinateTransform()
		...
		String tooltip = i18n.get(ZoomLevelMakerFace.class, "set_map_scale_to",
				I18n.TOOLTIP, "Set map scale to");
	...
	public void handleBoundary(OMGraphic omg, OMAction action) {
		...
		for (OMGraphic omr : boundaries) {
			if (omr instanceof OMRect) {
				String bounding_rectangle = i18n.get(ZoomLevelMakerFace.class,
						"bounding_rectangle", "Bounding Rectangle");
				boundsModel.addElement(new BoundsObject((OMRect) omr,
						bounding_rectangle + " " + (count++)));
			}
		}
	 ...
	 * 
	 * @param proj
	 *            Projection to use to render boundaries.
	 ...
	 * @author dietrick
	 */
	private final class BoundsListModel extends AbstractListModel<BoundsObject> {
		private static final long serialVersionUID = 1L;
	...
	public class BoundsObject implements Cloneable {
		...
		public BoundsObject clone() {
			OMRect copy = new OMRect(bounds.getNorthLat(), bounds.getWestLon(),
					bounds.getSouthLat(), bounds.getEastLon(),
			...
			addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent ae) {
					...
					if (active /* duh */&& organizer != null) {
						...
						setInclude(zfi.getLayers() != null
								&& !zfi.getLayers().isEmpty());
					}
				}
			});
		}
	...
	public ImageIcon createImageIcon(String path) {
		URL imgURL = ZoomLevelMakerFace.class.getClassLoader().getResource(
				"com/bbn/openmap/dataAccess/mapTile/" + path);
		if (imgURL != null) {
