...
public class ComponentFactory {
	...
	/**
	 * A property to use for the class name of new objects - "class". Can be used
	 * with the PropUtils.objectsFromProperties method as the defining property.
	 */
	...
	/**
	 * Given a Vector of marker name Strings, and a Properties object, look in the
	 * Properties object for the markerName.class property to get a class name to
	 * create each object. Then, if the new objects are PropertyConsumers, use the
	 * marker name as a property prefix to get properties for that object out of the
	 * Properties.
	 * 
	 ...
	 */
	public static List<?> create(List<String> markerNames, Properties properties) {
	 ...
	 * @return Vector containing the new Objects.
	 */
	public static List<?> create(List<String> markerNames, String prefix, Properties properties) {
	 ...
	 *                        names.
	 * @param properties      Properties object containing the details.
	 * @param progressSupport ProgressSupport object to provide progress updates to.
	 *                        It's OK if this is null to not have progress events
	 *                        sent.
	 * @return Vector containing the new Objects.
	 */
	public static List<?> create(List<String> markerNames, String prefix, Properties properties,
	 ...
	 * @return Vector containing the new Objects.
	 */
	public static List<?> create(List<String> markerNames, Properties properties, ProgressSupport progressSupport) {
	 ...
	 * @param markerNames          String of space separated marker names.
	 * @param properties           Properties object containing the details.
	 * @param progressSupport      ProgressSupport object to provide progress
	 *                             updates to. It's OK if this is null to not have
	 *                             progress events sent.
	 * @param matchInOutVectorSize if true, then if there is any trouble creating an
	 *                             object, it's marker name will be placed in the
	 *                             returned vector instead of a component. If false,
	 *                             only valid objects will be returned in the
	 *                             vector.
	 * @return Vector containing the new Objects. If a component could not be
	 *         created, the markerName is returned in its place, so you can figure
	 *         out which one couldn't be created. In any case, the size of the
	 *         returned vector is the same size as the markerNames vector, so you
	 *         can figure out which markerNames go with which objects.
	 */
	public static List<?> create(List<String> markerNames, Properties properties, ProgressSupport progressSupport,
	 ...
	 *         can figure out which markerNames go with which objects.
	 */
	protected List<?> _create(List<String> markerNames, String prefix, Properties properties,
			ProgressSupport progressSupport, boolean matchInOutVectorSize) {

		int size = markerNames.size();
		List<Object> vector = new ArrayList<>(size);

		if (progressSupport != null) {
			progressSupport.fireUpdate(ProgressEvent.UPDATE, "Creating Components", 100, 0);
		}

		String propertyPrefix = PropUtils.getScopedPropertyPrefix(prefix);
		int i = 0;
		for (String markerName : markerNames) {
			String componentName = propertyPrefix + markerName;
	 ...
	 * @param properties Properties to use to initialize the object, if the object
	 ...
	 * 
	 * @param className       Class name to instantiate.
	 * @param constructorArgs an Object array of arguments to use in the constructor
	 *                        of the component.
	 ...
	 * @return object if all goes well, null if anything bad happens.
	 */
	protected Object _create(String className, Object[] constructorArgs, Class<?>[] argClasses, String prefix,
			Properties properties) {
		...
		try {
			...
			try {
				...
				/*
				 * The argClasses may have subclasses of what the desired constructor needs, so
			...
			if (obj instanceof PropertyConsumer && properties != null) {
				...
				if (Debug.debugging(BasicI18n.DEBUG_CREATE)) {
					/*
					 * If we're interested in creating resource bundle files, we should cause these
					 * PropertyConsumers to ask for their property info, since this is where most of
					 * the elective GUI strings are queried and found.
					 */
	 ...
	 */
	protected Object createWithSubclassConstructorArgs(Class<?> newObjClass, Class<?>[] argClasses,
			Object[] constructorArgs) throws NoSuchMethodException, InstantiationException, IllegalAccessException,
		...
		if (DEBUG) {
			logger.finer(
					" - searching " + numConstructors + " possible constructor" + (numConstructors == 1 ? "" : "s"));
		}
		...
		for (int i = 0; i < numConstructors; i++) {
			...
			for (int j = 0; j < numArgs; j++) {
				if (arguments[j] == argClasses[j]) {
					...
					if (DEBUG) {
						logger.finer(" - superclass arg class match, arg " + j + " reassigning to "
					...
					if (DEBUG) {
						logger.finer(" - arg class mismatch on arg " + j + ", bailing (" + arguments[j].getName()
