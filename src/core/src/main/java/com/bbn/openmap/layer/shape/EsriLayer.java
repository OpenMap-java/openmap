 ...
 * @author Don Dietrick
 * @author Lonnie Goad from OptiMetrics provided selection bug solution and GUI interaction.
 */
public class EsriLayer
	...
	/**
	 * A simple list mechanism that will let selected OMGraphics to be drawn on top of all the others. Using this list
	 * instead of changing the order of the esri graphic list maintains the order of that list. We clear out this and
	...
	/**
	 * Creates an EsriPlugIn that will be configured through the <code>setProperties()</code> method
	 */
	public EsriLayer() {
	 ...
	 */
	public EsriLayer(String name, int type, int columnCount)
		throws Exception {
	...
	/**
	 * Standard OMGraphicHandlerLayer prepare called when the projection changes.
	 *
	 * @return OMGraphicList that should be painted on the map.
	 */
	...
	public EsriGraphicList getGeometry(URL shp) {
		GeoCoordTransformation coordTransform = getCoordTransform();

		return EsriGraphicList.getEsriGraphicList(shp, getDrawingAttributes(), getModel(), coordTransform);
	}
	 ...
	 *
	 * @return An int representing the type of layer, as specified in Esri's shape file format specification
	 */
	public int getType() {
		return _type;
	}
	...
	public Properties getPropertyInfo(Properties props) {
		...
		props.put(initPropertiesProperty, PARAM_SHP + " " + PARAM_DBF + " " + PARAM_SHX
			+ drawingAttributes.getInitPropertiesOrder() + " " + Layer.AddToBeanContextProperty);
		...
		props.put(PARAM_SHP + ScopedEditorProperty, "com.bbn.openmap.util.propertyEditor.FDUPropertyEditor");
		props.put(PARAM_DBF + ScopedEditorProperty, "com.bbn.openmap.util.propertyEditor.FDUPropertyEditor");
		props.put(PARAM_SHX + ScopedEditorProperty, "com.bbn.openmap.util.propertyEditor.FDUPropertyEditor");
	...
	public void showTable() {
		if (tableFrame == null) {
			String tableTitle = PropUtils.unnull(getName());
			tableFrame = new JFrame(tableTitle + " Shape Data Attributes");

			JScrollPane pane = new JScrollPane(getTable(), JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
	...
	// Instead of mouseClick(), replace with these three methods.
	 ...
	 */
	// To handle tooltips on mouseMoved, use getToolTipTextFor(OMGraphic)
	public String getToolTipTextFor(OMGraphic omg) {
		EsriGraphicList list = getEsriGraphicList();
		Integer I = ((Integer) omg.getAttribute(SHAPE_INDEX_ATTRIBUTE));
		if (I != null) {
			return getDescription(I.intValue());
		} else {
			return getDescription(list.indexOf(omg));
		}
	}
	...
	/*
	 ...
	 fireRequestToolTip(getDescription(index));
