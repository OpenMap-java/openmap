...
/**
 * A GraphicLoaderPlugIn is a PlugIn that receives its OMGraphics from some
 * other source, at any time. It just listens to its GraphicLoader for updates
 * to the OMGraphicList, and then updates the map as necessary.
 * <P>
 * The GraphicLoaderPlugIn passes projection changes onto the GraphicLoader, and
 * if the GraphicLoader is a MapMouseListener, the GraphicLoaderPlugIn will
 * defer all MouseEvents to it.
 * <p>
 * 
 * To add a GraphicLoader to the OpenMap application, you can do it several
 * ways:
 * <UL>
 * <LI>You can create a specific GraphicLoaderPlugIn that creates its own
 * GraphicLoader, and initializes it accordingly. You would add the
 * GraphicLoaderPlugIn to the openmap.layers property in the openmap.properties
 * file.
 * <LI>You can create a GraphicLoaderPlugIn by adding an entry to the
 * openmap.layer property in the openmap.properties file, and define what kind
 * of GraphicLoader to create in the properties for the GraphicLoaderPlugIn.
 * <P>
 * 
 * <pre>
 * 
 ...
 * </pre>
 * 
 * <LI>You can add a com.bbn.openmap.plugin.graphicLoader.GraphicLoaderConnector
 * to the openmap.components property, and then add the GraphicLoader to the
 * openmap.components property as well. The GraphicLoaderConnector will find the
 * GraphicLoader, and create a GraphicLoaderPlugIn/PlugInLayer for the
 * GraphicLoader and add it to the LayerHandler on top of the map.
 * </UL>
 * 
 */
public class GraphicLoaderLayer extends OMGraphicHandlerLayer {
     ...
     * 
     * @return OMGraphicList that should be displayed for the current
     *         projection.
     */
    ...
    /**
     * OMGraphicHandler method. This will cause doPrepare() to be called on the
     * PlugInLayer, which will result in a getRectangle() being called the
     * GraphicLoaderPlugIn, which will in turn cause setProjection() to be
     * called on the GraphicLoader. Watch out, in the GraphicLoader, for setting
     * the list as a result of a projection change - you can get into a loop.
     * The AbstractGraphicLoader checks to see if the projection has changed.
     */
    ...
    /**
     * Set whether to add the GraphicLoader, which is assumed to yet be added to
     * the GraphicLoaderPlugIn, to the MapHandler.
     */
    ...
    /**
     * Some GraphicLoaders want mouse events, this allows them to set the
     * MapMouseListener provided by the layer, redirecting MouseEvents to the
     * GraphicLoader instead of the MapMouseInterpreter.
     * 
    ...
    /**
     * If the MapMouseListener has been set by a GraphicLoader, it is returned
     * here to the MouseDelegator. Otherwise, this layer acts like any other
     * OMGraphicHandlerLayer and returns the MapMouseInterpreter.
     */
