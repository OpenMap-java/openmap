...
/**
 * This is a utility class that can be used by beans that need support for handling MapMouseListeners and firing
 * MapMouseEvents. You can use an instance of this class as a member field of your bean and delegate work to it.
 * <p>
 * You can set the behavior of how MouseEvents are propagated by setting whether to "consume" events. If the MouseMode
 * is consuming events, then the event is not propagated further than the first listener to successfully process it.
 * Otherwise the event is propagated to all listeners. The default is to consume events.
 */
public class MapMouseSupport
	...
	protected final static Logger logger = Logger.getLogger("com.bbn.openmap.event.MapMouseSupport");

	/**
	 * The flag that dictates whether the events should be passed to all the listeners or just limited to the first
	 * listener that can deal with it. The default value is set to true, which means the event will be consumed by the
	 * first layer that can handle it.
	 */
	protected boolean consumeEvents = true;

	/**
	 * The priority MapMouseListener will be guaranteed to receive events that go hand in hand (pressed - released,
	 * etc.).
	 */
	 ...
	 * Construct a default MapMouseSupport. The default value of consumeEvents is set to true.
	 *
	 * @param mode the parent MapMouseMode to use with creating the MapMouseEvent.
	 */
	 ...
	 *
	 * @param shouldConsumeEvents if true, events are propagated to the first MapMouseListener that successfully
	 * processes the event, if false, events are propagated to all MapMouseListeners
	 */
	 ...
	 *
	 * @param mode MapMouseMode parent mode this class is supporting.
	 */
	...
	/**
	 * Sets how the mouse support passes out events. If the value passed in is true, the mouse support will only pass
	 * the event to the first listener that can respond to the event. If false, the mouse support will pass the event on
	...
	public boolean fireMapMousePressed(MouseEvent evt) {
		...
		if (proxy == null || evt.isShiftDown() || (proxyDistributionMask & PROXY_DISTRIB_MOUSE_PRESSED) > 0) {
			...
			if (DEBUG && proxy != null && evt.isShiftDown()) {
				logger.fine(
					"MMS.fireMapMousePressed(): proxy enabled, but side stepping to send event to primary listeners");
			}
		...
		boolean ignoreConsumed = !consumed || (consumed && ((proxyDistributionMask & PROXY_ACK_CONSUMED_MOUSE_PRESSED)
			== 0));
	...
	/**
	 * Handle a mouseReleased MouseListener event. Checks to see if there is a priorityListener, and will direct the
	 * event to that listener. The priorityListener variable will be reset to null. If there is not a priorityListener,
	 * the event is passed through the listeners, subject to the consumeEvents mode.
	 *
	 * @param evt MouseEvent to be handled.
	 * @return true if event consumed.
	 */
	public boolean fireMapMouseReleased(MouseEvent evt) {
		...
		boolean ignoreConsumed = !consumed || (consumed && ((proxyDistributionMask & PROXY_ACK_CONSUMED_MOUSE_RELEASED)
			== 0));
	...
	/**
	 * Handle a mouseClicked MouseListener event. If the priorityListener is set, it automatically gets the clicked
	 * event. If it is not set, the other listeners get a shot at the event according to the consumeEvent mode.
	 *
	 * @param evt MouseEvent to be handled.
	 * @return true if event consumed.
	 */
	public boolean fireMapMouseClicked(MouseEvent evt) {
		...
		boolean ignoreConsumed = !consumed || (consumed && ((proxyDistributionMask & PROXY_ACK_CONSUMED_MOUSE_CLICKED)
			== 0));
	...
	public boolean fireMapMouseDragged(MouseEvent evt) {
		...
		boolean ignoreConsumed = !consumed || (consumed && ((proxyDistributionMask & PROXY_ACK_CONSUMED_MOUSE_DRAGGED)
			== 0));
	...
	/**
	 * Handle a mouseMoved MouseListener event. If the moved event is consumed, the rest of the listeners that didn't
	 * have a chance to respond get called in the mouse moved method without arguments.
	 *
	 * @param evt MouseEvent to be handled
	...
	public boolean fireMapMouseMoved(MouseEvent evt) {
		...
		boolean ignoreConsumed = !consumed || (consumed && ((proxyDistributionMask & PROXY_ACK_CONSUMED_MOUSE_MOVED)
			== 0));
	...
	/**
	 * Request to have the parent MapMouseMode act as a proxy for a MapMouseMode that wants to remain hidden. Can be
	 * useful for directing events to one object.
	 *
	 * @param mmm the hidden MapMouseMode for this MapMouseMode to send events to.
	 * @param pdm the proxy distribution mask to use, which lets this support object notify its targets of events if the
	 * parent is acting as a proxy.
	 * @return true if the proxy setup (essentially a lock) is successful, false if the proxy is already set up for
	 * another listener.
	 */
	 ...
	 *
	 * @param mmm MapMouseMode for proxy check.
	 * @return true if mmm is proxy for the parent mouse mode.
	 */
	...
	/**
	 * @return MapMouseMode being proxied (the hidden MapMouseMode that the parent mode is providing events to).
	 */
	...
	/**
	 * Set the mask that dictates which events get sent to this support object's targets even if the parent mouse mode
