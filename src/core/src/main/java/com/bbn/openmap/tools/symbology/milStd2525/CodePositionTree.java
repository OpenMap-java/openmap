...
/**
 * The CodePositionTree is a CodeScheme CodePosition object that has some
 * intelligence on how to starting building the SymbolPart tree for the
 * SymbolReferenceLibrary. It knows how to set up CodeOptions for the different
 * types of CodeSchemes, and starts the parsing of the hierarchy properties.
 */
public class CodePositionTree extends CodeScheme {
	...
	public CodePositionTree(Properties positionProperties) {
		...
		// CodePosition can be used to build the hierarchical symbol
		...
		List<CodePosition> basicOptions = new LinkedList<>();
		basicOptions.add(ca);
		...
		List<CodePosition> warfightingOptions = new LinkedList<>();
		...
		List<CodePosition> tacOptions = new LinkedList<>();
		tacOptions.add(ca);
		...
		List<CodePosition> intelOptions = new LinkedList<>();
		intelOptions.add(ca);
		...
		List<CodePosition> mootwOptions = new LinkedList<>();
		mootwOptions.add(ca);
	...
	public SymbolPart parseHierarchy(String name, Properties hierarchyProperties) {
		List<CodePosition> positions = getPositionChoices();

		SymbolPartTree head = new SymbolPartTree(name);
		List<SymbolPart> subs = new LinkedList<>();
		head.setSubs(subs);

		for (CodePosition cp : positions) {
			if (cp instanceof CodeScheme) {
				CodeScheme cs = (CodeScheme) cp;
