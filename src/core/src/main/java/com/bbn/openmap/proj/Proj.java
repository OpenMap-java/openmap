...
public abstract class Proj implements Projection, Cloneable, Serializable {
	...
	public String toString() {
		return (" center(" + centerX + ":" + centerY + ") scale=" + scale + " maxscale=" + maxscale + " minscale="
				+ minscale + " width=" + width + " height=" + height + "]");
	}
		...
		while (!pi.isDone()) {
			...
			if (type == PathIterator.SEG_MOVETO) {
				...
				if (type == PathIterator.SEG_QUADTO) {
					...
					path.curveTo(world.getX(), world.getY(), world2.getX(), world2.getY(), world3.getX(),
							world3.getY());
				}
			}
	 ...
	 *            corner of the area of interest. Note that this is the location
	 *            where you want ll1 to go in the new projection scale, not
	 *            where it is now.
	 ...
	 *            location where you want ll2 to go in the new projection, not
	 *            where it is now.
	 */
