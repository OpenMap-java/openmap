...
import com.bbn.openmap.layer.graphicLoader.GraphicLoaderLayer;
...
/**
 * The PilotLoaderLayer receives OMGraphics from the PilotLoader and displays
...
public class PilotLoaderLayer extends GraphicLoaderLayer {
	...
	public PilotLoaderLayer() {
		super();
		setGraphicLoader(new PilotLoader(this));
	}
	 ...
	 * 
	 * @return OMGraphic that should be displayed for the current projection.
	 */
	public OMGraphicList prepare() {
		...
		OMGraphicList list = new OMGraphicList(getList());
		list.generate(p);
	 ...
	 */
	public void setGraphicLoader(PilotLoader pl) {
		loader = pl;
		setMapMouseListener(pl);

		try {
			pl.setBeanContext(getBeanContext());
	...
	public void setBeanContext(BeanContext in_bc) throws PropertyVetoException {
		...
		if (loader instanceof PilotLoader) {
			((PilotLoader) loader).setBeanContext(in_bc);
		}
	}

} // end PilotLoaderLayer.
