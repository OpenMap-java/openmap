...
public class DeclutterMatrix {
	...
	public class MatrixIndexes {
		...
		public boolean setFromPixels(int pixelXLocation, int pixelYLocation, int pixelLength, int pixelHeight) {
			...
			int objLength = (int) Math.ceil((double) pixelLength / (double) x_pix_interval);
			int objHeight = (int) Math.ceil((double) pixelHeight / (double) y_pix_interval);
		...
		public boolean set(int objXIndex, int objYIndex, int objIndexLength, int objIndexHeight) {
			...
			withinMatrix = objOnMatrix(objXIndex, objYIndex, objIndexLength, objIndexHeight);
			if (!withinMatrix) {
				return false;
			}
	...
	protected boolean isClear(MatrixIndexes indexes, boolean markAsTaken) {
		...
		for (int taken = 0; taken < 2; taken++) {
			...
			for (int i = indexes.xStart; i <= indexes.xEnd; i++) {
				...
				// the spaces are open.
				if (taken == 0) {
					notClear = isMatrixLocationTaken(i, indexes.yStart, indexes.yEnd - indexes.yStart + 1);
					if (notClear) {
						return false;
					}
				} else {
	 ...
	 *            that where an object will be discarded if it's not at least
	 *            that close. -1 means find anywhere on the map where the object
	 *            will fit.
	 * @return Point of closest open space.
	 */
	public Point2D setNextOpen(Point2D point, int pixelLength, int pixelHeight, int pixelAwayLimit) {
					...
					if (dcPos[pos].direction == DCD_NS) {
						...
						if (dcPos[pos].direction == DCD_EW) {
							for (i = round - 1; (i >= -1 * (round - 1)) && !set; i--) {
								ret = isAreaClearBW(xpoint + i, ypoint, testPoint);
	...
	protected Point isAreaClearBW(int xPoint, int yPoint, Point point) {
		...
		while (count < indexes.origIndexLength && currentXIndex > leftMostIndex) {
			...
			if (currentXIndex >= 0 && currentXIndex <= maxx) {
				if (!isMatrixLocationTaken(currentXIndex, indexes.yStart, indexes.yEnd - indexes.yStart + 1)) {
					count++; // clear column
				} else {
