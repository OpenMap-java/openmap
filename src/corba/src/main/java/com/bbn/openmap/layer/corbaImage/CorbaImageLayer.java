...
package com.bbn.openmap.layer.corbaImage;
...
import com.bbn.openmap.layer.corbaImage.corbaImageServer.Server;
import com.bbn.openmap.layer.corbaImage.corbaImageServer.ServerHelper;
import com.bbn.openmap.layer.image.WebImageLayer;
...
/**
 *
 * This class asks for an image from a CorbaImageServer. There will be some
 * CORBA setup involved with using this PlugIn. You'll need to run some kind of
 * name service, and register the CorbaImageServer with it, or have the server
 * write out its IOR file to a location reachable from this client.
 * <P>
 *
 * The query to the CorbaImageServer looks something like this:
 * <P>
 * REQUEST=MAP&PROJTYPE=projection_type_value&SCALE=scale_value&LAT=
 * center_latitude
 * &LON=center_longitude&HEIGHT=map_pixel_height&WIDTH=map_pixel_width
 * &FORMAT=image_format&TRANSPARENT=true|false&BGCOLOR=background_color
 *
 * <P>
 * The projection information will be entered automatically by the plugin based
 ...
 * pluginlayer.plugin.name=Corba Naming Service Name (needed if ior is not
 * provided) pluginlayer.plugin.ior=URL to ior file (needed if name is not
 * provided) pluginlayer.plugin.format=image format (JPEG, GIF from
 * WMTConstants.java) pluginlayer.plugin.transparent=true or false, depends on
 * imageformat pluginlayer.plugin.backgroundColor=RGB hex string (RRGGBB)
 */
public class CorbaImageLayer extends WebImageLayer implements ImageServerConstants {
    ...
    public String createQueryString(Projection p) {
        ...
        if (p != null) {
            ...
            buf.append(PROJTYPE + "=" + p.getName() + "&" + SCALE + "=" + p.getScale() + "&" + LAT
                    + "=" + center.getY() + "&" + LON + "=" + center.getX() + "&"
                    + ImageServerConstants.HEIGHT + "=" + p.getHeight() + "&"
                    + ImageServerConstants.WIDTH + "=" + p.getWidth());
        } else {
            buf.append(PROJTYPE + "=name_undefined&" + SCALE + "=scale_undefined&" + LAT
                    + "=center_lat_undefined&" + LON + "=center_lon_undefined&"
                    + ImageServerConstants.HEIGHT + "=height_undefined&"
                    + ImageServerConstants.WIDTH + "=width_undefined");
        }
    ...
    public OMGraphicList prepare() {
        ...
        try {
            ...
            if (Debug.debugging("cis")) {
                Debug.output("CorbaImagePlugIn: got image data length " + imageData.length);
            }
