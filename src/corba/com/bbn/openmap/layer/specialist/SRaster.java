...
/** 
 * SRaster is a specialist graphic object that creates and
 * manipulates image data.  It can be used for image icons, or for map
 * subframes that tile together to cover the whole display.
 */
public class SRaster extends SGraphic /* used to be _RasterImplBase*/ {
   ...
   * yet known.  The memory for the pixels will be allocated, and then
   * they can be set with image data later when a database is
   * accessed.  
   * @see #setPixel(short x, short y, byte ctIndex)
   * @see #setColor(int index, CTEntry color) 
   */
   ...
   * images in a cache, for tiled mapping data or something else where
   * the data is not yet known.  The memory for the pixels will be
   * allocated, and then they can be set with image data later when a
