...
public abstract class GraphicWarehouseSupport implements VPFGraphicWarehouse {
     ...
     *        data, if needed. Not needed here.
     */
        ...
        for (int j = 0; j < size; j++) {
            ...
            if (cfscnt > 0) { // normal
                for (int i = 0; i < cfscnt; i++) {
                    ipts.add(new LLPoint((float) cfsvals[i * cfssz + 1], (float) cfsvals[i
                            * cfssz]));
                }
            } else { // reverse
                cfscnt *= -1; // normalize
    ...
    public LLPoint[] generatePolyPts(List ipts, float north, float south,
        ...
        /*
         * Let me explain. We might be inserting some extra points to work
         * around a problem displaying Antarctica in cylindrical projections. So
         * the initial capacity of the Vector is set to the number of points.
         * The increment value is set to 5 since that's how many points we'll
         * add if we hit the antarctica thing. That way we're not allocating a
         * huge new Vector when we don't really need to.
         */
     ...
     * limitied to tiles or feature from certain libraries. Specified here
     * instead of the LibrarySelectionTable in case the LST is shared among
     * layers, since the warehouse is specific to a particular layer.
     */
