 ...
 * staticArgs property as (key, value) pairs.  For political
 * boundaries and coastlines, for instance: <p>
 *
 * staticArgs=(coverageType, bnd)(featureTypes, edge area) (edge, polbndl coastl) (area, polbnda) (lineColor, FFFF0000) (fillColor, FFAAAA66) (draw, edge area)
 *
 * <p>
 *
 ...
 * that can be set via the VPFLayer properties.  See those javadocs
 * for more details.
 */
public class VPFSpecialist extends Specialist {
    ...
    public java.util.Properties processTokens(String args, 
					      StringBuffer cov, 
					      StringBuffer altcov, 
					      boolean usealt) {

	java.util.StringTokenizer tok = 
	    new java.util.StringTokenizer(args,")");
	...
	while (true) {
		...
		if (token.startsWith("(")) {
			...
			// Coverage types require special handling, since
			...
			// !! Assumes coverageType has been processed first

			// If there is an alternate coverage and it's required, use it
			if (usealt == true && !altcov.equals("")) {
			    if (key.startsWith("alt")) {
				key=key.substring(3,key.length()); // 3 is "alt", remove it
				props.setProperty(prefix+"."+key,value);
			    } else {
		    ...
		    // This case is for old style static args, when they
	...
	try {
	    ...
	    /* If the we are zoomed out so that we _might_ have the entire
	       world on the screen, then let's check to see if east and
	       west are approximately equal.  If they are, change them to
	       -180 and +180 so that we get the entire world without
	       having trouble with the floating point rounding errors that
	       allow west to be slightly less than east (by .00001) and
	       confuse our clipping routines into thinking that there
	       should be nothing on the screen since nothing falls in that
	       .00001 slice of the world. */
	    ...
	    boolean showEdges =
		(getHashedValueAsBoolean(dynArgs, DynArgEdges)
		 || buttons[0].checked);
	    warehouse.setEdgeFeatures(showEdges);

	    boolean showText =
		(getHashedValueAsBoolean(dynArgs, DynArgText)
		 || buttons[1].checked);
	    warehouse.setTextFeatures(showText);

	    boolean showAreas =
		(getHashedValueAsBoolean(dynArgs, DynArgArea)
		 || buttons[2].checked);
	    ...
	    // Now need to know in advance if we are using an alternate coverage
	    // for arguments
	    boolean usealt;
	    ...
	    java.util.Properties props = processTokens(staticArgs,retcov,retaltcov,usealt);

	    if (Debug.debugging("vpfspecialist")) {
		Debug.output("VPFSpecialist: with prefix " + prefix + ", properties: " + props);
	    }

	    // After processing properties, make sure they're available
	    ...
	    lst.drawTile(p.scale, p.width, p.height,
			 ((altcov != null) && (p.scale >= altCovScale)) ? altcov : cov,
			 warehouse, newll1, newll2);
