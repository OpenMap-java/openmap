...
public class Schema extends VPFHttpServlet {
	...
	list.addElement("DocFile Name: " + 
			fileURL(request, response, basepath,
				foo.getDocumentationFilename()));
	int reclen = foo.getRecordLength();
	String reclenstr = (reclen == -1) ? 
	    ("variable") : (Integer.toString(reclen) + " bytes");
	...
	rows.addElement(new WrapElement("CAPTION",
					new StringElement("Column Schema")));
	rows.addElement(thr);
	...
	for (int i = 0; i < dci.length; i++) {
	    ...
	    tr.addElement(fileURL(request, response,
				  basepath, dci[i].getVDT()));
	    tr.addElement(thematicURL(request, response, basepath,
				      dci[i].getThematicIndexName()));
	    tr.addElement(docURL(request, response, basepath,
