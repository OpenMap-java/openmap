...
public class DescribeDBServlet extends VPFHttpServlet {
    ...
    public void printLibrary(HttpServletRequest request,
                             HttpServletResponse response,
                             String pathPrefix, CoverageAttributeTable cat)
        throws ServletException, IOException  {
        PrintWriter out = response.getWriter();
        ...
        for (int i = 0; i < coverages.length; i++) {
            clist.addElement("<A HREF=\"#" + libName + "_" +
                             coverages[i] + "\">" + coverages[i] + "</A>");
        }
        ...
        list.generate(out);
        for (int i = 0; i < coverages.length; i++) {
          printCoverage(request, response, libpath + "/" + coverages[i],
                        libName, cat, coverages[i]);
        }
    }
    ...
    public void printCoverage(HttpServletRequest request, 
                              ...
                              throws ServletException, IOException {
        PrintWriter out = response.getWriter();
        out.println("<H3><A NAME=\"" + libName + "_" + covname +
                    "\">Coverage " +
        ...
        String fcsURL = buildURL(request, response, pathPrefix,
                                 "fcs?" + Data.RowSelectParam + "=" + Data.RowSelectAll, "fcs");
        list.addElement("Feature Class Schema: " + fcsURL);
        CoverageTable ct = cat.getCoverageTable(covname);
        //CoverageTable opens alot of files, go through and close them
        ...
        if (ftypeinfo.size() == 0) {
            ...
            HtmlListElement flist = new HtmlListElement("Feature Types (from " + buildURL(request, response, pathPrefix, "fca") + ")");
            ...
            for (Iterator i = ftypeinfo.values().iterator(); i.hasNext(); ) {
                ...
                String url = buildURL(request, response, pathPrefix,
                                      name + suffix, name);
        ...
        try {
            ...
            DcwRecordFile fcs = new DcwRecordFile(ct.getDataPath() + File.separator + 
                                                  "fcs" + (ct.appendDot ? "." : ""));
