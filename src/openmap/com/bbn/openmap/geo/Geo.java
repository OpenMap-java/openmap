...
public class Geo {
   ...
   /**
    * Vector cross product. Gives you the point 90 degrees from the great circle
    * line between this point and b. The side of the line depends on the right
    * hand rule.
    * 
   ...
   public double strictAzimuth(Geo v2) {
       ...
       * great circle between this and v2. The azimuth is the angle between them
       * but we specialized the cross product.
       */
   ...
   /**
    * Is the point, p, within radius radians of the great circle segment between
    * this and v2?
    */
   public boolean isInside(Geo v2, double radius, Geo p) {
       ...
       * vector along gc of the radius length. If the former is larger than the
       * latter, than p is further than radius from arc, so must not be isInside
       */
   ...
   public static boolean isInside(Geo v1, Geo v2, double radius, Geo p1, Geo p2) {
      return v1.isInside(v2, radius, p1) || v1.isInside(v2, radius, p2) || p1.isInside(p2, radius, v1)
            || p1.isInside(p2, radius, v2);
   }
   ...
   public boolean inBubble(Geo v2, double forwardRadius, double backRadius, Geo p) {
      return distance(p) <= ((v2.subtract(this).normalize().dot(p.subtract(this)) > 0.0) ? forwardRadius : backRadius);
   }
   ...
   /**
    * Find the intersection of the great circle between this and q and the great
    * circle normal to r.
    * <p>
    * 
...
//      return this.scale(x, ret).add(q.scale(1.0 - x), ret).normalize(ret);
    ...
    * @param err maximum angle of rounded edges, in radians. If 0, will directly
    *        cut outside bends.
   ...
   /**
    * Return a Geo array with the duplicates removed. May arbitrarily mutate the
    * input array.
    */
   ...
   /**
    * return a float array of alternating lat lon pairs where the first and last
    * pair are the same, thus closing the path, by adding a point if needed.
