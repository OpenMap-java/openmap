...
/** 
 * A sample Layer implementation.  It demonstrates
 * how to write a Layer that can be added to the MapBean.
 * <p>
 ...
 * <code>RouteLayer</code> does the latter.  A Layer that simply
 * displays a legend on top of the map might not do anything in
 * the <code>projectionChanged</code> method since its display
...
public class RouteLayer extends Layer implements MapMouseListener
{
    ...
    /**
     * Handler for <code>ProjectionEvent</code>s.  This function
     * is invoked when the <code>MapBean</code> projection changes.
     * The projection is stored, the graphics are reprojected and
     * the Layer is repainted.
     * <p>
     * It is important to store a <bold>clone</bold> of the
     * <code>Projection</code> and not the given <code>Projection</code>
     * since the <code>Projection</code> given is the actual
     * <code>MapBean</code> <code>Projection</code>.  This will ensure
     * that no other objects will be able to manipulate the
