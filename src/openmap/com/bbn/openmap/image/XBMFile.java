...
public class XBMFile {
    ...
    public XBMFile(String filename) {
       ...
       else System.out.println("XBMFile: framename not valid.");
    }
    ...
    /**  This method leaves all to the caller, by creating a bitmap
      *  from scratch.  Any modification to the bits must take into
      *  account the paddedWidth of the bitmap.  The paddedWidth is
      *  only a factor of the width is is not an even multiple of
      *  eight.  So, if you want to change the bits settings, you'll
    ...
    /**  readin() parses the file and sets the object variables.
      *  It assumes that the file f is valid/exists.
      */ 
    private void readin() {
      try{
    ...
    /**
      ...
      *  Assumes that the length of the tmpString is sufficient
      *  as dictated by the height and width settings.
      */ 
