...
public class MagicPlanetImageComponent extends OMComponent implements
	...
	public void setProperties(String prefix, Properties props) {
		...
		setHeight(PropUtils.intFromProperties(props, prefix + HeightProperty,
				getHeight()));
		setWidth(PropUtils.intFromProperties(props, prefix + WidthProperty,
				getWidth()));

		setScale(PropUtils.floatFromProperties(props, prefix + ScaleProperty,
				scale));
	...
	public Properties getProperties(Properties props) {
		...
		props.put(prefix + OutputDirectoryProperty, PropUtils
				.unnull(getOutputDirectoryString()));
		props
				.put(prefix + AutoUpdateProperty, Boolean
						.toString(isAutoUpdate()));
		...
		props.put(prefix + UpdateIntervalProperty, Integer
				.toString(getUpdateInterval()));
		props.put(prefix + CleanupProperty, Boolean.toString(isCleanup()));
		props.put(prefix + CleanupIntervalProperty, Integer
				.toString(getCleanupInterval()));
		props.put(prefix + LastImageFileProperty, PropUtils
				.unnull(getLastImageFile()));
		props.put(prefix + PostProcessingScriptProperty, PropUtils
				.unnull(getPostProcessingScript()));
	...
	public Properties getPropertyInfo(Properties props) {
		...
		interString = i18n.get(MagicPlanetImageComponent.class,
				OutputDirectoryProperty, I18n.TOOLTIP,
		...
		interString = i18n.get(MagicPlanetImageComponent.class,
				OutputDirectoryProperty, "Directory Path");
		props.put(OutputDirectoryProperty + LabelEditorProperty, interString);
		// -------
		interString = i18n
				.get(MagicPlanetImageComponent.class, AutoUpdateProperty,
						I18n.TOOLTIP,
		...
		interString = i18n.get(MagicPlanetImageComponent.class,
				AutoUpdateProperty, "Auto-Update");
		props.put(AutoUpdateProperty + LabelEditorProperty, interString);
		// -------
		interString = i18n.get(MagicPlanetImageComponent.class, HeightProperty,
				I18n.TOOLTIP,
	...
	public void createImage() {
		...
		if (lastImageFile != null) {
			try {
				...
				fos.write(new String(LAST_IMAGE_FILE_KEY + "=" + fileName)
						.getBytes());
				fos.flush();
				fos.close();
				...
				if (DEBUG) {
					Debug
							.output("  MP: done writing file noting last image file name: "
									+ lastImageFile);
				}
			} catch (IOException ioe) {
				Debug
						.error("MP: error writing file to note last image file name:\n"
								+ ioe.getMessage());
				ioe.printStackTrace();
				lastImageFile = null;
			}
	...
	protected String generatePostProcessingCmd(String script, String filePath) {
		...
		if (script != null && filePath != null) {
			...
			ret = ret.replaceAll(REPLACE_FILENAME_MARKER, filePath
					.substring(filePath.lastIndexOf('/') + 1));
			ret = ret.replaceAll(REPLACE_FILENAME_WOEXT_MARKER, filePath
	 ...
	 *             if the filename can't be decoded.
	 */
	 ...
	 * 
	 * @param deleteAll
	 *            if true, all images will be deleted, regardless of when they
	 *            were created.
	 */
			...
			for (int i = 0; i < files.length; i++) {
				...
				if (!deleteAll) {
					try {
						...
						if (DEBUG) {
							Debug
									.output("MagicPlanetImageComponent considering deleting "
											+ f.getName()
											+ ", file time:"
											+ ft
											+ ", current time:"
	...
	public void setOutputDirectoryString(String outputDirectoryString) {
		...
		JOptionPane
				.showMessageDialog(
						...
						"Problem Creating Directory", JOptionPane.ERROR_MESSAGE);
	}
