...
/**
 * The MagicPlanetImageComponent is an OpenMap Component designed to
 * create images for Global Imagination's MagicPlanet Globe. This
 * component, when added to an OpenMap MapHandler, will find the
 * LayerHandler so it can find out when the Layer given to the MapBean
 * change, so it knows which ones to use when creating an image file.
 * This component also connects to the MapBean as a
 * PropertyChangeListener to find out when the ocean color has
 * changed. The MagicPlanet software (Storyteller) has the option of
 ...
 * scale of the images. The scale of the image dictates its pixel
 * size, since the proportion of the projection has to be constant for
...
public class MagicPlanetImageComponent extends OMComponent implements
    ...
    /**
     * MapHandlerChild method extended through the OMComponent
     * hierarchy. This is the method called by the MapHandler with
     * objects added to the MapHandler.
     */
     ...
     * MapHandlerChild method extended through the OMComponent
     * hierarchy. This is the method called by the MapHandler with
     * objects removed from the MapHandler.
     */
    ...
    /**
     * @return the object currently known as the LayerHandler by this
     *         object.
     */
    ...
    /**
     * Set the LayerHandler, become a LayerListener object to it to
     * know when the layers on the MapBean change. If there is already
     * a LayerHandler known to this component, this component will
     * remove itself as a listener to the previous LayerHandler.
     * 
     * @param object LayerHandler.
     */
    ...
    /**
     * @return the object currently known as the MapBean by this
     *         object.
     */
    ...
    /**
     * Set the MapBean, become a PropertyChangeListener object to it
     * to know when the background color on the MapBean changes. If
     * there is already a MapBean known to this component, this
     * component will remove itself as a listener to the previous
     * MapBean.
     * 
     * @param object MapBean.
     */
    ...
    /**
     * Checks to see if there is a timer, and if the component wants
     * to automatically update the current image. If the timer isn't
     * running, it's started.
     */
    ...
    /**
     * Remove old files. Checks the current time against the
     * timestamps decoded by the names of files found in the output
     * directory, and deletes them if the difference between those
     * times is greater than the cleanupInterval.
     * 
     * @param deleteAll if true, all images will be deleted,
    ...
    /**
     * Set the 'ocean' color of the planet. The
     * MagicPlanetImageComponent listens for events from the MapBean
     * and will call this method if the ocean color on the MapBean is
     * changed.
     * 
     * @param paint
     */
    ...
    /**
     * Sets the scale for the projection, which directly affects the
     * size of the image. Larger numbers make smaller images. Calling
     * this method causes the setProj() method to be called with the
     * new projection to use for images.
     * 
     * @param scale
     */
    ...
    /**
     * Set whether a new image should be created immediately if the
     * MapBean's layers change, or if the MapBean's background color
     * changes. If false, a new image will be created on the next
     * normal timer cycle.
     * 
     * @param autoUpdate
     */
    ...
    /**
     * Set the interval, in milliseconds, between the current time and
     * the time old images were created before they are deleted. This
     * setting only matters if isCleanup() returns true.
     * 
    ...
    /**
     * Set the scaled pixel height of the images.-1 maintains what the
     * scale setting decides.
     * 
     * @param height pixels.
     */
    ...
    /**
     * Set the scaled pixel width of the images. -1 maintains what the
     * scale setting decides.
     * 
     * @param width pixels.
     */
