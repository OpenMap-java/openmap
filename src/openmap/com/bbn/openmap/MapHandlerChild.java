 ...
 * object for it be a good member of the MapHandler.  The MapHandler
 * is actually a BeanContext object, which is simply a container for a
 ...
 * running without references to objects it depends on.  It should
 * wait patiently for the other objects to be added to the MapHandler,
 * and then do the work itself to hook up.  It should also listen for
 * those objects to be removed from the MapHandler, disengage
 * gracefully, and wait patiently until it finds something else to
 * hook up to.<P>
 *
 * An object does not have to be a MapHandlerChild to be added to the
 * MapHandler, but it does need to be one to be able to use it.  If
 * you use the findAndInit(Iterator) method to look for objects,
 * you'll find it is called on two different conditions.  It's called
 * when this MapHandlerChild is added to the MapHandler, and it then
 ...
 * super classes handles the objects they care about.<P>
 *
