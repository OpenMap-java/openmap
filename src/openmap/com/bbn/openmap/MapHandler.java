...
public class MapHandler extends BeanContextServicesSupport {
    ...
    protected synchronized void addLater(Object obj) {
        if (addLaterVector == null) {
            addLaterVector = new ArrayList<Object>();
        }
        if (DEBUG) {
    ...
    protected synchronized void purgeLaterList() {
        if (addLaterVector != null && !addInProgress) {

            Object[] objs = addLaterVector.toArray();
            // Nulling out before the iteration avoids a stack overflow loop
            addLaterVector = null;
    ...
    public synchronized boolean add(Object obj) {
        try {
            ...
            if (obj != null) {
                logger.info("MapHandler caught ConcurrentModificationException when adding ["
