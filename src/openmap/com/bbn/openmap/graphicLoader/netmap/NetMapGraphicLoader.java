 ...
 * NetMapGraphicLoader.
 */
public class NetMapGraphicLoader extends MMLGraphicLoader
    ...
    /**
     * The component that provides controls to the NetMapReader,
	...
	// for now, print to debug, later, create OMGraphics out of it.
	if (DEBUG) {
	...
	// Used for many (if not all commands, might as well do this here.
	int index = LayerUtils.intFromProperties(eventProps, INDEX_FIELD, ERROR_VALUE_INT);
	if (cmd.equals(NODE_OBJECT)) {

	    int shape = LayerUtils.intFromProperties(eventProps, SHAPE_FIELD, ERROR_VALUE_INT);
		...
		int posX = LayerUtils.intFromProperties(eventProps, POSX_FIELD, ERROR_VALUE_INT);
		int posY = LayerUtils.intFromProperties(eventProps, POSY_FIELD, ERROR_VALUE_INT);
		int width = LayerUtils.intFromProperties(eventProps, WIDTH_FIELD, ERROR_VALUE_INT);
		int height = LayerUtils.intFromProperties(eventProps, HEIGHT_FIELD, ERROR_VALUE_INT);
		status = LayerUtils.intFromProperties(eventProps, STATUS_FIELD, 0);
		int menu = LayerUtils.intFromProperties(eventProps, MENU_FIELD, 0);
		int joffset = LayerUtils.intFromProperties(eventProps, JOFFSET_FIELD, ERROR_VALUE_INT);
		...
		float elevation = LayerUtils.floatFromProperties(eventProps, ELEVATION_FIELD, 0f);
	    ...
	    node = nodeList.get(index);
 	    ...
 	    if (node != null) {
		status = LayerUtils.intFromProperties(eventProps, STATUS_FIELD, ERROR_VALUE_INT);
		...
		status = LayerUtils.intFromProperties(eventProps, STATUS_FIELD, 0);
		int node1 = LayerUtils.intFromProperties(eventProps, LINK_NODE1_FIELD, ERROR_VALUE_INT);
		int node2 = LayerUtils.intFromProperties(eventProps, LINK_NODE2_FIELD, ERROR_VALUE_INT);
...
// 	    manageGraphics();
	...
	if (receiver instanceof PlugIn && omList != null) {
	    OMGraphic graphic = omList.getOMGraphicThatContains(e.getX(), e.getY());
	    String label = null;
...
// 		label = ((Line)graphic).getLabel();
		...
		if (comp instanceof Layer) {
			...
			((Layer)comp).fireRequestToolTip(e, "Node " + label);
			toolTipUp = true;
		    } else if (toolTipUp) {
			((Layer)comp).fireHideToolTip(e);
			toolTipUp = false;
		    }
		    return true;
		}
