...
public class LayerHandler
    ...
    * prefix is needed, just use the methods that let you use the prefix - don't
    * worry about the period, it will be added automatically.
    */
   public static final String startUpLayersProperty = "startUpLayers";
   /**
    * Flag to set synchronous threading on the LayerHandler, telling it to react
    * to layer order changes and layer visibility requests within the calling
    * thread. By default, this action is true. Setting it to false may eliminate
    * pauses in GUI reactions by off-loading work done by layers being added to
    * the MapBean, but there have been reports that the asynchronous nature of
    * the threading queue may be causing an unexpected state in layer order
    * and/or availability under certain intense layer management conditions
    * (created by automated processes, for example).
    */
   ...
   /**
    * If you use this constructor, the LayerHandler expects that the layers will
    * be created and added later, either by addLayer() or init().
    */
   public LayerHandler() {
   }
    ...
    * you are using the LayerHandler in a different setting, then you might want
    * to just call init() directly, or extend this class and have setProperties
    * do what you want.
    */
   ...
   /**
    * Initialize from an array of layers. This will cause the LayerListeners, if
    * they exist, to update themselves with the current list of layers. This
   ...
   public void init(Layer[] layers) {
      ...
      if (allLayers != null && allLayers.length > 0) {
         ...
         Vector<Layer> newLayers = new Vector<Layer>(allLayers.length + lLength);
         if (layers != null) {
    ...
    * 
    * @param p properties containing layers property, the startupLayers property
    *        listing the layers to make visible immediately, and the layer
    *        properties as well.
   ...
   protected Layer[] getLayers(String prefix, Properties p) {
      ...
      if (layersValueString == null) {
         layersValueString = p.getProperty(PropUtils.getScopedPropertyPrefix(Environment.OpenMapPrefix) + layersProperty);
      }

      if (startupLayersValueString == null) {
         startupLayersValueString =
               p.getProperty(PropUtils.getScopedPropertyPrefix(Environment.OpenMapPrefix) + startUpLayersProperty);
      }
    ...
    *        that should initially be set to visible when created, so that those
    *        layers are initially added to the map.
    ...
    * this if you know the layers are already in the MapHandler or don't need to
    * be. If you want layers to be added to the MapHandler (if the LayerHandler
    * knows about it), call init(Layer[]) instead.
    * <p/>
    * <p/>
    * Also, this method will disregard layer non-removable status for any layers
    * currently held, and will simply replace all layers with the ones provided.
    * If you want the non-removable flag to be adhered to, call init(Layers[]).
    * 
   ...
   protected String getLayerNamesFromArray(Layer[] lArray) {
   ...
   public Layer[] getMapLayers() {
      ...
      if (logger.isLoggable(Level.FINE)) {
         logger.fine("providing map layers: " + getLayerNamesFromArray(cake));
      }
    ...
    * less the layer is moved on top. If the position is greater or equal to the
    * number of layers, the layer is moved to the bottom of the pile.
    * 
   ...
   /**
    * Add a layer to the bottom of the layer stack. If the layer is already part
    * of the layer stack, nothing is done.
    * 
   ...
   /**
    * Add a layer to a certain position in the layer array. If the position is 0
    * or less, the layer is put up front (on top). If the position is greater
    * than the length of the current array, the layer is put at the end, (on the
    * bottom). The layer is placed on the map if it's visibility is true. A
   ...
   public void addLayer(Layer layer, int position) {
      ...
      if (position >= allLayers.length) {
         ...
         System.arraycopy(allLayers, position, newLayers, position + 1, allLayers.length - position);
      }
   ...
   public void removeLayer(Layer layer) {
      if (layer != null && layer.isRemovable()) {
         ...
         if (layer != null) {
            ...
            throw new com.bbn.openmap.util.HandleError("LayerHandler commanded to delete a layer (" + layer.getName()
                  + ") that is not removeable");
         }
      }
   }
   ...
   /**
    * A BeanContextMembershipListener interface method, which is called when new
    * objects are removed from the BeanContext. If a LayerListener or Layer is
    * found on this list, it is removed from the list of LayerListeners.
    * 
   ...
   public void addLayersToBeanContext(Layer[] layers) {
      ...
      for (int i = 0; i < layers.length; i++) {
         if (layers[i].getAddToBeanContext() && layers[i].getBeanContext() == null) {
            bc.add(layers[i]);
         }
      }
   }
    ...
    * findAndInit() to hook up with any objects that may already be added to the
    * BeanContext. A BeanContextChild method.
    * 
   ...
   public Properties getPropertyInfo(Properties props) {
      ...
      String internString =
            i18n.get(LayerHandler.class, SynchronousThreadingProperty, I18n.TOOLTIP, "Launch new threads to do work.");
