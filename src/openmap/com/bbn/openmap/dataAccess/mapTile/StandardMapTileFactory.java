...
public class StandardMapTileFactory
    ...
    * projections. The base projection is going to be defined for the mtc
    * transform set on the factory. Warping images are slower to generate for a
    * map projection than scaling rasters.
    * 
    * @param image
    * @param x
    * @param y
      ...
      for (int x = uvleftM; x < uvrightM; x++) {
         for (int y = uvupM; y < uvbottomM; y++) {
            ...
            boolean rightOMGraphicType =
                  (tileGraphic instanceof OMScalingRaster && isMercator) || (tileGraphic instanceof OMWarpingImage && !isMercator);
   ...
   /**
    * An array of scales for all of the possible zoom levels, from 1 to 20. They
    * get calculate the first time getZoomLevelForProj is called.
    */
   ...
   public int getZoomLevelForProj(Projection proj) {
      ...
      for (int currentZoom = low; currentZoom <= high; currentZoom++) {
         ...
         /*
          * Used to try to do this with scale comparisons, now just look at tile
          * sizes. float diff = currentScale - scales[currentZoom]; if (diff >
          * 0) { return currentZoom + 1; }
          */
      }
   ...
   public void setProperties(String prefix, Properties setList) {
      ...
      zoomLevelTileSize = PropUtils.intFromProperties(setList, prefix + ZOOM_LEVEL_TILE_SIZE_PROPERTY, zoomLevelTileSize);
   }
   ...
   public void setRootDir(String rootDirectory) {
      ...
      if (rootDirectory != null) {
         if (rootDirectory.endsWith("jar")) {
            ...
            /*
             * OK, things look a little crazy here, because there is a bit of
             * recursion going on. The configure call above may cause a
             * setRootDir call with a relative path stored in a jar file. These
             * rules below make sure the relative root dir from the inner loop
             * sets the rootDir, while preserving the rootDirProperty from the
             * outer loop.
             */
    ...
    * default ZoomLevelInfo is based on the OpenStreetMap tile layout, zoom
    * levels 0-20 (where level 0 is all the way zoomed out), and the tiles are
    * stored zoomLevel/x/y.(fileExt). You can set a different zoom level info if
    * you want to work with a tile set that is stored/defined differently than
    * OSM.
    * <p>
