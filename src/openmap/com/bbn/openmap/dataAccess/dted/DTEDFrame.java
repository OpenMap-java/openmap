...
public class DTEDFrame implements Closable {
    ...
    protected void read(BinaryFile binFile, boolean readWholeFile) {
        ...
        if (readWholeFile) readDataRecords();
        frame_is_valid = true;
    }
    ...
    public int interpElevationAt(float lat, float lon) {
       if (frame_is_valid == true) {
          if (lat >= dsi.sw_lat && lat <= dsi.ne_lat
            ...
            if (elevations[lclon_index]==null) readDataRecord(lclon_index);
...
//          System.out.println();System.out.println();
            ...
            float answer = resolveFourPoints(ul, ur, lr, ll, 
                                             lat_index, lon_index);
    ...
    public short[][] getElevations(int startx, int starty, int endx, int endy) {
        ...
        for (int x = left; x <= right; x++) {
            if (elevations[x]==null) readDataRecord(x);
    ...
    public OMGrid getOMGrid() {
        ...
        double vResolution = (double)dsi.lat_post_interval/36000.0; // Tenths of seconds between data rows
        // hResolution decimal degrees per column
        double hResolution = (double)dsi.lon_post_interval/36000.0; // Tenths of seconds between data columns.
        
        if (Debug.debugging("grid")) {
            Debug.output("DTEDFrame creating OMGrid with vResolution: " + vResolution +
                         ",  hResolution: " + hResolution + ", created from:" +
                         "\n\tNE LAT: " + dsi.ne_lat +
                         "\n\tSW LAT: " + dsi.sw_lat +
                         "\n\tNE LON: " + dsi.ne_lon +
                         "\n\tSW LON: " + dsi.sw_lon +
        ...
        OMDTEDGrid omg = new OMDTEDGrid(dsi.lat_origin, dsi.lon_origin, 
                                        dsi.ne_lat, dsi.ne_lon,
    ...
    public OMRaster getOMRaster(EqualArc proj) {
        ...
        grid.generate(proj);
        com.bbn.openmap.omGraphics.grid.SlopeGenerator sg = 
            new com.bbn.openmap.omGraphics.grid.SlopeGenerator();
