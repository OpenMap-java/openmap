...
public class CGM implements Cloneable { 
    ...
    public void paint(CGMDisplay d) {
        ...
        while (e.hasMoreElements()) {
            ...
            if (!((c instanceof FillColor || 
                   c instanceof ColorCommand)&& changeCGMFill)) {
    ...
    public void changeColor(Color oldc, Color newc) {
        ...
        for (int i=0;i<V.size();i++) {
            ...
            if (temp instanceof ColorCommand) {// compare color to oldc
                currcolor = ((ColorCommand)temp).C;
                if (currcolor.equals(oldc)) {
                    ((ColorCommand)temp).C = new Color(newc.getRed(),
                                                       newc.getGreen(),
                                                       newc.getBlue());
                }
            }
