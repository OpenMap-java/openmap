...
public class OMScalingRaster
    ...
    */
   public OMScalingRaster(double ullat, double ullon, double lrlat, double lrlon, int w, int h, byte[] bytes, Color[] colorTable,
                          int trans) {
    ...
    * to object). For indexed colormodel images, it also calls computePixels, to
    * resolve the colortable and the bytes to create the image pixels.
    * 
   ...
   public boolean generate(Projection proj) {
      ...
      if (!updateImageForProjection(proj)) {
         ...
         if (this.shape == null || checkForLabel) {
            setShape();
         }
    ...
    * gets displayed fit the projection. If the source image isn't over the map,
    * then this OMGraphic is set to be invisible. If part of the image is on the
    * map, only that part is used. The OMRaster bitmap variable is set with an
    * image that is created from the source image, and the point1 variable is
    * set to the point where the image should be placed. For instance, if the
    * source image upper left corner is off the map to the NorthWest, then the
    * OMRaster bitmap is set to a image, clipped from the source, that is
    * entirely on the map. The OMRaster point1 is set to 0, 0, since that is
    * where the clipped image should be placed.
    * 
   ...
   /**
    * Return the rectangle in screen co-ordinates that the scaled image has been
    * clipped to. This may return a null rectangle (i.e. the image is out of the
    * window). Otherwise the returned rectangle should always at least partially
    * lie within the bounds of the window.
    */
    ...
    * AffineTransformOp.TYPE_BICUBIC or AffineTransformOp.TYPE_NEAREST_NEIGHBOR.
    * 
    * @param scaleTransformType
    */
   public void setScaleTransformType(int scaleTransformType) {
      if (scaleTransformType == AffineTransformOp.TYPE_BILINEAR || scaleTransformType == AffineTransformOp.TYPE_BICUBIC
   ...
   /**
    * Creates an ImageWarp object from the contents of the OMScalingRaster. This
    * can be used in an OMWarpingImage to be used for display in projections
    * that don't match the raster's projection.
    * 
