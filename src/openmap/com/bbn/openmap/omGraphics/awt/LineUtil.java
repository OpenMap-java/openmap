...
public class LineUtil {
     ...
     * @return Point2D the point at distance d from A on the segment AB,
     *          the point B if d>AB
     */
    public static Point2D interpolatedPoint(
        Point2D A,
        Point2D B,
        double d) {
    ...
    protected static boolean retrievePoints(float length, LinkedList points, LinkedList polysegment) {
        ...
        if (consumedLength == length) {
            ...
            // we need to copy the last point back: it will be the first for the next call 
