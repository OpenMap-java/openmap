...
public abstract class OMList<T extends OMGeometry>
   ...
   /**
    * Used to set the order in which the list is traversed to draw or search the
    * objects. This means that the last things on the list will be on top of the
    * map because they are drawn last, on top of everything else. For searches,
    * objects added last to the list will be considered first for a search
    * match.
    */
   ...
   /**
    * Used to set the order in which the list is traversed to draw or search the
    * objects. This means that the first things on the list will appear on top
    * because they are drawn last, on top of everything else. For searches,
   ...
   /**
    * Used for searches, when OMDist doesn't have a graphic. The index of a null
    * graphic is NONE. If you try to remove or insert a graphic at NONE, an
    * exception will be thrown. If you try to get a graphic at NONE, you'll get
    * null;
    */
    ...
    * selectClosest(), findClosest(), getOMGraphicThatContains(), etc. This flag
    * becomes really helpful for embedded OMGraphicLists, not so much for
    * top-level OMGraphicLists.
    */
   protected boolean vague = false;

   /**
    * Flag used to allow duplicates in the OMGraphicList. True by default - this
    * prevents the list from doing the extra work for checking for duplicates at
    * addition time.
    */
    ...
    * the list. This method handles the spacing of sub-member descriptions. This
    * is really a debugging method.
    * 
   ...
   /**
    * Moves the graphic at the given index to the part of the list where it will
    * be drawn on top of one of the other graphics which is its neighbor on the
    * list. This method does check to see what the traverseMode of the list is,
    * and calls either moveIndexedToLast or moveIndexedToFirst, depending on
    * what is appropriate.
    * 
   ...
   /**
    * Moves the graphic at the given index to the part of the list where it will
    * be drawn on top of the other graphics. This method does check to see what
    * the traverseMode of the list is, and calls either moveIndexedToLast or
   ...
   /**
    * Moves the graphic at the given index to the part of the list where it will
    * be drawn under one of the other graphics, its neighbor on the list. This
    * method does check to see what the traverseMode of the list is, and calls
    * either moveIndexedOneToBack or moveIndexedOneToFront, depending on what is
    * appropriate.
    * 
    * @param location the index location of the graphic to move.
    * @see #moveIndexedOneToFront(int)
   ...
   /**
    * Moves the graphic at the given index to the part of the list where it will
    * be drawn under all of the other graphics. This method does check to see
   ...
   /**
    * Moves the graphic at the given index to the front of the list, sliding the
    * other graphics back on in the list in order. If the location is already at
    * the beginning or beyond the end, nothing happens.
    * 
    ...
    * <code>forceProjectAll</code> set to <code>true</code>. This is the same as
    * calling <code> generate(p, false)</code>
    * 
    ...
    * 
    * @param p a <code>Projection</code>
    * @param forceProjectAll if true, all the graphics on the list are generated
    *        with the new projection. If false they are only generated if
    *        getNeedToRegenerate() returns true
    * @see #generate(Projection, boolean)
    */
    ...
    * <code>render()</code>! This recursively calls generate() on the OMGraphics
    * on the list.
    * 
    * @param p a <code>Projection</code>
    * @param forceProjectAll if true, all the graphics on the list are generated
    ...
    * invisible OMGeometries would save processing time and effort. If you don't
    * want visibility to be considered when processing OMGeometries/OMGraphics,
   ...
   /**
    * Get the settings for the programmatic override for shouldProcess method to
    ...
    *        here to pass on in case the OMGraphic provided is an OMGraphicList,
    *        and to use to decide if deselect should be called on the provided
    *        graphic.
    * @return OMDist with an OMGraphic if the graphic passed in is the current
    *         closest. OMDist.graphic could be null, OMDist.d could be Infinity.
    */
    ...
    * @param limit the max distance that a graphic has to be within to be
    *        returned, in pixels.
    * @return OMGraphic the closest on the list within the limit, or null if not
    *         found.
    */
    ...
    * @param addTo OMGraphicList to add found OMGraphics to, if null a list will
    *        be created.
    * @return OMGraphicList containing all of the OMGraphics within the limit,
    ...
    * 
    * @param x the horizontal pixel position of the window, from the left of the
    *        window.
   ...
   /**
    * Finds the object located the closest to the coordinates, regardless of how
    * far away it is. Sets the select paint of that object, and resets the paint
    * of all the other objects. The search starts at the first-added graphic.
    * 
    * @param x the x coordinate on the component the graphics are displayed on.
    * @param y the y coordinate on the component the graphics are displayed on.
    * @return the closest OMGraphic on the list, with selected having been
    *         called on that OMGraphics. This OMGraphic will be within the limit
    *         or null if none found. Will return this list if this list is set
    *         to be vague.
    */
   ...
   /**
    * Finds the object located the closest to the point, if the object distance
    * away is within the limit, and sets the paint of that graphic to its select
    * paint. It sets the paints to all the other objects to the regular paint.
    * The search starts at the first-added graphic. Any graphics where
   ...
   /**
    * Finds the first OMGraphic (the one on top) that is under this pixel. If an
    * OMGraphic is an OMGraphicList, its contents will be checked. If that check
    * is successful and OMGraphicList is not vague, its OMGraphic will be
    ...
    * it). If the geometry is null, the list checks for an action to take on the
    * list (deselectAll).
    */
    ...
    * This is checked by the OMGraphicList when it iterates through its list for
    * render and gesturing. It is not checked by the internal OMGraphic methods,
    ...
    * implement their own particular criteria for sorting an OMGraphicList. Does
    * nothing for a generic OMGraphicList.
    */
   public void sort() {
   }

   /**
    * Convenience method to cast an object to an OMGraphic if it is one. Returns
    * null if it isn't.
    */
