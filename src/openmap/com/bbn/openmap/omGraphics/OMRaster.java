...
public class OMRaster
    ...
    * the alpha value in each Color regulates the transparency of the image. The
    * value of this variable should stay in the range:
    ...
    * to be drawn. Otherwise, you have to figure out when the Image is complete,
    * so that you can get the layer to paint it! Use the ImageIcon constructor
    * if you don't mind blocking to wait for the pixels to arrive.
    * 
    * @param lt latitude of the top of the image, before the offset.
    * @param ln longitude of the left side of the image, before the offset.
    * @param offset_x1 number of pixels to move image to the right.
   ...
   /**
    * Lat/lon placement with XY offset with a indexed colormodel, which is using
    * a colortable and a byte array to construct the int[] pixels.
    * 
    * @param lt latitude of the top of the image, before the offset.
    * @param ln longitude of the left side of the image, before the offset.
    * @param offset_x1 number of pixels to move image to the right.
    ...
    * @param x Horizontal location of pixel from left.
    * @param y Vertical location of pixel from top.
    * @param ctIndex The array index of the applicable color in the color table.
    * @return true if x, y location valid.
    */
   ...
   public void setBits(byte[] values) {
      ...
      if ((values.length) != (height * width)) {
         if (logger.isLoggable(Level.FINE)) {
            logger.fine("OMBitmap: new byte[] size (" + +values.length + ") doesn't" + " match [height*width (" + height * width
                  + ")]");
         }
      }
   }
   ...
   /**
    * Set the color table to the int RGB values passed in. Valid for the indexed
    * colormodel only. The pixels will be colored according to these values.
    * 
   ...
   /**
    * Set the color table according to the java.awt.Color array passed in. Valid
    * for the indexed colormodel only. The pixels will be colored according to
    * these values. The transparency values of these colors will only take
    * effect of they are less than the transparency value of the images' value.
    * 
    ...
    * color table, and builds a big array of ints to use in the bitmap image. If
    * the bits are null, then the object was created in the direct color model
    * where the colors are already built into the pixels. SO, if you call this,
    * the pixels have to be null and the bits good indexes into the colortable.
    * 
   ...
   public boolean generate(Projection proj) {
      ...
      if (getNeedToRegenerate()) {
         ...
         if (colorModel != COLORMODEL_IMAGEICON) {
            ...
            if (width <= 0 || height <= 0) {
               // NeedToRegenerate should still be true, so it won't render.
               return false;
            }
