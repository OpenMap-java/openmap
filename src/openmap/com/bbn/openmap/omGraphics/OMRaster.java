...
/**
 * The OMRaster object lets you create multi-colored images.  An
 * image is a two dimensional array of pixel values that correspond to
 * some color values.  The pixels are used from the top left, across
 * each row to the right, down to the bottom row.<p>There are two
 * colormodels that are implemented in OMRaster - the direct
 * colormodel and the indexed colormodel.  The direct colormodel is
 * implemented when the pixel values contain the actual java.awt.Color
 * values for the image.  The indexed colormodel is implemented when
 * the pixel values are actually indexes into an array of
 * java.awt.Colors.  NOTE: The direct colormodel OMRaster is faster to
 * display, because it doesn't need to take the time to resolve the
 * colortable values into pixels.  <P>
 *
 ...
 * to call render before generate after the pixels have been set.
 * This feature is for cached rasters, where the content may be
 * changed later.  Use this (null pix) if you are building images in a
 * cache, for tiled mapping data or something else where the data is
 * not yet known.  The memory for the pixels will be allocated, and
 ...
 * will have to resolve empty colortables and set the pixels
 * later. Use this method (null bytes) if you are building images in a
 * cache, for tiled mapping data or something else where the data is
 * not yet known.  The memory for the pixels will be allocated, and
 * then they can be set with image data later when a database is
 * accessed.
...
public class OMRaster extends OMRasterObject implements Serializable {
     ...
     * than 255, this value is used for all colors in the image.  If
     * it is set to 255, then the alpha value in each Color regulates
     * the transparency of the image.  The value of this variable
     * should stay in the range:
    ...
    //////////////////////////////////////////////////////
    /**  
     * Just a simple check to see if the x, y pair actually fits
     ...
     * @param x Horizontal location of pixel from left.
     * @param y Vertical location of pixel from top.
     * @param ctIndex The array index of the applicable 
     ...
     * image.  Checks to see of the length matches the height *
    ...
    /**
     * Set the color table to the int RGB values passed in.  Valid
     * for the indexed colormodel only.  The pixels will be colored
    ...
    /**
     * Get the array of colors used in the indexed color model. If
     * the image is not a indexed colormodel, the int[] will be null.
     *
