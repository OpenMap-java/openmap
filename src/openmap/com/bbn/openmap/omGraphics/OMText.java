...
public class OMText
   ...
   /**
    * The default text matte stroke that is used to surround each character with
    * the color set in the textMatteColor attribute.
    */
   ...
   /**
    * The longitude location for the text, used for lat/lon or offset rendertype
    * texts, in decimal degrees.
    */
   ...
   /**
    * Location of the baseline of the text compared to the location point of the
    * OMText object. You can set this to be BASELINE_BOTTOM (default),
   ...
   /**
    * The fmHeight is the FontMetric height to use for calculating the footprint
    * for the line. This becomes important for multi-line text and text in
    * decluttering, because it dictates the amount of space surrounding the
    * text. The default height is to take into account the ascent, descent and
    * leading of the font.
    */
    ...
    * string for text. For this instance to be useful it needs text (setData), a
    * location (setX, setY, setLat, setLon) and a renderType (setRenderType).
    */
   public OMText() {
    ...
    * font, set the font sizer to null. Flushes the cache fields <code>fm</code>
    * , <code>widths</code>, and <code>polyBounds</code>. Calls setScaledFont.
    * 
   ...
   /**
    * Set the FontSizer object, which provides different font sizes at different
    * scales. If set to null, the font size will remain constant regardless of
    * projection scale.
    */
   ...
   /**
    * Get the FontSizer object, which provides different font sizes at different
    * scales. If set to null, the font size will remain constant regardless of
    * projection scale.
    */
    ...
    * 
    * @return the horizontal window location of the string, from the left of the
    *         window.
    */
   public int getX() {
      if (point != null) {
    ...
    * 
    * @param newY the vertical pixel location of the window to place the string.
    */
   ...
   /**
    * Get the latitude location of the string. Applies to LATLON and OFFSET text
    * objects.
    * 
    ...
    * the map after it has been projected. This method lets the declutter matrix
    * find out where the text should go.
    * 
    ...
    * Not for the faint hearted. Used by the DeclutterMatrix to replace text on
    * the map after it has been projected. This method lets the declutter matrix
    * put the text in an uncluttered place.
    * 
    ...
    * For best effect the stroke thickness should be larger than 1 and it should
    * be continuous.
    * 
    ...
    * HACK synchronized so that it doesn't interfere with other methods that are
    * using parsedData.
    * 
   ...
   /**
    * Sets the location of the baseline of this OMText. Flushes the cache fields
    * <code>fm</code>,<code>widths</code>, and <code>polyBounds</code>.
    * 
    ...
    * with MATT, the font was a X representation of a font. That's what is being
    * done here - we're taking the Font structure, and then going to XFont type
    * text structure. Dashes need to be included, line feeds are not. They are
    * here only for readability. The rebuildFont method brings this back to a
    * java Font.
    * 
   ...
   /**
    * This function can be called to initialize the internals such as height and
    * width of OMText. Lets you use the graphics, and thus the FontMetrics
   ...
   public synchronized void render(Graphics g) {
      ...
      switch (justify) {
         ...
         case JUSTIFY_CENTER:
            ...
            for (int i = 0; i < parsedData.length; i++) {
               renderString(g, parsedData[i], pt.getX() - (widths[i] / 2), baselineLocation + (height * i));
            }
            break;
         case JUSTIFY_RIGHT:
            computeStringWidths(fm);
            for (int i = 0; i < parsedData.length; i++) {
               renderString(g, parsedData[i], pt.getX() - widths[i], baselineLocation + (height * i));
            }
            break;
      }
   }
   ...
   protected void computeBounds() {
      ...
      if (polyBounds == null && pt != null && fm != null) {
         ...
         if (Debug.debugging("omtext")) {
            Debug.output("OMText.computeBounds() didn't compute because polybounds = " + polyBounds + " or  pt = " + pt
                  + " or fm = " + fm + ", (only polybounds should be null)");
         }
      }
   }
