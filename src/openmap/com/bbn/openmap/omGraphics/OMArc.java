...
public class OMArc
   ...
   /**
    * The simplest constructor for an OMArc, and it expects that all fields will
    * be filled in later. Rendertype is RENDERTYPE_UNKNOWN.
    */
   public OMArc() {
   ...
   /**
    * Create an OMArc, positioned with a lat-lon center and x-y axis. Rendertype
    * is RENDERTYPE_OFFSET.
    * 
   ...
   /**
    * Get the vertical pixel diameter of the arc. This is meaningful only if the
    * render type is RENDERTYPE_XY or RENDERTYPE_OFFSET.
    * 
   ...
   /**
    * Set the vertical pixel diameter of the arc. This is meaningful only if the
    * render type is RENDERTYPE_XY or RENDERTYPE_OFFSET.
    * 
         ...
         case RENDERTYPE_LATLON:
            ...
            if (proj instanceof GeoProj) {
               ...
               for (int i = 0; i < size; i += 2) {
                  ...
                  if (correctFill) {
                     float[][] alts =
                           doPolarFillCorrection(xpoints, ypoints, (llCenter.getRadLat() > 0f) ? -1 : proj.getWidth() + 1);
               ...
               tempShape =
                     new GeneralPath(proj.forwardShape(new Arc2D.Double(center.getX() - degRadius, center.getY() - degRadius,
                                                                        2 * degRadius, 2 * degRadius, start, extent, arcType)));
            }
   ...
   /**
    * An internal method designed to fetch the ArrayList for LATLON OMArcs. This
    * method is smart enough to take the calculated position information and
    * make a call to Projection.forwardArc() with start, extent and arcType
    * information.
    */
