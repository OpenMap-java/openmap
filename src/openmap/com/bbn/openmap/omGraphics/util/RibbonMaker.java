...
public class RibbonMaker
{
   ...
   private static Logger logger = Logger.getLogger("com.bbn.openmap.omGraphics.util.RibbonMaker");
   ...
   protected void handlePointsForOuterRing(Geo g1, Geo g2, Geo g3, double dist, OMAreaList ret)
   {
      ...
      else
      {
         if (bend == BENDS_LEFT)
         {
            // short, need to find intersection of two legs and remove points
    ...
    * @param g2
    * @param g3
    * @return STRAIGHT if no bend, BENDS_LEFT if bends less than PI, BENDS_RIGHT
    *         if bends more than PI.
    */
   ...
   /**
    * Takes a poly that's going to be added to the buffer and removes any points
    * that may be too close to the original poly.
    *
   ...
   protected OMGraphic getPushbackPoly(OMPoly omp, double dist)
   {
      ...
      Vector<Geo> results = new Vector<Geo>();
   ...
   protected void addShortLegPolyForIntersection(Geo g1, Geo g2, Geo g3, int ribbonSide, double dist, OMAreaList ret)
   {
       ...
       * intersection point between the two legs. So, we need to recalculate the
       * polys so the represent the entire legs, in order to find the
       * intersection point.
       *
       ...
       * points are further away than buffer distance to the opposite poly, they
       * should be included on the polygon added to the list. If a point is
       * inside that distance, it should be disregarded. If there is only one
       * point (i.e. the original intersection point), then a duplicate
    ...
    * that the Vector has valid coordinates on it. Does not do a closeness check
    * to the original poly, expected that's been done.
    *
    * @param geos a set of coordinates
    * @return OMPoly
    */
   protected OMPoly getOMPolyFromGeos(Vector<Geo> geos)
   {
   ...
   protected OMPoly getHalfPoly(RibbonIterator rIterator, int side, boolean first)
   {
      ...
      {
         Ribbon rib = (Ribbon) rIterator.next();
         Geo g = rib.get(side);
         results.add(g);
      }
      ...
      if (numCoords > 0)
      {
         ...
         Vector<Geo> newGeoCoords = new Vector<Geo>();
   ...
   {
      Vector<Geo> bufferCoords = new Vector<Geo>();
      while (rIterator.hasNext())
      {
         Ribbon rib = (Ribbon) rIterator.next();
   ...
   public OMGraphic getArc(Geo gc, OMPoly poly1, OMPoly poly2)
   {
      ...
      {
         logger.fine("Making arg starting at " + Length.DECIMAL_DEGREE.fromRadians(radAngle1) + ", "
                     + Length.DECIMAL_DEGREE.fromRadians(radAngle2 - radAngle1));
      }

      Vector<Geo> points = new Vector<Geo>();
