...
public class PropUtils {
	...
	public static Logger logger = Logger
			.getLogger("com.bbn.openmap.util.PropUtils");
	 ...
	 * 
	 * @param markerList
	 *            a string containing a space delimited list of marker names.
	 ...
	 *            the properties list string.
	 * @return Properties object containing keys and values.
	 * @param propertySeparators
	 *            the key-values are assumed to be separated from other
	 *            key-value pairs by any character in this String.
	 * @param keyValueSeparators
	 *            each key is separated from its value by any character in this
	 *            String.
	 * @throws PropertyStringFormatException
	 *             if a key doesn't have a value.
	 */
	public static Properties parsePropertyList(String list,
			String propertySeparators, String keyValueSeparators)
			throws PropertyStringFormatException {
		...
		for (int i = 0; i < keyValuePairs.size(); i++) {
			// Next, tokenize the space delimited string
			StringTokenizer tokens = new StringTokenizer(keyValuePairs
					.elementAt(i), keyValueSeparators);
	 ...
	 * 
	 * @param props
	 *            the instance to receive the loaded properties
	 * @param dir
	 *            the directory where the properties file resides
	 * @param file
	 ...
	 * 
	 * @param properties
	 *            the Properties instance to receive the properties.
	 * @param propsIn
	 *            an InputStream to read properties from
	 * @return true if the properties file exists and was loaded.
	 */
	public static boolean loadProperties(Properties properties,
		...
		try {
			...
			if (logger.isLoggable(Level.FINE)) {
				logger
						.warning("Caught IOException loading properties from InputStream.");
			}
			return false;
		}
	}
	...
	public static Properties promptUserForProperties() {
		...
		try {
			...
			System.err
					.println("PropUtils.promptUserForProperties: Exception reading properties file.");
	 ...
	 * 
	 * @param realComponentPropertyPrefix
	 *            the top-level prefix that the PropertyConsumer being set with
	 *            the properties has. Can be null.
	 * @param anyDesiredScoper
	 *            an additional scoping mechanism if there are more than one
	 *            custom editors being used for a given getPropertyInfo call.
	 * @return The string that is used for a marker for a custom editor.
	 */
	public static String getDummyMarkerForPropertyInfo(
			String realComponentPropertyPrefix, String anyDesiredScoper) {
	...
	public static String decodeDummyMarkerFromPropertyInfo(
		...
		if (possibleDummyMarker != null
			...
			if (lastDot != -1) {
				possibleDummyMarker = possibleDummyMarker
						.substring(lastDot + 1);
			}
		}
	 ...
	 *            the name of the property
	 * @param defaultPaths
	 *            the value of the paths to set if the property doesn't exist,
	 *            or if is doesn't contain anything.
	 * @return Array of strings representing paths.
	 */
	public static String[] initPathsFromProperties(Properties p,
			String propName, String[] defaultPaths) {
	 ...
	 *            properties
	 * @param propName
	 *            name of the property associated with the wanted value.
	 * @param defaultValue
	 *            what to return if the property name doesn't exist, or if the
	 *            value isn't a numerical value.
	...
	public static Color parseColorFromProperties(Properties p, String propName,
			String dfault) throws NumberFormatException {
		return ColorFactory
				.parseColorFromProperties(p, propName, dfault, false);
	}
	 ...
	 */
	public static Color parseColorFromProperties(Properties p, String propName,
			String dfault, boolean forceAlpha) throws NumberFormatException {
	...
	public static String getProperty(Color color) {
		StringBuffer hexstring = new StringBuffer(Integer.toHexString(color
				.getRGB()));
	 ...
	 * 
	 * @param askingClass
	 *            the class asking for the URL. Can be null.
	 * @param name
	 *            name of the resource, file or URL.
	 * @throws java.net.MalformedURLException
	 * @return URL
	...
	public static URL getResourceOrFileOrURL(
			Class<? extends Object> askingClass, String name)
			throws java.net.MalformedURLException {
		...
		if (retval == null) {
			...
			try {
				...
				logger.warning("AccessControlException trying to access "
						+ name);
				retval = null;
			} catch (Exception e) {
	 ...
	 * 
	 * @param i18n
	 *            i18n object to use to search for internationalized strings.
	 * @param info
	 *            the properties class being used to set information into.
	 * @param classToSetFor
	 *            class to use for i18n search.
	 * @param propertyName
	 *            property to set for.
	 * @param label
	 *            label to use for GUI (can be null if N/A).
	 * @param tooltip
	 *            tooltip to use for GUI (can be null if N/A).
	 * @param editor
	 *            editor class string to use for GUI (can be null if N/A).
	...
	public static Properties setI18NPropertyInfo(I18n i18n, Properties info,
			Class<? extends Object> classToSetFor, String propertyName,
			String label, String tooltip, String editor) {
		if (info == null) {
			info = new Properties();
		}
		if (i18n != null) {
			if (tooltip != null) {
				String internString = i18n.get(classToSetFor, propertyName,
						I18n.TOOLTIP, tooltip);
	...
	public static void putDataPrefixToLayerList(String dataPrefix,
			Properties props, String layerListProperty) {
		Vector<String> layersValue = parseSpacedMarkers(props
				.getProperty(layerListProperty));
	 ...
	 * 
	 * @param p
	 *            Properties object containing all properties
	 * @param markerListProperty
	 *            listProperty in example above
	 * @param definingProperty
	 *            definingProperty in example above, scoped property when
	 *            combined with marker name to define the class that should be
	 *            created for an object.
	 * @return
	 */
	public static List<?> objectsFromProperties(Properties p,
			String markerListProperty, String definingProperty) {
...
//		return ret;
	 ...
	 * objects are PropertyConsumers, they will be given the properties and
	 * their scoped property prefix so they can configure themselves.
	 * 
	 * <pre>
	 * 
	 ...
	 * @return
	 */
	public static List<?> objectsFromScopedProperties(Properties p,
			String markerListProperty, String definingProperty, String parentMarker) {
		String markerList = p.getProperty(markerListProperty);
		List<Object> ret = new LinkedList<Object>();
