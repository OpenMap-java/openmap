...
public class Inspector implements ActionListener {
	 ...
	 * 
	 * @param prefix
	 *            the property prefix for the property consumer. Received from
	 *            the PropertyConsumer.getPropertyPrefix() method. Properties
	 *            that start with this prefix will have the prefix removed from
	 *            the display, so the GUI will only show the actual property
	 *            name.
	 * @param props
	...
	public JComponent createEmbeddedPropertyGUI(String prefix,
		...
		String propertyList = info
				.getProperty(PropertyConsumer.initPropertiesProperty);
		Vector sortedKeys;

		if (propertyList != null) {
			Vector propertiesToShow = PropUtils
					.parseSpacedMarkers(propertyList);
		...
		for (Iterator it = sortedKeys.iterator(); it.hasNext();) { // iterate
			...
			label
					.setToolTipText(toolTip == null ? "No further information available."
							: toolTip);
		}
