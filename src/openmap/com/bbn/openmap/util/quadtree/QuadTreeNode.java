...
public class QuadTreeNode implements Serializable {
	 ...
	 * @param maximumItems
	 *            number of items to hold in a node before splitting itself into
	 *            four children and redispensing the items into them.
	 */
	...
	protected void split() {
		...
		children[NORTHEAST] = new QuadTreeNode(bounds.north, ewHalf, nsHalf,
				bounds.east, maxItems);
		children[SOUTHEAST] = new QuadTreeNode(nsHalf, ewHalf, bounds.south,
				bounds.east, maxItems);
	...
	public Object get(double lat, double lon, MutableDistance bestDistance) {
		...
		if (children == null) // This must be the node that has it...
		{
			...
			for (QuadTreeNode child : children) {
				double childDistance = child.bounds.borderDistanceSqr(
						lat, lon);
	 ...
	 * 
	 * @param north
	 *            top location in QuadTree Grid (latitude, y)
	 * @param west
	 *            left location in QuadTree Grid (longitude, x)
	 * @param south
	 *            lower location in QuadTree Grid (latitude, y)
	 * @param east
