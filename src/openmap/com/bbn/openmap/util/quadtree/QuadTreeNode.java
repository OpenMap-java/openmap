...
public class QuadTreeNode implements Serializable {
    ...
    /**
     * Added to avoid problems when a node is completely
     * filled with a single point value.
     */
     ...
     * splitting itself into four children and redispensing the items
     * into them.
     */
    ...
    /** 
     * Constructor to use if you are going to store the objects in
     * x/y space, and there is a smallest node size because you don't
     * want the nodes to be smaller than a group of pixels.
     *
     * @param north northern border of node coverage.
     * @param west western border of node coverage.
     * @param south southern border of node coverage.
     ...
     * we care).  The items in this node are cleared after they are
     ...
     *  checked first, to see if they are closer than the best
     *  distance already found.  If a closer object is found,
     *  bestDistance will be updated with a new Double object that has
     *  the new distance.
     *
     * @param lat up-down location in QuadTree Grid (latitude, y)
     * @param lon left-right location in QuadTree Grid (longitude, x)
