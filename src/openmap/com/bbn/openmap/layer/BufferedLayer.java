...
public class BufferedLayer extends OMGraphicHandlerLayer implements
	...
	public void setProperties(String prefix, Properties props) {
		...
		Vector<String> layersValue = PropUtils.parseSpacedMarkers(props
				.getProperty(prefix + LayersProperty));
		Vector<String> startuplayers = PropUtils.parseSpacedMarkers(props
				.getProperty(prefix + VisibleLayersProperty));

		Layer[] layers = LayerHandler.getLayers(layersValue, startuplayers,
				props);
	...
	public Properties getProperties(Properties props) {
		...
		for (int i = 0; i < comps.length; i++) {
			...
			if (logger.isLoggable(Level.FINE)) {
				logger.fine("BufferedLayer: getting properties for "
						+ layer.getName() + " "
		...
		props.put(prefix + VisibleLayersProperty, startupLayersListProperty
				.toString());
	...
	public class BLMapBean extends BufferedMapBean {
		...
		/**
		 * @param parent
		 *            the parent layer of this MapBean.
		 */
		public BLMapBean() {
			super(false);
		...
		public void paintChildren(Graphics g, Rectangle clip) {
			if (bufferDirty) {
				...
				Graphics gr = getMapBeanRepaintPolicy()
						.modifyGraphicsForPainting(drawingBuffer.getGraphics());
