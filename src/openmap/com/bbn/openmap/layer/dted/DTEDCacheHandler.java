 ...
 * is managed by the DTEDCacheManager, and the manager asks it for
...
public class DTEDCacheHandler {
    ...
    protected DTEDFrameColorTable colortable; // numColors, reduced colortable
    ...
    protected int currentFrameCacheSize = -10; // guarantees that it will changed first time.
	...
	this(null, DTEDFrameColorTable.DTED_COLORS, 
    ...
    public DTEDCacheHandler(String[] dataPaths, 
			    int numColors,
			    int opaqueness, 
     ...
     * to the number of frames needed to cover the screen.  If you
     * want to limit the size it can grow, set the size.  If it's
     * negative, then there will be no limit.  
     */
     ...
     * load the frame and get the data. 
     */
	...
	while (true) {
		...
		newframe = false;
		if (subx == (numXSubframes-1)) sf_width = lastSubframeWidth;
		if (suby == (numYSubframes-1)) sf_height = lastSubframeHeight;
		...
		sf_lrlon = (double)frameLon+
		    ((double)subx*width_degrees)+sf_width_degrees; 
		
		if ((ulCoords.getLatitude() > sf_lrlat && 
		     lrCoords.getLatitude() < sf_ullat) && 

		    (ulCoords.getLongitude() < sf_lrlon && 
		     lrCoords.getLongitude() > sf_ullon) &&
		    ...
		    if (Debug.debugging("dteddetail")) {
		        Debug.output(" gni: Looking for Subframe " + 
				     subx + ", " + suby);
		    }
		    ...
		    if (subframe != null) {
			if (Debug.debugging("dted")) {
			    Debug.output(" gni: Subframe " + subx + ", " + 
					 suby + " found :)");
			}
			return subframe;
		    }
		    ...
		    Debug.output(" gni: Subframe " + subx + ", " + 
				 suby + " didn't meet screen criteria");
		}
	    }
