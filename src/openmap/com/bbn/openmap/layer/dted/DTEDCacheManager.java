...
/** 
 * The DTEDCacheManager is the object you need if you want to
 * retrieve DTED files en masse for display.  You set it up with the
 * paths to the files, and then ask it for an OMGraphicList that
 * contains the frames for a geographical area.  <P>The
 * DTEDCacheManager also manages objects called DTEDCacheHandlers.
 ...
 * handlers needed.  <P>There are two calls to the Cache that you need
 * to use.  The constructor sets up the cache with the location of the
...
public class DTEDCacheManager {
     ...
     * negative or zero, the cache is calculating the cache size based on
     * the projection. 
     */
    ...
    /** 
     * Get an elevation at a point.  Always uses the main cache to
