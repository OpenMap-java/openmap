 ...
 */
public class TestLinkServer extends LinkServer 
	...
	while (true) {
		...
		while (it.hasNext()) {
			...
			if (index != Link.UNKNOWN) {
			    ...
			    clientCreatedGraphics.setOMGraphicAt(gu.graphic, index);
			} else {
     ...
     * information about the parameters of the map screen of the
     * client.
...
// 	lineProperties.setProperty(LPC_URL, "http://blatz.bbn.com/users/dietrick/LinkProtocol.html");
	...
	for (int i = 0; i < bounds.length; i++) {
	    ...
	    gr.addLine(bounds[i].maxY, bounds[i].minX, 
		       ...
		       (int)OMGraphic.LINETYPE_STRAIGHT, -1,
		       lineProperties);
	    count++;
	    ...
	    gr.addGrid(42.0f, -70.0f, 100, 100, 0.0f,
		       .001f, .001f, LinkGraphicConstants.COLUMN_MAJOR, data, 
		       new LinkProperties());
	    count++;
...
// 	des = LinkUtil.setMask(des, MOUSE_MOVED_MASK);
	...
	props.setProperty(LPC_INFO, ("Mouse Clicked at: x = " + 
							 glq.getX() + 
							 ", y = " + glq.getY()));
