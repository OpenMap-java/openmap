...
/**
 * The LinkGraphicList is an object that describes a list of
 * graphics.  This object can be used to write the graphics to the
 * link, and read the graphics response section from the link.<P>
 *
 * To use it to write to the link, create the LinkGraphicList
 * with the constructor that takes a link as its only argument, and
 * then use the write methods to add graphics.  When all the graphics
 * are written to the link, close the section by calling end() with
 * the approproate symbol.  <P>
 *
