...
public class CoverageTable {
   ...
   public final static String VDTColumnNames[] = {
      Constants.VDT_TABLE,
      Constants.VDT_ATTRIBUTE,
      Constants.VDT_VALUE,
      Constants.VDT_DESC
   };
   ...
   public final static int intVDTschemalength[] = {
      1,
      -1,
      -1,
      1,
   ...
   public final static int charVDTschemalength[] = {
      1,
      -1,
      -1,
      -1,
   ...
   private FeatureClassInfo[] internSchema(FeatureClassInfo[] fti, String foreign_key, String tablename)
   ...
   private void internSchema(boolean DCW) {
      ...
      try {
         ...
         for (List<Object> fcsrec = new ArrayList<Object>(fcs.getColumnCount()); fcs.parseRow(fcsrec);) {
            ...
            internSchema(feature_class.toLowerCase(), table1.toLowerCase(), foreign_key.toLowerCase(), table2.toLowerCase(),
      ...
      try {
         if (table1.equals("fac")) {
            ...
            textinfo = internSchema(textinfo, foreign_key, table2);
               ...
               && (foreign_key.equals("end_id") || foreign_key.equals("cnd_id") || foreign_key.equals("fac_id")
                     || foreign_key.equals("edg_id") || foreign_key.equals("txt_id"))) {
            if (Debug.debugging("vpf")) {
               Debug.output("CoverageTable: Found entry for: " + feature_class + ": " + table1 + "|" + foreign_key + "|" + table2
                     + "|" + primary_key);
            }

            FeatureClassInfo featureClass =
                  new FeatureClassInfo(this, foreign_key.intern(), tablepath.intern(), table1.intern(), table2.intern(),
                                       foreign_key.intern());
    ...
    */
   public void drawTile(TileDirectory drawtd, VPFGraphicWarehouse warehouse, LatLonPoint ll1, LatLonPoint ll2, float dpplat,
                        float dpplon) {
   ...
   /**
    * This function uses the warehouse to get a list of features, and then looks
    * in the featureList to see what feature tables handle those features. Using
    * the appropriate feature table, the function then tracks down the tile that
    * contains that feature, and the feature index into that tile file, and then
    * contacts the warehouse to get that feature created into a graphic.
    */
         ...
         try {
            ...
            for (List<Object> fcirow = new ArrayList<Object>(); fci.getRow(fcirow, getrow++);) {
               ...
               if (tileID != oldTileID) {
                  ...
                  if (currentTile == null) {
                     Debug.error("VPFLayer|CoverageTable.drawFeatures: null tile from bogus ID (" + tileID + ") from "
               ...
               if (tables.drawFeature(primitiveID, warehouse, ll1, ll2, dpplat, dpplon, currentFeature) != null) {
                  didSomething = true;
               }
    ...
    *        type.
    */
   protected boolean drawFeaturesFromThematicIndex(FeatureClassInfo fci, VPFFeatureWarehouse warehouse, TableHolder tables,
      ...
      try {
         ...
         for (int i = 0; i < indexes.length; i++) {
            ...
            if (currentTile == null) {
               Debug.error("VPFLayer|CoverageTable.drawFeatures: null tile from bogus ID (" + tileID + ") from " + fci.filename
                     + ", skipping...");
               continue;
            }
            ...
            if (currentTile.inRegion(ll1.getLatitude(), ll2.getLatitude(), ll2.getLongitude(), ll1.getLongitude())) {
               ...
               for (int j = 0; j < featureID.length; j++) {
                  ...
                  tables.drawFeature(primitiveID, warehouse, ll1, ll2, dpplat, dpplon, currentFeature);
               }
                     ...
                     if (tileID != oldTileID) {
                        ...
                        if ((tileID == -1)
                           ...
                           if (!warehouse.needToFetchTileContents(libraryname, featureName, currentTile)) {
                              if (Debug.debugging("vpf")) {
                                 Debug.output("CoverageTable: Loaded Cached List for " + featureName + " and "
                                       + currentTile.getPath());
                              }
                              continue;
                           }
   ...
   protected char whatFeatureType(VPFWarehouse warehouse, String featureName) {
      ...
      if ((type == AREA_FEATURETYPE && warehouse.drawAreaFeatures()) || (type == TEXT_FEATURETYPE && warehouse.drawTextFeatures())
...
class TableHolder {
    ...
    */
   protected OMGraphic drawFeature(int primitiveID, VPFFeatureWarehouse warehouse, LatLonPoint ll1, LatLonPoint ll2, double dpplat,
                                   double dpplon, String currentFeature)
         throws FormatException {
      ...
      if (aft != null || tft != null || edg != null || ent != null || cnt != null) {
         ...
         if ((aft != null) && aft.getRow(primitiveVector, primitiveID)) {
            return aft.drawFeature(warehouse, dpplat, dpplon, ll1, ll2, primitiveVector, currentFeature);
         }
         if ((tft != null) && tft.getRow(primitiveVector, primitiveID)) {
            return tft.drawFeature(warehouse, dpplat, dpplon, ll1, ll2, primitiveVector, currentFeature);
         }
         if ((ent != null) && ent.getRow(primitiveVector, primitiveID)) {
            return ent.drawFeature(warehouse, dpplat, dpplon, ll1, ll2, primitiveVector, currentFeature);
         }
         if ((cnt != null) && cnt.getRow(primitiveVector, primitiveID)) {
            return cnt.drawFeature(warehouse, dpplat, dpplon, ll1, ll2, primitiveVector, currentFeature);
         }
         if ((edg != null) && edg.getRow(primitiveVector, primitiveID)) {
            return edg.drawFeature(warehouse, dpplat, dpplon, ll1, ll2, primitiveVector, currentFeature);
         }
      }
    ...
    */
   protected void drawTile(TileDirectory tile, VPFGraphicWarehouse warehouse, LatLonPoint ll1, LatLonPoint ll2, float dpplat,
                           float dpplon) {
