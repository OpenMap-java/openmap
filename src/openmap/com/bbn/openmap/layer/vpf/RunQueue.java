 ...
 *
 * TerminatingRunnable objects get added to the queue.  As the thread gets
 * cpu time, it will sequentially dequeue and run the objects in the queue
 * in a LIFO order.  Duplicates are allowed in the queue.  Run will get called
 * exactly once for each enqueue call.  Only one thread will run at a time.
 * (The next object won't run until the previous one completes.)
...
public class RunQueue extends java.lang.Thread {
     ...
     * unlocks the queue, and calls run on the object that was dequeued.
