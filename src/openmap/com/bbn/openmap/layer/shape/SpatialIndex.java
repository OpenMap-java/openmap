 ...
 * <p>
 * The file has a 100 byte header identical to a Shape Index
 * followed by <i>n</i> records.
 * <p>
 ...
 *
 * <H2>Notes</H2>
 * When reading the Shape file, the content length is the length
 * of the record's contents, exclusive of the record header (8 bytes).
 * So the size that we need to read in from the Shape file is actually
 * denoted as ((contentLength * 2) + 8).  This converts from 16bit units
 * to 8 bit bytes and adds the 8 bytes for the record header.
 *
 * <H2>To Do</H2>
 * <UL>
 * <LI> index arcs</LI>
...
//          return new ESRIMultipointRecord(b, off);
    ...
    /**
     * Locates records in the shape file that intersect with
     * the given rectangle.  The spatial index is searched for
     * intersections and the appropriate records are read
     * from the shape file.
     *
...
//                  System.out.flush();
    ...
    /**
     * Determines if two rectangles intersect.  Actually, this
     * method determines if two rectangles don't intersect, and
     * then returns a negation of that result.  But the bottom
     * line is the same.
     *
...
//              Debug.output("Processed " + recNum + " records");
     ...
     * from the shape file, writing appropriate index records to
     * the spatial index file.
     *
...
//          case SHAPE_TYPE_POLYLINE:
    ...
    /**
     * Prints a usage statement describing how to use this
