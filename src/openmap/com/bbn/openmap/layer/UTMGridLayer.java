...
// **********************************************************************
// 
// $Source: /cvs/distapps/openmap/src/openmap/com/bbn/openmap/layer/UTMGridLayer.java,v $
// $RCSfile: UTMGridLayer.java,v $
// $Revision: 1.21 $
...
/**
 * The UTMGridLayer renders UTM Zone areas, and renders a grid marking
 ...
 * <p>
 * 
 * Currently, this layer only draws 100km distance squares. Updates on the way.
 * The layer has the following properties that may be set:
 * <p>
 * 
 * <pre>
 * 
 ...
 * </pre>
 */
public class UTMGridLayer extends OMGraphicHandlerLayer {
    ...
    protected OMGeometryList createUTMZoneHorizontalLines() {
        ...
        for (double lat = -80f; lat <= 72f; lat += 8f) {
            points = new double[] { lat, -180f, lat, -90f, lat, 0f, lat, 90f, lat, 180f };
            horizontalList.add(new PolylineGeometry.LL(points, OMGraphic.DECIMAL_DEGREES, OMGraphic.LINETYPE_RHUMB));
        }

        points = new double[] { 84f, -180f, 84f, -90f, 84f, 0f, 84f, 90f, 84f, 180f };
        horizontalList.add(new PolylineGeometry.LL(points, OMGraphic.DECIMAL_DEGREES, OMGraphic.LINETYPE_RHUMB));
    ...
    protected void addLabel(LatLonPoint llp, UTMPoint utm, QuadTree<OMText> labelTree) {
        ...
        labelTree.put(latitude, longitude, new OMText(latitude, longitude, 2, -2, utm.zone_number
        ...
        for (i = startNorthing; i < endNorthing; i++) {
            ...
            if (doPolys) {
                for (int j = 0; j < numVertLines; j++) {
                    ...
                    if (UTM_DEBUG_VERBOSE) {
                        Debug.output("for vline " + j + ", point " + i + ", easting: "
                                + utmp.easting + ", northing: " + utmp.northing + ", lat:"
            ...
            for (i = 0; i < vertPoints.length; i++) {
                if (UTM_DEBUG_VERBOSE) {
                    for (int k = 0; k < vertPoints[i].length; k += 2) {
                        System.out.println(" for poly " + i + ": lat = " + vertPoints[i][k]
                                + ", lon = " + vertPoints[i][k + 1]);
                    }
                }
    ...
    protected OMGeometryList createMGRSRectangles(LatLonPoint llp, int accuracy, int numRects,
        ...
        for (double i = -numRects * accuracyBonus; i < numRects * accuracyBonus; i += accuracyBonus) {
            for (double j = -numRects * accuracyBonus; j < numRects * accuracyBonus; j += accuracyBonus) {
     ...
     */
    protected OMGeometry createMGRSRectangle(MGRSPoint mgrsBasePoint, double voffset,
                                             double hoffset, double interval, Ellipsoid ellipsoid) {
        ...
        poly.putAttribute(LABEL_ATTRIBUTE, mgrsString);
        return poly;
    }

    /**
     * The prepare is the main call when the projection changes.
     * 
     * @return OMGraphicList for rendering for the current projection.
     */
    public OMGraphicList prepare() {
        ...
        if (distanceGridResolution > 0) {
            ...
            if (showLabels && p.getScale() <= dglc) {
                ...
                while (it.hasNext()) {
                    ...
                    String labelString = (String) pll.getAttribute(LABEL_ATTRIBUTE);
                    if (labelString == null) {
                        continue;
                    }
        ...
        if (showLabels && p.getScale() <= labelCutoffScale) {
            ...
            Collection labels = labelTree.get(ul.getY(), ul.getX(), lr.getY(), lr.getX());
    ...
    public Component getGUI() {
        ...
        JCheckBox setZonesButton = new JCheckBox(i18n.get(UTMGridLayer.class, "setZonesButton", "Show UTM Zone Grid"), showZones);
        ...
        JCheckBox set100kGridButton = new JCheckBox(i18n.get(UTMGridLayer.class, "set100kGridButton", "Show 100Km Distance Grid"), show100kGrid);
        ...
        JCheckBox setLabelsButton = new JCheckBox(i18n.get(UTMGridLayer.class, "setLabelsButton", "Show Zone Labels"), showLabels);
        ...
        JPanel resPanel = PaletteHelper.createPaletteJPanel(i18n.get(UTMGridLayer.class, "resPanel", "Distance Grid Units"));

        String[] resStrings = { i18n.get(UTMGridLayer.class, "resStrings.noGrid", " No Grid "),
                i18n.get(UTMGridLayer.class, "resStrings.10000m", " 10,000 meter   "),
                i18n.get(UTMGridLayer.class, "resStrings.1000m", " 1000 meter "),
                i18n.get(UTMGridLayer.class, "resStrings.100m", " 100 meter "),
                i18n.get(UTMGridLayer.class, "resStrings.10m", " 10 meter "),
                i18n.get(UTMGridLayer.class, "resStrings.1m", " 1 meter ") };
        ...
        JButton utmGridColorButton = new JButton(DrawingAttributes.getIconForPaint(getUTMGridPaint(), true));
        ...
        utmGridColorButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                Color tmpPaint = getNewPaint((Component) ae.getSource(), i18n.get(UTMGridLayer.class, "utmGridColorChooser", "Choose UTM Grid Color"), (Color) getUTMGridPaint());
                if (tmpPaint != null) {
        ...
        JLabel utmGridLabel = new JLabel(i18n.get(UTMGridLayer.class, "utmGridColorButton", "Set UTM Grid Color"));
        ...
        JButton distGridColorButton = new JButton(DrawingAttributes.getIconForPaint(getDistanceGridPaint(), true));
        ...
        distGridColorButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                Color tmpPaint = getNewPaint((Component) ae.getSource(), i18n.get(UTMGridLayer.class, "distanceGridColorChooser", "Choose Distance Grid Color"), (Color) getDistanceGridPaint());
                if (tmpPaint != null) {
        ...
        JLabel distanceGridLabel = new JLabel(i18n.get(UTMGridLayer.class, "distGridColorButton", "Set Distance Grid Color"));
    ...
    public void setProperties(String prefix, Properties props) {
        ...
        showLabels = PropUtils.booleanFromProperties(props, prefix + ShowLabelsProperty, showLabels);
        showZones = PropUtils.booleanFromProperties(props, prefix + ShowZonesProperty, showZones);
        show100kGrid = PropUtils.booleanFromProperties(props, prefix + Show100kGridProperty, show100kGrid);
        labelCutoffScale = PropUtils.floatFromProperties(props, prefix + LabelCutoffScaleProperty, labelCutoffScale);
        utmGridPaint = PropUtils.parseColorFromProperties(props, prefix + UTMGridColorProperty, utmGridPaint);
        ...
        labelsAsMGRS = PropUtils.booleanFromProperties(props, prefix + LabelsAsMGRSProperty, labelsAsMGRS);
    }
    ...
    public Properties getPropertyInfo(Properties props) {
        ...
        interString = i18n.get(UTMGridLayer.class, ShowZonesProperty, I18n.TOOLTIP, "Show UTM Zone Grid Lines.");
        props.put(ShowZonesProperty, interString);
        interString = i18n.get(UTMGridLayer.class, ShowZonesProperty, ShowZonesProperty);
        props.put(ShowZonesProperty + LabelEditorProperty, interString);
        props.put(ShowZonesProperty + ScopedEditorProperty, "com.bbn.openmap.util.propertyEditor.YesNoPropertyEditor");

        interString = i18n.get(UTMGridLayer.class, UTMGridColorProperty, I18n.TOOLTIP, "Color for UTM Zone Grid lines.");
        props.put(UTMGridColorProperty, interString);
        interString = i18n.get(UTMGridLayer.class, UTMGridColorProperty, UTMGridColorProperty);
        props.put(UTMGridColorProperty + LabelEditorProperty, interString);
        props.put(UTMGridColorProperty + ScopedEditorProperty, "com.bbn.openmap.util.propertyEditor.ColorPropertyEditor");

        interString = i18n.get(UTMGridLayer.class, ShowLabelsProperty, I18n.TOOLTIP, "Show Labels for Grid Lines");
        props.put(ShowLabelsProperty, interString);
        interString = i18n.get(UTMGridLayer.class, ShowLabelsProperty, ShowLabelsProperty);
        props.put(ShowLabelsProperty + LabelEditorProperty, interString);
        props.put(ShowLabelsProperty + ScopedEditorProperty, "com.bbn.openmap.util.propertyEditor.YesNoPropertyEditor");

        interString = i18n.get(UTMGridLayer.class, Show100kGridProperty, I18n.TOOLTIP, "Show 100Km Distance Grid Lines");
        props.put(Show100kGridProperty, interString);
        interString = i18n.get(UTMGridLayer.class, Show100kGridProperty, Show100kGridProperty);
        props.put(Show100kGridProperty + LabelEditorProperty, interString);
        props.put(Show100kGridProperty + ScopedEditorProperty, "com.bbn.openmap.util.propertyEditor.YesNoPropertyEditor");

        interString = i18n.get(UTMGridLayer.class, DistanceGridColorProperty, I18n.TOOLTIP, "Color for Equal-Distance Grid Lines.");
        props.put(DistanceGridColorProperty, interString);
        interString = i18n.get(UTMGridLayer.class, DistanceGridColorProperty, DistanceGridColorProperty);
        props.put(DistanceGridColorProperty + LabelEditorProperty, interString);
        props.put(DistanceGridColorProperty + ScopedEditorProperty, "com.bbn.openmap.util.propertyEditor.ColorPropertyEditor");

        interString = i18n.get(UTMGridLayer.class, DistanceGridResolutionProperty, I18n.TOOLTIP, "Meter Resolution for Distance Grid Lines (0-5)");
        props.put(DistanceGridResolutionProperty, interString);
        interString = i18n.get(UTMGridLayer.class, DistanceGridResolutionProperty, DistanceGridResolutionProperty);
        props.put(DistanceGridResolutionProperty + LabelEditorProperty, interString);

        PropUtils.setI18NPropertyInfo(i18n, props, UTMGridLayer.class, LabelsAsMGRSProperty, "Labels as MGRS", "Flag to display labels in MGRS notation, or UTM.", "com.bbn.openmap.util.propertyEditor.YesNoPropertyEditor");

        props.put(initPropertiesProperty, ShowZonesProperty + " " + UTMGridColorProperty + " "
                + Show100kGridProperty + " " + ShowLabelsProperty + " "
                + DistanceGridResolutionProperty + " " + DistanceGridColorProperty + " "
    ...
    public void setDistanceGridResolution(int value) {
        ...
        if (distanceGridResolution < 0 || distanceGridResolution > MGRSPoint.ACCURACY_1_METER) {
