...
public class RpfCacheHandler {
   ...
   /**
    * A flag to let the cache manager know that the subframes needed for the map
    * make sense.
    */
    ...
    * subframeBuffer refers to the number of subframes added on each side of the
    * matrix.
    */
...
//    private float lastScaleDifference = -1f;
    ...
    * changes, the frame cache gets tossed and recreated via setScreenSubframes.
    * 
    ...
    */
   public synchronized void setCache(float ullat, float ullon, float lrlat, float lrlon, CADRG proj) {
...
//         if (currentPercentCoverage <= currentBox.setPercentCoverage(ullat, ullon, lrlat, lrlon, start, end)
...
//      }
   ...
   protected OMGraphicList getSubframes(float ullat, float ullon, float lrlat, float lrlon, CADRG proj, OMGraphicList omGraphics) {
      ...
      for (int subx = start.x; subx <= end.x; subx++) {
         for (int suby = start.y; suby <= end.y; suby++) {
            ...
            if (subframe != null) {
               ...
               omGraphics.add(subframe.image);

               subframe.image.generate(proj);
               subframe.image.setSelected(viewAttributes.showInfo);
    ...
    * is bigger than this cache size, you'll need to use the getCached that lets
    * you supply the subframe number that you are requesting, so that the
    * RpfCacheHandler knows when to stop caching subframes during a retrival..
    * Otherwise, the cache will overwrite data and subframes will not show up on
    * the map.
    */
    ...
    * equal to the size of the subframe cache, then use this method to provide a
    * running count of how many subframes you've already called for to use in
    ...
    *        so far for the current map. Should be used it there is concern that
    *        the number of subframes needed for the map is greater than the size
    *        of the subframe.
    */
    ...
    * number gets bigger than the cache size, then the RpfCacheHandler will keep
    * fetching data without storing the extra subframes in the cache. Otherwise,
    * the previous images in the cache would be replaced before they were
    * painted, and they would not appear on the map. If subframeCount is less
    * than subframe size, then the latest retrieved subframe will be stored in
    * the cache.
    * 
    * @param cbx the x index of subframe in the rcbIndex A.TOC space.
    * @param cby the y index of subframe in the rcbIndex A.TOC space.
    * @param subframeCount a running count of the number of subframes retrieved
    *        so far for the current map. Should be used if there is concern that
    *        the number of subframes needed for the map is greater than the size
    *        of the cache.
    */
            ...
            if (ret.opaqueness != viewAttributes.opaqueness) {
               ...
               if (pixels != null) {
                  ...
                  for (int i = 0; i < pixels.length; i++) {
                     pixels[i] = (0x00FFFFFF & pixels[i]) | (viewAttributes.opaqueness << 24);
                  }
