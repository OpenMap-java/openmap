...
//      }
     ...
     * valid.
     */
    public OMColor[] getColors(BinaryFile binFile, 
			       RpfColortable ct) {
...
// 	    binFile = new BinaryFile(framePath);
     ...
     * resulting image will be loaded into the RpfSubframe.  If null,
     * a new RpfSubframe will be created.
     ...
     */
    public RpfSubframe decompressSubframe(int x, int y,
     ...
     */
    public int[] decompressSubframe(int x, int y,
				    RpfColortable colortable) {
	// Convert x,y to the subframe index in the frame - they come in as
	// cache subframe indexes
	x = x % 6;
	...
	com.bbn.openmap.util.ArgParser ap = new 
	    com.bbn.openmap.util.ArgParser("RpfFrame");
	...
	BufferedImage bigImage = 
	    new BufferedImage(width * 6, height * 6,
	...
	for (int x = 0; x < 6; x++) {
		...
		java.awt.Image bitmap = tk.createImage(
	...
	JLabel picture = 
	    new JLabel(new ImageIcon(bigImage));
