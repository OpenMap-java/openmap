...
public class RpfCacheManager
    ...
    * returned. This function also figures out what part of the earth is covered
    * on the screen, and creates auxiliary cache handlers as needed. The CADRG
    * projection held inside the view attributes, used by the RpfTocHandlers, is
    * set here. If the projection passed in is not CADRG, and the caller doesn't
    * care, then a new CADRG projection is created.
    * 
   ...
   public synchronized OMGraphicList getRectangle(Projection proj) {
      ...
      if (!(proj instanceof CADRG)) {
         if (viewAttributes.requireProjection) {
            ...
            viewAttributes.proj =
                  new CADRG((LatLonPoint) proj.getCenter(new LatLonPoint.Float()), proj.getScale(), proj.getWidth(),
                            proj.getHeight());
         }
      } else {
       ...
       * the equator and the dateline. When the screen straddles one or both of
       * these lat/lon lines, lon_minus and lat_minus get adjusted, causing two
       * or four different calls to the tochandler to get the data above/below
       * the equator, and left/right of the dateline. Plus, each path gets
       * checked until the required boxes are filled.
       */
      ...
      // box.
      caches[0].getSubframes(lat[ya - lat_minus], lon[xa - lon_minus], lat[ya], lon[xa], viewAttributes.proj, list);
      ...
      if (lon_minus == 1) {
         ...
         caches[1].getSubframes(lat[ya - lat_minus], lon[0], lat[ya], -1f * lon[1], viewAttributes.proj, list); // -1
         // to
         // make
         // it
         // 180
      ...
      if (lat_minus == 1) {
         ...
         caches[2].getSubframes(lat[0], lon[xa - lon_minus], -1f * lat[1], // flip
                            // breather
                            lon[xa], viewAttributes.proj, list);
      ...
      if (lon_minus == 1 && lat_minus == 1) {
         ...
         caches[3].getSubframes(lat[0], lon[0], -1f * lat[1],
                            -1f * lon[1], viewAttributes.proj, list);
