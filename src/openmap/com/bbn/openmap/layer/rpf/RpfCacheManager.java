 ...
 * RPF files.  You set it up with a RpfFrameProvider, and it gives
 * that provider to the RpfCacheHandler it creates.  The
 * RpfCacheManager then handles working with the RpfCacheHandlers to
 * get subframes to display.
 * <P>
 *
 ...
 * needed in case there are many places where there is RPF data.  <P>
 *
 ...
 * and creates the cache handlers needed.  <P>There are two calls to
 * the Cache that you need to use.  The constructor sets up the cache
 * with the location of the data.  The getRectangle() call returns an
...
public class RpfCacheManager implements Serializable {
    ...
    /** Contains information about displaying the RPF data. Also
     * passed to the RpfTocHandlers to determine chart selection.
    ...
    /** The size of the aux caches, which are used when the map
     *  crosses the equator or dateline. */
...
//      }
    ...
    /**
     *  Returns the Vector containing RpfCoverageBoxes from the
     *  primary RpfCacheHandler.  The Vector is the same that was
     *  returned to the cache handler from the RpfFrameProvider as a
     *  result of the last setCache call.  These provide rudimentary
     *  knowledge about what is being displayed.
