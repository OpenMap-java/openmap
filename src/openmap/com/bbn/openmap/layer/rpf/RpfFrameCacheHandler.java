...
//      }
     ...
     *
     * 1. Fixed bug that was preventing it from actually returning
     * the best coverage box in the first position of the Vector. 
     * 2. Prevented the method from returning early when it found a
     * box that claimed perfect coverage.  This was causing problems in some
     * areas, where boxes that claimed coverage didn't actually have it.  This
     * could be a data problem, so maybe we can add the early return back in
     * in the future.
     * 3. Changed < to <= below.
	...
	for (i = 0; i < tocs.length; i++) {
           ...
           // results.
           coverageEntries = tocs[i].getBestCoverageEntry(ullat, ullon,
               lrlat, lrlon, proj,
               viewAttributes);
			    ...
			    if (!addedCurrent) {
                              if (j == 0) {
                                ...
                                // scale, and then considering coverage.
                                 ...
                                 // scale difference but the second one actually
                                 // had better coverage, it would have gotten
                                 // discarded.
                                if (scaleDifference(proj, currentCoverage) <=
                                    scaleDifference(proj, rcb) && currentCoverage.percentCoverage
                                    >= rcb.percentCoverage) {
                                  coverageBoxes.addElement(currentCoverage);
                                  addedCurrent = true;
                                  if (currentCoverage.scale == rcb.scale) {
                                    ...
                                    // if these scales don't match, none of the
                                    // subsequent rcbs will either. We're done.
                                    break;
                                  }
                                } else {  // rcb percent coverage is better
                                  coverageBoxes.addElement(rcb);
                                }
                              } else {  // j!= 0
                                if (currentCoverage.percentCoverage >= rcb.percentCoverage) {
                                  ...
                                  // we already know that scale matches since it was
                                  // added previously
                                  coverageBoxes.add(rcb);
                                }
                              }
                        ...
                        // than someone else but wasn't added because of the scale.
                        // If that happened, then okToAddCurrent is false.
