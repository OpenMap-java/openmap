 ...
 * important to have a valid A.TOC directory.  <P> 
 *
 ...
 * within the RPF directory.  That's why the RpfLayer needs a path to
 * a RPF directory - it's really looking for the A.TOC file, and
 * knows where to find it.  It also needs a path to the RPF directory
 * because it needs to prepend that path to the paths to the files
 * that the A.TOC file knows about. <P>
 *
 * The A.TOC files that can be created with this MakeToc class can be
 * created to contain absolute frame paths.  The MakeToc class can
 * take the paths to several RPF directories, and create a single
 * A.TOC file that preserves all of their current file paths.  You
 * have to use alot of caution with this capability, however.  These
 * A.TOCs containing absolute file paths will not work if the data is
 * moved to another machine, or if referenced by a machine with a
 * different type file system (i.e. Windows).  They may not work for
 * other implementations of code that display RPF data - the code in
 * this package has been modified to test for absolute file names. <P>
 * 
 ...
 * there is more than one RPF directory. <P>
 * <pre>
 ...
 * for other options.
 *
 * <P>NOTE: Make sure that the RPF directories and their contents are in
 * upper case.  Its a spec requirement, although with CD copies and
 * FTP downloads, the file name cases sometimes get switched.  Use
 * com.bbn.openmap.layer.rpf.ChangeCase to modify the file name
 * cases. </P>
...
public class MakeToc {
     ...
     * RPF file root directory.  Other options can be found by using a
     * -help option.
     */
        ...
        if (arg != null) {
            try {
                ...
                Debug.output("MakeToc: Tried to pass a bogus integer (" +
                             arg[0] + ") as a boundary limit.  Using default of 200.");
                max_side = DEFAULT_MAX_SIDE;
            }
        }
        mt.setMaxSide(max_side);
        mt.fireProgressUpdate(ProgressEvent.START,
                              "Searching for RPF frames", 0, 100);
     ...
     * these paths are relative, the MakeToc class should be set for
     * that.
     ...
     * their own group.  False is default.  Dchum are replacement subframes.
     * @exception MakeTocException if anything goes wrong.  
     */
     ...
     */
    public void organizeFrames(String[] framePaths, RpfHeader head, Vector frames) {
        ...
        for (i = 0; i < nFrames; i++) {
            ...
            if (Debug.debugging("maketoc")) {
                Debug.output("MakeToc: frame number " + i + 
                             ", " + framePath);
            }
...
//              binFile = new BinaryFile(framePath);
                ...
                if (!head.read(binFile)) {
                    ...
                    Debug.error("MakeToc: " + framePath + " is not a RPF file - ignoring");
                    continue;
                }
            ...
            // This will be the actual file name, without parental path.
            String framename;
...
//              File newFile = new File(frame.filename.substring(0, tail),
//                                      head.filename);
...
//          }
            ...
            if (rpi == RpfConstants.UK) {
                ...
                // need to figure out how to consult the frame for what it is.
                // RpfAttributes.chartSeriesCode might have something to base it off.
                // GNC = GN, JNC = JN, ONC = ON, TPC = TP, JOG = 15, TLM50 = V7,
                // But I'm not sure about the others.  For now, prompt for scale.
                scaleString = promptForScale("What is the scale for " + frame.filename + "? (Answer should look like: 1:XXX,XXX)");
            ...
            if (isoverview) {
                coverage.nwlat = coverage.nelat =
                    coverage.nwlon = coverage.swlon =
                    coverage.swlat = coverage.selat =
                    coverage.nelon = coverage.selon = 0;
                coverage.latInterval =
                    coverage.lonInterval =
                    coverage.nsVertRes =
                    coverage.ewHorRes = 0;
            }
            ...
            if (Debug.debugging("maketocframedetail")) {
                Debug.output("MakeToc: nw_lon = " + coverage.nwlon + 
                             ...
                             "\n         nwlat = " + coverage.nwlat + 
                             ", selat = " +  coverage.selat +
                             "\n    NEW: swlat = " + coverage.swlat + 
                             ", swlon = " + coverage.swlon +
     ...
     */
    public void writeTOCFile(String filename, RpfHeader head, 
                             Vector frames, Vector groups) 
        throws MakeTocException {
        ...
        int TOC_Nitf_hdr_size = 0;       /* ?? Nitf header size for output TOC */
        ...
        int Frame_index_rec_len = 33;  /* Frame index record length (was 37) */
        ...
        int[] uniq_dir_ptr = new int[nFrames]; /* index from filename to uniq direct. */
        int[] uniq_dir_pos = new int[nFrames]; /* position of direct. name in file */
        ...
        try {
            ...
            fout.writeBoolean(head.endian); // Big Endian - should match head.endian
            ...
            int Loc_sec_comp_len = 10; /* Location section component length */
            ...
            /* compon. aggregate len: unknown here. Fill in after doing all else.*/
            ...
            Frame_sec_len = Frame_hdr_len + 
                nFrames * Frame_index_rec_len + path_table_len ;
            ...
            /* 0 + 48 + 54 */
            fout.writeInt(TOC_Nitf_hdr_size + head.HEADER_SECTION_LENGTH + Loc_sec_len);
            ...
            /* DKS.   Physical location */
            /* 0 + 48 + 54 + 8 */
            fout.writeInt(TOC_Nitf_hdr_size + head.HEADER_SECTION_LENGTH +
            ...
            for (i = 0; i < groupCount; i++) {
                ...
                /* Key off flag to write proper data to A.TOC for browse menu later */
                if (group.cib) {
                    fout.writeBytes("CIB  ");
                    ...
                    fout.writeBytes("6.5:1");  /* Compr. ratio: VARIABLE */
                }
                else {
                ...
                /* PBF - If group is polar, change boundaries from rect coordinates
                   to lat-lon -- 6-19-94 */
                if (group.zone ==  '9' || group.zone == 'J') {     /* polar zone */
                    ...
                    if (group.zone == '9') {   /* "9": NORTH POLE */
                        top = 90 - (Math.sqrt((ytop * ytop) + (xleft * xleft)) * 
                                    (group.h_interval));
                        bottom = 90 - (Math.sqrt((ybottom * ybottom) + (xright * xright)) *
                                       (group.h_interval));
                    } /* North pole */
                    ...
                    /* For both poles: */
                    if (xleft < 0) { /* left half of earth has negative longits */
                        left_t = -left_t;
                        left_b = -left_b;
                    }
                    /* This will hardly ever happen: */
                    if (xright < 0) {  /* left half of earth has negative longs */
                        right_t = -right_t;
                        right_b = -right_b;
                    }
                    ...
                    //  #if 0
                    //                  /* !!!!!!!!!!!!!!!!!!! Fix to getlat [80,90], longit. [-180,180] */
                    //                  bottom = 80.0 ;
                    //                  top = 90.0 ;   
                    //                  left = -180.0 ;
                    //                  right = 180.0 ;
...
//              continue;
                ...
                /* DKS.  Changed from top to bottom to fix bug in Theron's frame numbering */
                /* Should start numbering at BOTTOM (southern-most part) of group */
                ...
                /*    ui = head.HEADER_SECTION_LENGTH + Loc_sec_len + Bound_sec_len + Frame_hdr_len + nFrames*Frame_index_rec_len + pathname_pos[i] ;  */
            ...
            /* Pathname table */
            /* Write UNIQUE pathnames: really Directory name, e.g. "./CENTRAL.USA/" */
            for (j = 0; j < uniq_dir_cnt; j++) {
                /* DKS new */
                /* write pathname length. !!?? may be padded in front to
                   align on word bndary!!?? */
    ...
    public void groupFrames(Vector frames, Vector groups, boolean isDchum) 
        ...
        for (int i = 0; i < nFrames; i++) {
            ...
            if (!frame.marked) {
                ...
                /* If Dchum, create 1 group for each file.  No need
                   for call to "add". */
                if (!isDchum) {
    ...
    protected boolean addFrameToGroup(Group grp, Frame frm, int index) 
        ...
        if (frm.marked ||
            !frm.scale.equalsIgnoreCase(grp.scale) ||
            frm.zone != grp.zone) {
        ...
        if (frm.left >= grp.vert_pos[grp.left] - eps &&
            ...
            if (Debug.debugging("maketocdetail")) {
                Debug.output(frm.filename + " add frame to group " + 
                             index + ": left side");
            }
            
            if (grp.left == 0) {
                throw new MakeTocException("Boundary rectangle too small - Increase the boudary size to be larger than " + maxSide);
            }
        ...
        /* DKS 8/16/94: h_resolution (meters/pix) will vary from frame to frame NS */
    ...
    protected String latlong2GEOREF(double latitude, double longitude) {
        ...
        if (Debug.debugging("maketocdetail")) {
            Debug.output("latlon2GEOREF: lat = " + latitude + 
                         ", lon = " + longitude +
                         ", GEOREF = " + ret);
        }
