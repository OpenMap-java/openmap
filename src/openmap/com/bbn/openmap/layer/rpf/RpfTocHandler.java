...
public class RpfTocHandler {
   ...
   /**
    * Set by the RpfFrameProvider, and used to track down this particular TOC to
    * get to the frames offered by it's coverages.
    */
    ...
    * specification. This is reset automatically when the A.TOC file is read. If
    * the first two characters of the directory paths are ./, then it stays
    * false.
    */
    ...
    * 
    * NOTE: method getZone() of the CADRG projection is only relevant (according
    * to OpenMap documentation) when you're viewing a map type (ONC, etc) at its
    * proper scale (i.e. 1:1mil for ONC). There was a method in RpfTocHandler
    * that only checked a TOC for coverage if the TOC zone matched the zone of
    * the projection. This caused gaps of coverage when viewing the maps at
    * large scales that were different from their proper scale (e.g. viewing JNC
    * at 1:10mil). Modified this method so that it obtains all the possible
    * zones the current map projection could be in, and compares the TOC zones
    * to that set.
    * 
    ...
    */
    public List<RpfTocEntry> getBestCoverageEntry(float ullat, float ullon, float lrlat,
       ...
       for (RpfTocEntry currentEntry : entries) {
         ...
         if (scaleFactor >= lowerScaleFactorLimit
            ...
            if (isOkZone(currentEntry.zone, okZones)) {
               ...
               } else if (hits > prevBoundaryHits && (currentEntry.coverage.setPercentCoverage(ullat, ullon, lrlat, lrlon) > 0f)) {
                  bestEntry = currentEntry;
                  prevBoundaryHits = hits;
