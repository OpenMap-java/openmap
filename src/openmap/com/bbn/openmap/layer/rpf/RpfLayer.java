...
public class RpfLayer
   ...
   /**
    * The main source for the images and attribute information. All requests for
    * graphic objects should go through this cache, and it will automatically
    * handle getting the frame files, decoding them, and returning an object
    * list.
    */
   ...
   public Properties getPropertyInfo(Properties list) {
      ...
      interString = i18n.get(RpfLayer.class, AuxCacheSizeProperty, I18n.TOOLTIP, "Number of frames to hold in aux. frame caches.");
      ...
      list.put(initPropertiesProperty, RpfPathsProperty + " " + KillCacheProperty + " " + CacheSizeProperty + " "
            + AuxCacheSizeProperty + " " + viewAttributes.getInitPropertiesOrder() + " " + AddToBeanContextProperty + " "
            + AddAsBackgroundProperty + " " + RemovableProperty + " " + CoverageProperty + " " + tmpCov.getInitPropertiesOrder());
   ...
   public void setList(OMGraphicList list) {
      super.setList(list);
   }
   ...
   public synchronized OMGraphicList prepare() {
      ...
      if (Debug.debugging("rpf")) {
         Debug.output(getName() + "|RpfLayer.prepare(): " + "calling getRectangle " + " with projection: " + projection + " ul = "
      ...
      if (!(projection instanceof CADRG)) {
         cadrgProj =
               new CADRG((LatLonPoint) projection.getCenter(new LatLonPoint.Float()), projection.getScale(), projection.getWidth(),
      ...
      try {
         ...
         retList.add(this.cache.getRectangle(cadrgProj));

         if (Debug.debugging("basic")) {
            Debug.output("RpfLayer.prepare(): finished with " + retList.size() + " graphics");
         }

      } catch (java.lang.NullPointerException npe) {
         Debug.error(getName() + "|RpfLayer.prepare(): Something really bad happened - \n " + npe);
         npe.printStackTrace();
