...
public class RpfLayer extends OMGraphicHandlerLayer implements ActionListener,
	 ...
	 * 
	 * @param pathsToRPFDirs
	 *            paths to the RPF directories that hold A.TOC files.
	 */
	 ...
	 * 
	 * @param pathsToRPFDirs
	 *            Array of strings that list the paths to RPF directories.
	 */
	...
	public Properties getProperties(Properties props) {
		...
		props
				.put(prefix + KillCacheProperty, new Boolean(killCache)
						.toString());
		props.put(prefix + CacheSizeProperty, Integer
				.toString(subframeCacheSize));
		props.put(prefix + AuxCacheSizeProperty, Integer
				.toString(auxSubframeCacheSize));
	...
	public Properties getPropertyInfo(Properties list) {
		...
		interString = i18n.get(RpfLayer.class, RpfPathsProperty, I18n.TOOLTIP,
				"Paths to RPF directories.  Semi-colon separated paths.");
		list.put(RpfPathsProperty, interString);
		list
				.put(RpfPathsProperty + ScopedEditorProperty,
		...
		interString = i18n
				.get(RpfLayer.class, KillCacheProperty, I18n.TOOLTIP,
		...
		interString = i18n
				.get(RpfLayer.class, KillCacheProperty, "Clear Cache");
		list.put(KillCacheProperty + LabelEditorProperty, interString);

		interString = i18n.get(RpfLayer.class, CacheSizeProperty, I18n.TOOLTIP,
				"Number of frames to hold in the frame cache.");
		list.put(CacheSizeProperty, interString);
		interString = i18n.get(RpfLayer.class, CacheSizeProperty,
				"Frame Cache Size");
		list.put(CacheSizeProperty + LabelEditorProperty, interString);

		interString = i18n.get(RpfLayer.class, AuxCacheSizeProperty,
				I18n.TOOLTIP, "Number of frames to hold in aux. frame caches.");
	 ...
	 * 
	 * @param rva
	 *            the RpfViewAttributes used for the layer.
	 */
		...
		if (coverage != null) {
			if (viewAttributes != null
					&& !viewAttributes.chartSeries
							.equalsIgnoreCase(RpfViewAttributes.ANY)) {
	 ...
	 * 
	 * @param e
	 *            The projection event, most likely fired from a map bean.
	 */
	public void projectionChanged(ProjectionEvent e) {
		projectionChanged(e, true);
	}
	 ...
	 * @param e
	 *            The projection event, most likely fired from a map bean.
	 * @param saveGraphicsForRedraw
	 *            flag to test for whether the scale and zone has changed for
	 *            the projection. If true, and the scale and zone is the same,
	 *            we'll just reproject and redraw the current frames before
	 *            getting new ones, to fake something happening quickly.
	 */
	public void projectionChanged(ProjectionEvent e,
		...
		if (saveGraphicsForRedraw && oldProj instanceof CADRG
			 ...
			 **/
				...
				// In case the list gets rendered before new frames are loaded,
				// the old ones will be drawn at the correct location.
	...
	public synchronized OMGraphicList prepare() {
		...
		if (!(projection instanceof CADRG)) {
			cadrgProj = new CADRG((LatLonPoint) projection
					.getCenter(new LatLonPoint.Float()), projection.getScale(),
	...
	public java.awt.Component getGUI() {
		if (box == null) {
			...
			showMapsCheck = new JCheckBox("Show Images",
					viewAttributes.showMaps);
			showMapsCheck.setActionCommand(showMapsCommand);
			showMapsCheck.addActionListener(this);

			showInfoCheck = new JCheckBox("Show Attributes",
			...
			if (coverage != null) {
				JCheckBox showCoverageCheck = new JCheckBox("Show Coverage",
						coverage.isInUse());
			...
			JButton setProperties = new JButton(i18n.get(RpfLayer.class,
					"setProperties", "Change All Settings"));
			...
			JPanel opaquePanel = getTransparencyAdjustmentPanel(i18n.get(
					RpfLayer.class, "layerTransparencyGUILabel",
					"Layer Transparency"), JSlider.HORIZONTAL,
	...
	public static void main(String[] argv) {
		...
		System.out
				.println("# This property should reflect the paths to the RPF directories");
		System.out.println("layer.paths=<Path to RPF dir>;/cdrom/cdrom0/RPF");
		System.out
				.println("# Optional properties - Defaults will be set for properties not included (defaults are listed):");
		System.out
				.println("# Number between 0-255: 0 is transparent, 255 is opaque");
		System.out.println("layer.opaque=255");
		System.out
				.println("# Number of colors to use on the maps - 16, 32, 216");
		...
		System.out
				.println("# The scale factor to allow when scaling images (2x, 4x, also mean 1/2, 1/4).  Default is 4.");
		...
		System.out
				.println("# Limit the display to the chart code specified. (GN, JN, ON, TP, etc.)");
		System.out.println("layer.chartSeries=ANY");
		System.out
				.println("# Set the subframe cache size. (Number of subframes to hold on to, 256x256 pixels");
		System.out.println("layer.subframeCacheSize=128");
		System.out
				.println("# Get the subframe attribute data from the frame provider.");
