 ...
 * 
 * <P>The profile tool uses the ProfileStateMachine, and the Profile
...
public class ProfileGenerator implements TerrainTool {
    ...
    /** Array of LatLonPoints.  The points are the clicked
     * points, and the points in between, on a great circle.  Have to
     * figure these points out, and not rely only on the poly line
     * points, because we need to get the elevations for all the
    ...
    /**
     * Create the line object, the state machine, and the vectors
     * used to keep track of the line being drawn.
     */
    public void init() {
        lastMouse = null;
        ...
        profileLine = new OMPoly(setLLPoints(), OMGraphic.RADIANS,
...
//      System.loadLibrary("com_bbn_openmap_terrain_ProfileGenerator");
    ...
    public void createProfileImage() {
        ...
        for (int i = 0; i< heights.length; i++) {
            ...
            if (tmp == DTEDFrameCache.NO_DATA) tmp = -1;
            if(tmp > max) max = tmp;
            heights[i] = tmp;
        }
    ...
    protected void createGIFFile(int distance, int max,
        ...
        graphics.fillRect(gif_width_buffer, gif_height_buffer, 
        ...
        // inside edge
        graphics.drawRect(gif_width_buffer, gif_height_buffer, 
                          box_width, box_height);
        ...
        for (int i = 1; i<post_height.length; i++) {
            graphics.drawLine(gif_width_buffer+total_distance, 
                              ...
                              gif_height_buffer+box_height-box_height_buffer - post_height[i]);
        ...
        javax.swing.JFrame jf = 
            com.bbn.openmap.util.PaletteHelper.getPaletteWindow(new javax.swing.JLabel(ii),
                                                                "Path Profile", 
...
//          } else {
//              Debug.error("ProfileGenerator: can't create image file, because the openmap.TempDirectory was not set.");
//          }
//      } catch (IOException e) {
...
//      layer.fireRequestBrowserContent(imageString);
    ...
    /**
     * Used to keep track of another point for the line, as
     ...
     * between two points on the screen.  The interior points are
     * added to the coords array, but not to the xy points array.
     *
    ...
    protected void addGreatCirclePoints(MouseEvent beginning, MouseEvent ending) {
        ...
        int num_points = (TerrainLayer.numPixelsBetween(beginning.getX(), 
                                                        beginning.getY(),
                                                        ending.getX(), 
                                                        ending.getY()) - 2)/MAX_SPACE_BETWEEN_PIXELS;

        float[] radPoints = GreatCircle.great_circle(beg.radlat_, beg.radlon_,
                                                     end.radlat_, end.radlon_,
                                                     num_points, true);
