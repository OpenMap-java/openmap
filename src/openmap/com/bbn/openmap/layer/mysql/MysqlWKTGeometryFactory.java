 ...
 * Geometry Objects. It takes as input a WKT Representation (text)
 * of the Geometry. It should have less performance than one
 * which manipulates WKB (binary) --> ToDo.<p>
 *
...
public class MysqlWKTGeometryFactory {
	...
	if (type.equals(MysqlGeometry.POINTTYPE)) {
		...
		for(int i = 0; i < typeVector.size();i++) {
		    ...
		    MysqlGeometry newGeometry = util(newRoot,(String) typeVector.elementAt(i));
	    ...
	    Debug.output("MysqlWKTGeometryFactory: Type "+
	...
	for (int i=0;i < n.countChildren(); i++) {
	    Vector v = parseStringByToken(n.getChildByNumber(i).getGeoWKT().replaceAll(","," "), ' ');
	    double[] d = createDoubleLatLongArray(v);
	    rings.add(d);
	} 
	g.setRings(rings);
