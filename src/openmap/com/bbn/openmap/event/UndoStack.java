...
public class UndoStack
    ...
    * current state to this stack. This current state reflects the state of some
    * object at the current time. If another state gets set, then the old state
    * is now put on the undo stack. If the undo command is given to this stack,
    * then an event is popped off the undo stack, set to be the current state.
    * The old state gets pushed to the redo stack.
    */
   ...
   /**
    * Sets the current state of some object on the stack. The stack doesn't care
    * what that event/state represents, since it has all the info needed to tell
    * components how to get back to this state later. Anyway, this current state
    * kinda hangs out in limbo. If another event/state comes in, the current
    * state gets pushed on the undo stack. The redo stack gets cleared, since a
    * new state path forward has been established.
    * 
    * @param event
    */
