 ...
 *
 * DropListenerSupport keeps a list of all DefaultDnDCatcher objects from
 * a MapHandler. It adds itself to the MapBean as MouseListener and
 * MouseMotionListener. On MousePressed, MouseDragged, and MouseReleased events
 * it loops through the DnDCatchers and invokes a consume() method in each
 * of them.
 *
 * @see DefaultDnDCatcher
 */
public class DropListenerSupport
    implements PropertyChangeListener, java.io.Serializable, BeanContextChild, BeanContextMembershipListener, SoloMapComponent, MouseListener, MouseMotionListener {
     ...
     * been removed from the parent BeanContext.  The DropListenerSupport
     * looks for the MapBean it is managing MouseEvents for, and any
     * DefaultDndCatchers that may be removed.
     *
     * @param bcme event that contains an iterator that can be used to
    ...
    /**
     * Called when an object should be evaluated by the DropListenerSupport
    ...
    /**
     * Eventually gets called when the DropListenerSupport is added to the
     * BeanContext, and when other objects are added to the
     * BeanContext anytime after that.  The DropListenerSupport looks for a
     * MapBean to manage MouseEvents for, and DefaultDndCatchers to use to
