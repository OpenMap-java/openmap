 ...
 *
 * Drag: When a mouseDragged event occurs, DropListenerSupport forwards it
 * to the DefaultDnDCatcher (consume() method). If it's the first mouseDragged event, 
 * dragGestureRecognized is fired and drag starts.
 *
 * Drop: Each layer in the LayerHandler listens to the drop events.
 * When a drop occurs, a list of potential targets (layers) is shown in the popup menu.
 *
 * DefaultDnDCatcher recognizes Location as the droppable object.
 *
 * DefaultDnDCatcher recognizes OMGraphicHandlerLayer layers as potential drop targets.
 */

public class DefaultDnDCatcher
    extends DnDListener
    implements BeanContextChild, BeanContextMembershipListener, PropertyChangeListener, Serializable, ProjectionListener, LayerListener, ActionListener {
    ...
    */
    ...
    /**     
     * Construct a new DefaultDnDCatcher     
     ...
     * looks for the MapBean it is managing DnD and MouseEvents for, and 
     * any layers that may be removed.
     *
     * @param bcme event that contains an iterator that can be used to
    ...
    public void drop(java.awt.dnd.DropTargetDropEvent dtde) {
        ...
        Border compoundborder = BorderFactory.createCompoundBorder(BorderFactory.createEtchedBorder(), BorderFactory.createEmptyBorder(2, 2, 2, 2));
        ...
        if (transferData instanceof Location) {
            ((Location) transferData).setLocation(dropLocation.x, dropLocation.y, proj);
        ...
        //
        popup.show(((DropTarget) dtde.getSource()).getComponent(), dropLocation.x, dropLocation.y);
    ...
    /**
     * Called when an object should be evaluated by the DefaultDnDCatcher
     ...
     * LayerHandler to get OMGraphicHandlerLayer layers to manage 
     * Drag and Drop events for.
     * If a MapBean is added to the BeanContext while another already 
     * is in use, the second MapBean will take the place of the first.
     *
    ...
    /**
     * The mouseDragged event gets interpreted as DragGestureRecognized 
     * when startDrag boolean is true. After the first mouseDragged event,
     * set startDrag to false.
     * 
     */
    public void mouseDragged(MouseEvent e) {
        Debug.message("defaultdndcatcher", "mouseDragged, startDrag=" + startDrag);
        if (startDrag) {
            startDrag = false;
            if (md.getActiveMouseMode() instanceof SelectMouseMode) {
                appendEvent(e);
                setComponent((Component) e.getSource());
                fireDragGestureRecognized(DnDConstants.ACTION_MOVE, ((MouseEvent) getTriggerEvent()).getPoint());
            }
        }
    }
    /**
    ...
    /**
     * The method is invoked when there is a change in layers 
     * property in the LayerHandler.
     */
