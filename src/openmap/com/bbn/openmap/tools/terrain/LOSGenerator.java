...
public class LOSGenerator {
    ...
    final static int DEFAULT_INVISIBLE = new OMColor(0, 0, 0, 0).getRGB();
    final static int DEFAULT_VISIBLE = new OMColor(255, 0, 255, 0).getRGB();
    final static int DEFAULT_MAYBEVISIBLE = new OMColor(255, 255, 255, 0).getRGB();
     ...
     * @param startObjHeight the elevation of point 1 above the surface,
     * in meters.  The surface elevation of the point will be looked
     * up and added to this value.
     ...
     * two end points.  Can be dependent on the Projection of the
     * current map, and based on the number of pixels between the
     * projected points.  Could also be based on the number of
     * elevation posts between the two points in the DTED database.
     ...
     * curvature into account, based on the spherical model.  The
     * slope returned is the angle from the end point to the beginning
