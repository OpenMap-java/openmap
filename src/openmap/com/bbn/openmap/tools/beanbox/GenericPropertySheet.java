 ...
 * {@link com.bbn.openmap.tools.beanbox.BeanBox} to display the properties of 
 * a bean that the user has clicked on the map. 
 * An instance of GenericPropertySheet can also be used as a custom 
...
extends JDialog implements PropertyChangeListener {
  ...
  /**
   * If an instance of this class is used as a custom editor component 
   * of a bean's property that is itself a bean, this member contains 
   * a reference to the custom editor.
   */
   ...
   * This param is usually non-null only if this is a top-level property-sheet.
   * When this param is non-null, this propertysheet will inform the BeanBox
   * whenever a property on the bean changes by calling the beanChanged method
   * on BeanBox. Additionally the propertysheet will call the editComplete 
   * method on the BeanBox when the user closes the window.
   */
  public GenericPropertySheet (Object target, int x, int y, 
  ...
  /**
   * adds a window listener to this property sheet. The windowClosing
   * method calls the editComplete method on the BeanBox associated with this
  ...
  protected void setFrameSize () {
    ...
    int approxTitleWidth = (getTitle() == null) ?
      approxControlBarWidth * 2 :
      (approxIconWidth +
       (getTitle().length() * approxFontWidth) +
       approxControlBarWidth);
    int width = (approxTitleWidth > 220)? approxTitleWidth : 220;
    setSize
      (width, approxTitleBarHeight+numEditorsToDisplay*approxFontHeight*2);
  }
   ...
   * will re-initialize to display the bean's properties when this method 
   * is called.
   */
  public void setTarget (Object bean) {
    //System.out.println("Enter> GenericPropertySheet.setTarget()");
    ...
    //System.out.println("Exit> GenericPropertySheet.setTarget()");
  }
   ...
   * called whenever one of the properties of the associated bean changes.
   * If there is a PropertyEditor associated with this property sheet, this
   * method will generate a call to the editor's setValue method.
   * If there is a BeanBox associated with this property sheet, this
   * method will generate a call to beanChanged method on the BeanBox.
   */
...
extends JPanel {
  ...
  synchronized void setTarget (Object targ) {
    //System.out.println("Enter> PropertySheetPanel.setTarget()");
  ...
  {
    //System.out.println("Enter> PropertySheetPanel.wasModified");
	...
	continue;
      ...
      //System.out.println("  editors[" + i + "]=" + editors[i]);
