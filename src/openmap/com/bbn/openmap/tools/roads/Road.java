...
// **********************************************************************
// 
// $Source: /cvs/distapps/openmap/src/openmap/com/bbn/openmap/tools/roads/Road.java,v $
// $RCSfile: Road.java,v $
// $Revision: 1.1 $
...
public class Road implements RoadObject {
	...
	// logger.warning ("" + this + " pt 0 " + points[0] + " pt 1 " + points[1] + " getSecondInter " + getSecondIntersection ());
	for (int i = 1; i < points.length; i++) {
	    LatLonPoint thisPoint = points[i].getLocation();
	    kilometers +=
		GreatCircle.
		spherical_distance(prevPoint.getLatitude(),
	...
	for (int i = 1; i < points.length; i++) {
	    ...
	    float thisLength =
		GreatCircle.
		spherical_distance(prevLat, prevLon, thisLat, thisLon);
		...
		if (deltaLon < -180f)
		    ...
		    deltaLon -= 360f;
		return new LatLonPoint(prevLat + fraction * deltaLat,
				       prevLon + fraction * deltaLon);
	    }
    ...
    public String toString() {
        return name + 
            " from " + getFirstIntersection() + " to " + getSecondIntersection() + " " + points.length + " points.";
    }
}
