...
// **********************************************************************
// 
// $Source: /cvs/distapps/openmap/src/openmap/com/bbn/openmap/tools/roads/Route.java,v $
...
public class Route implements Cloneable {
	...
	for (float snakeFactor = 1.0f; snakeFactor < 40f; snakeFactor *= 2f) {
		...
		for (Enumeration e = toDo.elements(); e.hasMoreElements(); ) {
		    ...
		    //  System.out.println ("examining " + thisIntersection);
			...
			if (nextInfo == null) {
			    ...
			    float crowsPathDistance =
				GreatCircle.spherical_distance(toLat, toLon,
                                                               nextLoc.getLatitude(),
			    ...
			    //System.out.println (" - (redo) best road for " + nextIntersection + " is " + road);
			    if (newTime + nextInfo.crowsPathHours > bestTime)
                                continue;
			    newToDo.addElement(nextIntersection);
			} else {
	...
	if (haveRoute) {
		...
		// System.err.println ("adding Road #" + (i++) + " - " + info.bestRoad);
	...
	Intersection from = (startWithFirstIntersection ?
			     roads[0].getFirstIntersection() :
