 ...
 * of whether it is a tool, and appears accordingly:
 * <P>
 * 
 * If the OMDrawingTool is being used as a tool (getUseAsTool() ==
 * true), then it will set itself to be visible. If you are putting an
 * OMDrawingTool in the OpenMap application and you want the
 ...
 * is true by default. You can set that property (visibleWhenInactive)
 * to change this behavior.
 * 
 * If the OMDrawingTool is not being used as a tool, it can be brought
 * up in a window. This window can be brought up with a right click or
 * control-click on the object being edited.
 * <P>
 * 
 * If the OMGraphic being edited doesn't want to have the
 * OMDrawingTool visible, it won't be. Neither the tool nor the option
 * to bring the window up won't be displayed with a right/control
 * click.
 * <P>
 * 
 ...
 * other components may have set on the OMDrawingTool.
 * <P>
 * 
 ...
 * GraphicAttributes object. The GraphicAttributes GUI within the tool
 * lets you change the colors, line width and line dash pattern of the
 * current OMGraphic. The GraphicAttributes conttribution to the GUI
 * is not yet implemented, but will let you change the render type and
 * line type of the OMGraphic. Finally, the EditableOMGraphic is given
 * an opportunity to change and set parameters of the OMGraphic that
 * is knows about - for instance, the EditableOMLine object will soon
 * provide an interface to set arrowheads on the lines, as well as set
 * the amount of arc a line has (it's currently not implemented).
 * <P>
 */
public class OMDrawingTool extends OMToolComponent implements DrawingTool,
    ...
    /**
     * A placeholder for the last mouse mode active before the drawing
     * tool took over.
     */
    protected MapMouseMode formerMouseMode = null;
    /**
     * The JComponent the drawing tool is servicing, usually the
     * MapBean.
     */
     ...
     * the ability to delete the OMGraphic. A combination of
     * GUI_VIA_POPUP and ALT_POPUP.
     */
    ...
    /**
     * Debug flag turned on when <B>drawingtool </B> debug flag
     * enabled.
     */
    protected boolean DEBUG = false;

    /**
     * A handle to the InformationDelegator to use for status
     * messages.
     */
     ...
     * deactivate calls. Set internally in activate() and
     * deactivate().
     */
    protected boolean activated = false;

    /**
     * Tell the drawing tool to be invisible when it is inactive. True
     * by default.
     */
    protected boolean visibleWhenInactive = true;

    /**
     * The property, visibleWhenIactive, to set to false if you want
     * that behavior.
     */
    ...
    /**
     * Create the mouse mode used with the drawing tool. Called in the
     * default empty constructor, returns a OMDrawingToolMouseMode by
     * default.
     */
     ...
     * null object. If you aren't sure of the behavior mask set in the
     * tool, and you want a particular behavior, set it before calling
     * this method.
     * 
     ...
     * can modify it. If a loader cannot be found that can handle a
     * graphic with the given classname, this method will return a
     * null object. This method gives you the option of supressing the
     * GUI for the EditableOMGraphic. If you aren't sure of the
     ...
     * user. If you aren't sure of the behavior mask set in the tool,
     * and you want a particular behavior, set it before calling this
     * method.
     * 
     * @param g OMGraphic to modify
     * @param requestor the Component that is requesting the
     ...
     * A slightly different edit method, where the EditableOMGraphic
     * is put directly into edit mode, and the mouse events
     * immediately start making modifications to the OMGraphic. If you
     * aren't sure of the behavior mask set in the tool, and you want
     * a particular behavior, set it before calling this method.
     * 
    ...
    /**
     * @return true if the OMDrawingTool is editing where it wasn't
     *         before.
     */
     ...
     * set. You can set it to null all the time. This method triggers
    ...
    /**
     * Put the message in a display line that the OMDrawingTool is
     * using.
     */
    ...
    /**
     * Turn the OMDrawingTool on with the caveaut that the
     * OMDrawingTool may be active already, and that a complete hookup
     * may not be needed. If a complete hookup is needed, this methid
     * will attach the OMDrawingTool to the MouseDelegator or the
     * canvas component it is assigned to and display the drawing
     * palette. Called automatically from the create/edit methods for
     * complete hookup. Partial hookup is called from select()
     * methods.
     */
     ...
     *        OMGraphicConstants that describes an OMAction to take on
    ...
    public void eomgChanged(EOMGEvent event) {
        ...
        if (event.shouldShowGUI() && isMask(ALT_POPUP_BEHAVIOR_MASK)) {
            ...
            /**
             * Let's see if we should bring up pop-up menu with all
             * sorts of lovely options - if the right mouse key was
             * pressed, or if the ctrl key was pressed with the mouse
             * button being released, display the option menu.
             * Otherwise, just get ready to end.
             */
