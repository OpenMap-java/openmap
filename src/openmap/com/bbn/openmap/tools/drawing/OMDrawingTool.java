...
public class OMDrawingTool
      ...
      implements DrawingTool, Serializable, PropertyChangeListener, ProjectionListener, EOMGListener, PaintListener,
      SelectionProvider {
   ...
   /**
    * A behavior mask to add a menu item to the popup that will allow the GUI to
    * appear. If the OMDrawingTool is not being used as a tool and this is set
    * along with USE_POPUP_BEHAVIOR_MASK or ALT_POPUP_BEHAVIOR_MASK, then the
    * OMDrawingTool will appear in a window when the <B>Change Appearance </B>
    * option is selected in the popup menu.
    */
    ...
    * appear to be over, and when the ctrl key or right mouse button is pressed.
    */
    ...
    * OMGraphic. Don't call this if you have already started using the tool, the
    * tool won't do anything if anything else is currently being modified.
    */
   public final static int PASSIVE_MOUSE_EVENT_BEHAVIOR_MASK = 1 << 4; // + 16
   /**
    * This behavior is used internally, when the OMDrawingTool should be told to
    * clean up as soon as it is safe.
    */
    ...
    * many objects of the same type, and don't want the gui to keep going back
    * and forth between the default and special settings. Usually set to in the
    * drawingComplete method of an EditorTool. Reset to true when showPalette is
    * called.
    */
   ...
   /**
    * Create a new OMGraphic, encased in a new EditableOMGraphic that can modify
    * it. If a loader cannot be found that can handle a graphic with the given
    * classname, this method will return a null object. If you aren't sure of
    * the behavior mask set in the tool, and you want a particular behavior, set
    ...
    * Create a new OMGraphic, encased in a new EditableOMGraphic that can modify
    * it. If a loader cannot be found that can handle a graphic with the given
    * classname, this method will return a null object. This method gives you
    * the option of suppressing the GUI for the EditableOMGraphic. If you aren't
    ...
    *        should be displayed. The behaviorMask will be adjusted accordingly.
    * @return OMGraphic of the classname given, null if the DrawingTool can't
    *         create it.
    */
   ...
   /**
    * Given an OMGraphic, wrap it in the applicable EditableOMGraphic, allow the
    * user to make modifications, and then call requestor.drawingComplete(). If
    * you aren't sure of the behavior mask set in the tool, and you want a
   ...
   /**
    * Given an OMGraphic, wrap it in the applicable EditableOMGraphic, allow the
    * user to make modifications, and then call requestor.drawingComplete().
    * This methods gives you the option to suppress the GUI from the
    * EditableOMGraphic. If you aren't sure of the behavior mask set in the
    * tool, and you want a particular behavior, set it before calling this
    * method.
    * 
    * @param g OMGraphic to modify
    * @param requestor the Component that is requesting the OMGraphic. The
    ...
    * set in the tool, and you want a particular behavior, set it before calling
    * this method.
    * 
   ...
   public EditableOMGraphic getEditableGraphic(OMGraphic g) {
      ...
      while (iterator.hasNext()) {
         ...
         try {
            ...
            if (DEBUG) {
               Debug.output("OMDrawingTool.getEditableGraphic(" + g.getClass().getName() + ") comparision couldn't find class for "
                     + key);
            }
         }
      }
   ...
   /**
    * Get the current EditableOMGraphic being used by the drawing tool. Could be
    * null if nothing valid is happening, i.e. if the OMDrawingTool isn't
      ...
      if (classnames != null) {
         for (int i = 0; i < classnames.length; i++) {
            ...
            if (etl == loader) {
               ...
               if (DEBUG) {
                  Debug.output("DrawingTool.removeLoader: loader to be removed isn't the current loader for " + classnames[i]
                        + ", ignored.");
               }
            }
         }
    ...
    * with a particular loader to create a particular graphic. But here, we just
    * deal with the actual controls over the particular graphic loaded and being
    * modified.
    * <P>
    * 
    * @return this.
    ...
    * the drawing palette. Called automatically from the create/edit methods for
    * complete hookup. Partial hookup is called from select() methods.
    */
   ...
   public synchronized void deactivate(int actionToDoWithOMGraphic) {
      if (DEBUG) {
         Debug.output("OMDrawingTool: deactivate(" + (activated ? "while active" : "while inactive") + ")");
      }
      ...
      /**
       * Just need something to allow editors to clean up with the mouse modes.
       * Only does this if the action to take is to delete the non-existent
       * graphic.
       */
    ...
    * OMDrawingTool is activated. The MapBean is used to get mouse events if the
    * MouseDelegator isn't loaded, and is also used to help out with smooth
    ...
    * object you may want to take away from the OMDrawingTool. The OMDrawingTool
    * figures out if it should disconnect itseld from the object.
    */
   ...
   public void showPalette() {
      ...
      /*
       * This repaint needs to be here because sometimes the GUI doesn't update
       * on the revalidate being called in the GUI.
       */
      repaint();
      ...
      boolean shouldBeVisible = !getUseAsTool() || (isMask(SHOW_GUI_BEHAVIOR_MASK) && getUseAsTool());
   ...
   public void showInWindow() {
      ...
      if (ws != null && !getUseAsTool()) {
         ...
         Debug.output("OMDrawingTool.showPalette(): NOT showing palette, ws == null:" + (ws == null) + ", used as tool:"
               + getUseAsTool());
      }
   }
   ...
   /**
    * PropertyChangeListener method. If DrawingAttribute parameters change, this
    * method is called, and we update the OMGraphic parameters.
    */
   ...
   public void eomgChanged(EOMGEvent event) {
      ...
      if (event.shouldShowGUI() && isMask(ALT_POPUP_BEHAVIOR_MASK)) {
         ...
         /**
          * Let's see if we should bring up pop-up menu with all sorts of lovely
          * options - if the right mouse key was pressed, or if the ctrl key was
   ...
   protected boolean doPopup(int x, int y, java.util.List additionalOptions) {
      ...
      if (showPopup) {
         ...
         if (map != null && x >= 0 && y >= 0) {
            ...
            Debug.error("OMDrawingTool: no " + (map == null ? "/component" : "/") + ((x < 0 || y < 0) ? "location/" : "/")
                  + " to show popup on!");
         }
         return true;
      }
   ...
   public JPopupMenu createPopupMenu() {
      ...
      if ((g.getAttribute(OMGraphicConstants.CHANGE_APPEARANCE)) == null
         ...
         if (isMask(SHOW_GUI_BEHAVIOR_MASK | GUI_VIA_POPUP_BEHAVIOR_MASK) && !getUseAsTool()) {
            ...
            Debug.output("Not adding Change Appearance to popup: guiViaPopup(" + isMask(SHOW_GUI_BEHAVIOR_MASK) + ") isTool("
                  + getUseAsTool() + ")");
         }
      }
