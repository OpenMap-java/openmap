...
public abstract class GeoProj
   ...
   private float[] getGreatVertices(float latp, float lonp, float latn, float lonn, Point from, Point to, boolean include_last,
      ...
      if (nsegs < 1) {
         ...
         /*
          * determine what would be a decent number of segments to draw. HACK:
          * this is hardcoded calculated by what might look ok on screen. We
          * also put a cap on the number of extra segments we draw.
          */
         nsegs = dist >> 3;// dist/8
         if (nsegs == 0) {
            nsegs = 1;
    ...
    *        dependent on radians parameter - the start parameter is in radians
    *        if radians equals true, decimal degrees if not.
    ...
    * filled or non-filled). Many of these restrictions apply to other poly-like
    ...
    * south pole and then connect these ends back the other way around the world
    * (not across the dateline) with a few extra line segments (remember the <a
    * href="#line_restrictions">line length restrictions </a>). This way you've
    * removed the polar anomaly from the data set. On the screen, all you see is
    * a sliver artifact down along the dateline. This is the very method that
    * our DCW data server shows Antarctica.
    * <p>
    ...
    * The projection library deals internally in radians, and so you're required
    * to pass in an array of radian points. See <a
    ...
    *         it'll just be the center point longitude. For LLC, it'll
    *         be the reference meridian.
    */
