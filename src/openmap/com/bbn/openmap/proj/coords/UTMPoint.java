...
public class UTMPoint {
    ...
    public static UTMPoint LLtoUTM(LatLonPoint llpoint, Ellipsoid ellip, UTMPoint utmpoint) {
        ...
        if ( Lat >= 56.0f && Lat < 64.0f &&
             Long >= 3.0f && Long < 12.0f ) {
            ZoneNumber = 32;
        }

        // Special zones for Svalbard
        if( Lat >= 72.0f && Lat < 84.0f ) {
            if(Long >= 0.0f  && Long <  9.0f ) ZoneNumber = 31;
            else if( Long >= 9.0f  && Long < 21.0f ) ZoneNumber = 33;
            else if( Long >= 21.0f && Long  < 33.0f ) ZoneNumber = 35;
            else if( Long  >= 33.0f && Long  < 42.0f ) ZoneNumber = 37;
        }
