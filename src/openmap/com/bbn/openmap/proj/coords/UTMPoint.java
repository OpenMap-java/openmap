...
public class UTMPoint {
    ...
    * @param utmpoint a UTMPoint to put the results in. If it's null, a UTMPoint
    *        will be allocated.
   ...
   public static UTMPoint LLtoUTM(LatLonPoint llpoint, Ellipsoid ellip, UTMPoint utmPoint, int zoneNumber, boolean isNorthern) {
      ...
      double UTMEasting =
            (k0
                  * N
                  * (A + (1 - T + C) * A * A * A / 6.0d + (5 - 18 * T + T * T + 72 * C - 58 * eccPrimeSquared) * A * A * A * A * A
                        / 120.0d) + 500000.0d);
    ...
    * class where the Zone can be specified as a single string eg."61N" which is
    * then broken down into the ZoneNumber and ZoneLetter.
    * 
    * @param ellip an ellipsoid definition.
   ...
   public static LatLonPoint UTMtoLL(Ellipsoid ellip, double UTMNorthing, double UTMEasting, int zoneNumber, char zoneLetter,
      ...
      double lat =
            phi1Rad
                  ...
                  * (D * D / 2 - (5 + 3 * T1 + 10 * C1 - 4 * C1 * C1 - 9 * eccPrimeSquared) * D * D * D * D / 24 + (61 + 90 * T1
                        + 298 * C1 + 45 * T1 * T1 - 252 * eccPrimeSquared - 3 * C1 * C1)
                        * D * D * D * D * D * D / 720);
      lat = ProjMath.radToDeg(lat);

      double lon =
            (D - (1 + 2 * T1 + C1) * D * D * D / 6 + (5 - 2 * C1 + 28 * T1 - 3 * C1 * C1 + 8 * eccPrimeSquared + 24 * T1 * T1) * D
                  * D * D * D * D / 120)
                  / Math.cos(phi1Rad);
