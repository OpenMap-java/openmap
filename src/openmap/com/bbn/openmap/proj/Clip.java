...
public class Clip {
	...
	//*2 for x,y pairs, *2 for potential extra points, +2 for closure,
     ...
     *
     */
    public static final int[] liang_clip(
	int xleft, int xright, int ytop, int ybottom,
	int[] x, int[] y, int n, int[] ret_val)
    {
	int i, num = 0;
	...
	for (i = 0; i < n; i++) {
		...
		if (tin1 > 0) {			/* case 5 - turning vertex */
		    ret_val[num++] = (int)
			ProjMath.qint((double) (xin));
		    ret_val[num++] = (int)
			ProjMath.qint((double) (yin));
		}
		if (tin2 <= 1) {		/* case 3, 4, 6 */
		    if (dx != 0)
			toutx = (xout - x[i]) / dx;
		    else			/* vertical */
			toutx = ((xleft <= x[i]) && (x[i] <= xright))
			...
			if (tin2 <= tout1) {	/* case 4 - visible segment */
				...
				if (tinx > tiny) {	/* vertical boundary */
					...
					ProjMath.qint(xin);
				    ret_val[num++] = (int)
					ProjMath.qint(y[i] + (tinx*dy));
				}
				else {			/* horiz boundary */
				    ret_val[num++] = (int)
					ProjMath.qint(x[i] + (tiny*dx));
				    ret_val[num++] = (int)
					ProjMath.qint(yin);
				}
				...
				if (toutx < touty) {	/* vertical boundary */
					...
					ProjMath.qint(xout);
				    ret_val[num++] = (int)
					ProjMath.qint(y[i] +(toutx*dy));
				}
				else {			/* horiz boundary */
				    ret_val[num++] = (int)
					ProjMath.qint(x[i] +(touty*dx));
				    ret_val[num++] = (int)
					ProjMath.qint(yout);
				}
			    }
    ...
    private static final int _quick_code(
	    int x, int y,
	    int xleft, int xright, int ytop, int ybottom)
    {
	...
	if (x < xleft) val = Values.LEFT; 		
	else if (x > xright) val = Values.RIGHT;
	if (y < ytop) val |= Values.TOP;
	else if (y > ybottom) val |= Values.BOTTOM;
     ...
     *
     */
    public final static int quickCheckLineClip(
	    int x1, int y1, int x2, int y2,
	    int xleft, int xright, int ytop, int ybottom)
	// xleft < xright, ytop < ybottom
    {
	/* return values:
	 * 1 = line is fully in window
	 * 0 = line is NOT fully in window.  THIS DOES NOT IMPLY THAT LINE IS
	 * PARTIALLY INSIDE WINDOW.
	 * -1 = line is NOT in window at all
	 */
	int pt1 = _quick_code(x1,y1,xleft,xright,ytop,ybottom);
	int pt2 = _quick_code(x2,y2,xleft,xright,ytop,ybottom);

	if ((pt1 & pt2)!=0) return -1;		// completely outside
	else if ((pt1 | pt2)==0) return 1;	// completely inside
	else return 0;				// partially inside?
    }

    /*
    public static void main(String[] args) {
	int xleft = 0; int xright = 1024;
	int ytop = 0; int ybottom = 1024;
	int[] ret_val = null;
	int[] xs = null, ys = null;
	...
	ret_val = liang_clip(
		xleft, xright, ybottom, ytop, xs, ys, n, ret_val);
	n = ret_val[ret_val.length-1]-2;// unclosed length

	Debug.output("num pairs = " + n);
	Debug.output("total buffer len = " + ret_val.length);
	for (int i = 0; i < n; i+=2) {
	    Debug.output(
