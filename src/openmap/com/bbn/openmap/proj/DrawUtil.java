...
public class DrawUtil {
    ...
    public final static int[] lineSegments(int x1, int y1, int x2, int y2,
                                           int n, boolean include_last, int[] ret_val) {
        if (n <= 0) {
            ret_val = new int[2];
            ret_val[0] = x1;
            ret_val[1] = y1;
     ...
     * @return Point[]
     */
    public final static Point[] lineSegments(Point pt1, Point pt2, int n, boolean include_last) {
        ...
        for (int i = 1; i < end; i++, t += inc) {
            ret_val[i] = new Point(pt1.x+(int)((float)v.x*t),
                                   pt1.y+(int)((float)v.y*t));
        }
        return ret_val;
    }
...
//                         x1 + "," + y1 + ")->(" + x2 + "," + y2 + ")");
     ...
     * @return boolean
     */
    public final static boolean inside_polygon(int[] xpts, int[] ypts, int ptx, int pty) {
    ...
    /**
     * Returns the distance from Point (x,y) to the closest line segment
    ...
    public final static float closestPolyDistance(int[] xpts, int[] ypts, 
                                                  int ptx, int pty, boolean connected) {
        if (xpts.length == 0) return Float.POSITIVE_INFINITY;
        if (xpts.length == 1) return distance(xpts[0], ypts[0], ptx, pty);

        float temp, distance = Float.POSITIVE_INFINITY;
        int i, j;

        for (i=0, j=1; j<xpts.length; i++, j++) {
            temp = distance_to_line(xpts[i], ypts[i], xpts[j], ypts[j], ptx, pty);
//          Debug.output(
...
//              temp);
        ...
        // connect
        if (connected) {
            temp = distance_to_line(xpts[i], ypts[i], xpts[0], ypts[0], ptx, pty);
            if (temp < distance) distance = temp;
        }
        return distance;
    }
    ...
    /**
     * Calculate the "pixel distance" between two points (squaring
     * not envolved).
     * <p>
     * @param x1 x coord
     * @param y1 y coord
     ...
     */
    public final static int pixel_distance(
     ...
     *
     */
    public final static float OLD_distance_to_line(
        int x1, int y1, int x2, int y2, int x, int y) {
     ...
     *
     */
    public final static float perpendicular_distance_to_line(
        ...
        return Math.abs((/*Math.abs*/(x12*y13) - /*Math.abs*/(x13*y12))/D12);
    }

    /**
     * Computes the distance from a point to a line segment.
     * <p>
     * Variable usage as follows:<p>
     * <ul>
     ...
     * the line as one travels from the start point to the end point.<p>
     *
     * @param x1 line x coord1
     * @param y1 line y coord1
     * @param x2 line x coord2
    ...
    /**
     * Generates a line with width lw, returns an ArrayList of 4
     * x-y coords.
     * <p>
     * @param lw line width
     * @param x1 line x coord1
     ...
     */
    public static ArrayList generateWideLine(
...
//      lw = lw -1;
     ...
     *
     */
    final public static ArrayList generateWidePoly(
...
//      System.out.print("DrawUtil.generateWidePoly Points for lw="+lw);
//      for (int i=0;i<len;i++) {
...
//      Debug.output("");
    ...
    public static void main(String[] args) {
        ...
        Debug.output("OLD_distance_to_line(-34,12,44,104, -44,-50): " +
                           OLD_distance_to_line(-34,12,44,104, -44,-50));
        System.exit(0);
        ...
        xpts[1] = 2; ypts[1] = 0;
        xpts[2] = 2; ypts[2] = 2;
        Debug.output(closestPolyDistance(xpts, ypts, 0,1, true));//round
        Debug.output(closestPolyDistance(xpts, ypts, 0,1, false));//1

        // linewidth testing

        Debug.output("");
        ArrayList vec = generateWideLine(3, 0, 0, 5, 5);

        int[] x = (int[])vec.elementAt(0);
        int[] y = (int[])vec.elementAt(1);
        ...
        x = (int[])vec.elementAt(0);
        y = (int[])vec.elementAt(1);
        System.out.print("wide line: ");
        for (int i = 0; i<x.length; i++) {
            System.out.print(x[i] + "," + y[i] + " ");
        }
        ...
        for (int j = 0; j < size; j+=2) {
            x = (int[])vec.elementAt(j);
            y = (int[])vec.elementAt(j+1);
            System.out.print("wide poly: ");
            for (int i = 0; i<x.length; i++) {
                System.out.print(x[i] + "," + y[i] + " ");
            }
            Debug.output("");
